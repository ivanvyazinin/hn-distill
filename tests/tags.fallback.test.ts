...
mock.module("@config/openrouter", () => ({
  TAGS_FALLBACK_MODELS: ["default-model", "fallback-model"] as const,
}));
...
describe("tags bulk fallback handling", () => {
  test("switches to a different model after rate limit", async () => {
    const {
      runTagsWorkflowWithFallback,
      __setTagsWorkflowRunnerForTests,
    ...
    const workflowMock = mock(async (
      _services: unknown,
      _storyIds: number[],
      customEnv: { TAGS_MODEL: string }
    ) => {
    ...
    try {
      ...
      expect(usedModels).toEqual(["default-model", "fallback-model"]);
    } finally {
      __resetTagsModelRotationForTests();
      mock.restore();
    }
