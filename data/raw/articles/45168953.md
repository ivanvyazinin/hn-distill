Experimenting with local LLMs on macOS | Fatih's Personal Blog

### [Fatih's Personal Blog](/)

# Experimenting with local LLMs on macOS

September 08, 2025 · 9 minutes to read
...
[![classic orihime with leek meme, but with multicolored "you're absolutely correct" overlaid on top of it](/static/207dbd07fb221f623d3055156906c562/1c72d/orihime_leek.jpg)](/static/207dbd07fb221f623d3055156906c562/0c02e/orihime_leek.jpg)
...
If you still want to run an LLM on your macOS[1](#fn-1), let’s get started. There are two options that I recommend, one is open-source and the other is easier to use, as all things are. I only cover macOS because that’s what I have; if that’s not your platform, you can still follow this guide with platform-specific tweaks, or find another guide.
...
## [](#how-to-choose-a-good-llm-for-your-use-case)How to choose a good LLM for your use case
...
### [](#finding-a-good-model)Finding a good model
...
Let me part with a final tip: LM Studio shows you how much of the context window is being used. So you may find it useful to ask for it to summarize the conversation so far, when the context window gets close to being filled. This way you can help it remember important information that it would otherwise forget.[4](#fn-4)
...
1.  With an M-series, [Apple Silicon](https://en.wikipedia.org/wiki/Apple_silicon) chipset, Intel chips are pretty old at this point and wouldn’t run LLMs well[↩](#fnref-1)
2.  You may be wondering about performance and if it will be usable. When you generate a model response, two things happen in sequence; prompt processing and token generation. Prompt processing is tokenizing the entire chat history, is done in parallel, and compute bound. Token generation is sequential, and is memory bandwidth bound. Both of these things get slower as the context size increases.  
    Also, counterintuitively, these models don’t run on the [Neural Engine](https://en.wikipedia.org/wiki/Neural_Engine). Both `llama.cpp` and MLX run on the GPU, using Metal shaders. I’m not entirely sure why though.[↩](#fnref-2)
3.  It’s called that because the common wisdom is that models reason with tokens, and a smaller model can generate a higher quality response if it generates more tokens. To protect the user from the burden of reading through more slop, these reasoning tokens are usually hidden from the user.[↩](#fnref-3)
4.  This is called _compaction_ and, coding tools such as [Claude Code](https://docs.anthropic.com/en/docs/claude-code/costs#reduce-token-usage) can do this automatically, or provide a command for you to trigger it manually.[↩](#fnref-4)

[Share on Bluesky](https://bsky.app/intent/compose?text=You should totally read "Experimenting with local LLMs on macOS" https%3A%2F%2Fblog.6nok.org%2Fexperimenting-with-local-llms-on-macos%2F) · [Edit on GitHub](https://github.com/frontsideair/blog/edit/main/content/blog/experimenting-with-local-llms-on-macos/index.md)

* * *

![Fatih Altinok](/static/65b7df1976a236ae672b05a53975bc2f/cb4b7/profile-pic.jpg)

Written by [**Fatih Altinok**](https://bsky.app/profile/fatih.6nok.org), who cares a lot about user experience, teamwork and functional programming. [Learn more about me →](/about/)

*   [← Tailscale is pretty useful](/tailscale-is-pretty-useful/)
...
I promise I won't send you spam or sell your email. [Powered by Buttondown.](https://buttondown.email/refer/frontsideair)

[Bluesky](https://bsky.app/profile/fatih.6nok.org) · [GitHub](https://github.com/frontsideair)[RSS](/rss.xml)