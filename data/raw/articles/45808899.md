...
November 2, 2025 [Hacker News](https://news.ycombinator.com/item?id=45808899) [Lobsters](https://lobste.rs/s/ooxamp/jvm_exceptions_are_weird_decompiler)
...
Perhaps the most confusing implication is that while exception handling ranges can cross control flow constructs (e.g. it’s possible for `from` to be located outside an `if` and for `to` br inside an `if`), ranges of _exemption_ from EH correspond to single positions in source code, and thus cannot cross control flow. So in the eyes of a decompiler, the code above should be parsed like this:
