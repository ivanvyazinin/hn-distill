...
Mon Oct 06 2025authored by [veritas](/author/veritas)
...
The idea is simple, we create a content script that runs at document\_start that injects a tiny bit of code that replaces `Array.prototype.push` with a wrapper that logs and then calls the original.
...
> **Editor's Note**: Some readers have kindly pointed out to me that this _is_ actually possible to accomplish from an extension. However, the custom browser approach still has benefits explained later in the post :-)
...
We can see a value being pushed to an array in the console
...
Functions calls appearing in our sidebar after hooks have been fired
...
Canvas and other various hooks firing on Tiktok.com's homepage
...
I wanted to run this against more anti-bots. Out of curiosity I pointed the demo at a site using Cloudflare’s Turnstile. I _knew_ Turnstile was collecting various browser signals, but to my surprise, my sidebar showed nothing. Why was I seeing zero logs?
...
The Turnstile widget is enclosed inside of an iframe in a closed shadow root
...
A stream of events appearing when navigating to a Cloudflare IUAM page
...
The default "Hello World" page running in our custom Electron build
...
From there, you define an _agent_, the bridge that connects Blink’s internals to the DevTools Protocol so your new CDP domain can send and receive events.
...
While digging through how other events were dispatched, I noticed something interesting. The agents weren’t called directly. Instead, events were emitted through _probes_. These probes act as intermediary hooks that Blink uses to fire instrumentation events into the DevTools pipeline.
...
The Canvas toDataURL operation was caught by our custom build and sent through CDP!
...
Obfuscation tab running against a pretend CTF challenge

### Overwriting properties and functions

Hooking and reading is fun, but sometimes you want to _change_ behavior. You now know that doing so in a browser environment, across OOPIFs and with anti-tamper checks is non-trivial. I built an **Overrides** tab where you can define custom JavaScript snippets that overwrite functions or properties across _all_ frames. These execute without triggering common integrity checks, giving a clean way to spoof or alter these values.

Your browser does not support the video tag.

Overriding navigator.userAgent and navigator.vendor from the Overrides tab
...
FingerprintJS tab that shows the decoded payload in a nice table
...
2.  **Hook all the things**  
    I’ve already implemented a broad set of hooks. Canvas, WebGL, audio fingerprinting, `navigator` accessors, document and window properties, and more. But can we hook _everything_?  
...
This one is pretty festive! It includes two pandas, Santa Claus, and a rainbow.
...
![A piece of a pangram whilst also being very happy!](/posts/reverse-engineering-browser/pangram.png)

A piece of a pangram whilst also being very happy!
...
![veritas profile picture](https://avatars.githubusercontent.com/u/25884226)

Find veritas on:twitter: [https://twitter.com/blastbots](https://twitter.com/blastbots)fedi: [https://infosec.exchange/@voidstar](https://infosec.exchange/@voidstar)bluesky: [https://bsky.app/profile/1999.nyc](https://bsky.app/profile/1999.nyc)discord: nullptrs

Content on this site is licensed [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/)[RSS](/feed.rss)