...
Firstly, Anthropic has been suffering from pretty [terrible](https://status.anthropic.com/incidents/s1cd6vc8wms9) [reliability](https://status.anthropic.com/incidents/pstkfl3t9jfd) [problems](https://status.anthropic.com/incidents/k6gkm2b8cjk9) . And looking at OpenRouter's data they are not alone (nb. OpenRouter's data is not conclusive, but I believe it does give a somewhat interesting overview of reliability). ![OpenRouter API reliability data](/img/openrouter-reliability-data.png) ![OpenRouter model availability stats](/img/openrouter-model-availability.png) If you've been using coding agents, you'll know how flakey they can be, often getting stuck and requiring retries, a bit like your 56k's modem dropping in bad weather or someone wanting to do a call.
...
![OpenRouter token usage growth chart](/img/openrouter-token-usage-growth.png) Again, we must caveat this data strongly:
...
I've been playing around with [Cerebras Code](https://www.cerebras.ai/blog/introducing-cerebras-code) which (was) a fork of Gemini CLI produces tokens 20-50x faster (very similar to the leap from dialup to the first ADSL/cable modems in speed increase).
...
A lot of the discourse in the press is expecting something similar to what happened in the early 2000s [telecoms crash](https://en.wikipedia.org/wiki/Telecoms_crash) - where capacity was built out far faster than consumption (and in recent years, broadband bandwidth consumption has virtually plateaued - growing only 10-15% y/y in many markets). While I'm not ruling out some pullback in datacentre construction, I don't see the fundamental demand curve flatlining in the same way.
...
I think this will then result in less advantageous pricing models for developers, which are very 'unrefined'. While I don't think [inference is a huge loss leader](https://martinalderson.com/posts/are-openai-and-anthropic-really-losing-money-on-inference/); there are clearly huge challenges for the providers at 'peak times'. These tend to be when both the US market and Europe market overlap in work hours.
...
Each of these 'phases' of LLM growth is unlocking a lot more developer productivity, _for teams and developers that know how to harness it_. I think there is a lot of change coming to how software engineers work and a lot of developers and teams are not prepared for it.
...
I don't think we're in a transition period heading towards stability any time soon and it feels like there is still so much [low hanging fruit](https://martinalderson.com/posts/claude-code-static-analysis/) to improve agents on a tooling level, never mind what would be possible with much faster models.
...
Thanks for reading! I'm really excited about consulting projects around AI tooling adoption, LLM integration, and helping organizations like yours leverage these technologies effectively.

[Get in touch](/contact/) if you'd like to work together.

[← Back to Home](/)

© 2025 Martin Alderson | [RSS](/feed.xml)