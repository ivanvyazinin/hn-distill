...
Every systems engineer at some point in their journey yearns to write a filesystem. This sounds daunting at first - and writing a battle-tested filesystem _is_ hard - but the minimal surface area for a “working” FS is surprisingly small, simple, and in-distribution for coding agents.
...
# Training a filesystem [#](#training-a-filesystem)
...
I then wrote a filesystem interaction simulator, which sampled various operations against a sandboxed `LoggingLoopbackFS` to generate diverse FUSE prompt/completion pairs. Concretely, I captured only the minimal set of [operations needed](https://github.com/rohangpta/llmfuse/blob/main/train/generate_data.py#L246) for R/W-ish capability (no open, xattrs, fsync etc).
...
![](/images/llmfuse_example.png) Poking around a Docker container with a mounted LLMFuse.

# Compressing the filesystem [#](#compressing-the-filesystem)
...
As it turns out, compression and AI are intimately related. Using LLMs to lossily compress text is one of the most common applications, so it’s not entirely unintuitive. However, one researcher (Marcus Hutter) claimed back in 2006 that they are _equivalent_ (and in fact [bet $500K on this claim!](http://prize.hutter1.net/hfaq.htm#about)).
...
Researchers at DeepMind (including Hutter himself) have [explained the math in detail](https://arxiv.org/pdf/2309.10668), so I’ll direct the most inquisitive of you readers there, but for a simplified understanding of what’s going on, forget everything you might know about working with LLMs today. There’s no prompting involved!
...
Let’s suppose in our example this turns out to be 0.012. We can represent this decimal in roughly \\(- \\log\_{2}{P(m)} = 6.4\\) bits, which is our final compression size.

There’s a few elegant things about this algorithm:

*   _Any_ number within this interval is uniquely determined by tracing the arithmetic coding algorithm through the specific probabilistic model’s weights. “Decoding” is simply a retracing operation (see the line through the probability distributions above)
...
Sounds cool! But **how good really** is this compression? On comparing arithmetic coding backed by `Qwen3-4B` against `gzip` for [`lipsum.txt`](https://www.lipsum.com/), we already see pretty dramatic results:
...
Self-compression in filesystems isn’t a novel concept. For example, there exists the [`squashfs`](https://docs.kernel.org/filesystems/squashfs.html) tool (created in 2002) to create R/O compressed filesystems. Squashfs compresses files, inodes, and directories together, not unlike what we’re doing here!
...
gzip-compressed file contents, inodes, directory tables

llmfuse (fine-tuned)

21 bytes

Arithmetic coded XML state
...
What started off as a little experiment mostly to get my hands dirty with training and inference evolved into a full blown [nerd snipe](https://xkcd.com/356/) and intellectual adventure. Thanks for making it this far!
...
## SquashFS comparison methodology [#](#squashfs-comparison-methodology)
...
## Compression as a metric [#](#compression-as-a-metric)
