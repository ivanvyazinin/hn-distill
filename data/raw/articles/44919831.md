A Visual Exploration of Gaussian Processes      A Visual Exploration of Gaussian Processes                                                        

[Distill](/)

[About](/about/) [Prize](/prize/) [Submit](/journal/)
...
[Jochen Görtler](http://www.jgoertler.com)

[University of Konstanz](https://www.cgmi.uni-konstanz.de)

[Rebecca Kehlbeck](https://www.cgmi.uni-konstanz.de)

[University of Konstanz](https://www.cgmi.uni-konstanz.de)

[Oliver Deussen](https://www.cgmi.uni-konstanz.de/en/persons/prof-dr-oliver-deussen/)
...
[10.23915/distill.00017](https://doi.org/10.23915/distill.00017)
...
Σ\=Cov(Xi,Xj)\=E\[(Xi−μi)(Xj−μj)T\] \\Sigma = \\text{Cov}(X\_i, X\_j) = E \\left\[ (X\_i - \\mu\_i)(X\_j - \\mu\_j)^T \\right\] Σ\=Cov(Xi​,Xj​)\=E\[(Xi​−μi​)(Xj​−μj​)T\]
...
Through _marginalization_ we can extract partial information from multivariate probability distributions. In particular, given a normal probability distribution P(X,Y)P(X,Y)P(X,Y) over vectors of random variables XXX and YYY, we can determine their marginalized probability distributions in the following way:

X∼N(μX,ΣXX)Y∼N(μY,ΣYY) \\begin{aligned} X &\\sim \\mathcal{N}(\\mu\_X, \\Sigma\_{XX}) \\\\ Y &\\sim \\mathcal{N}(\\mu\_Y, \\Sigma\_{YY}) \\end{aligned} XY​∼N(μX​,ΣXX​)∼N(μY​,ΣYY​)​

The interpretation of this equation is that each partition XXX and YYY only depends on its corresponding entries in μ\\muμ and Σ\\SigmaΣ. To marginalize out a random variable from a Gaussian distribution we can simply drop the variables from μ\\muμ and Σ\\SigmaΣ.

pX(x)\=∫ypX,Y(x,y)dy\=∫ypX∣Y(x∣y)pY(y)dy p\_X(x) = \\int\_y p\_{X,Y}(x,y)dy = \\int\_y p\_{X|Y}(x|y) p\_Y(y) dy pX​(x)\=∫y​pX,Y​(x,y)dy\=∫y​pX∣Y​(x∣y)pY​(y)dy

The way to interpret this equation is that if we are interested in the probability density of X\=xX = xX\=x, we need to consider all possible outcomes of YYY that can jointly lead to the result The corresponding [Wikipedia article](https://en.wikipedia.org/wiki/Marginal_distribution) has a good description of the marginal distribution, including several examples. .
...
Now that we have the basic framework of Gaussian processes together, there is only one thing missing: how do we set up this distribution and define the mean μ\\muμ and the covariance matrix Σ\\SigmaΣ? The covariance matrix Σ\\SigmaΣ is determined by its _covariance function_ kkk, which is often also called the _kernel_ of the Gaussian process. We will talk about this in detail in the next section. But before we come to this, let us reflect on how we can use multivariate Gaussian distributions to estimate function values. The [following figure](#PriorFigure) shows an example of this using ten test points at which we want to predict our function:

In Gaussian processes we treat each test point as a random variable. A multivariate Gaussian distribution has the same number of dimensions as the number of random variables. Since we want to predict the function values at ∣X∣\=N|X|=N∣X∣\=N test points, the corresponding multivariate Gaussian distribution is also NNN -dimensional. Making a prediction using a Gaussian process ultimately boils down to drawing samples from this distribution. We then interpret the iii\-th component of the resulting vector as the function value corresponding to the iii\-th test point.

### Kernels

Recall that in order to set up our distribution, we need to define μ\\muμ and Σ\\SigmaΣ. In Gaussian processes it is often assumed that μ\=0\\mu = 0μ\=0, which simplifies the necessary equations for conditioning. We can always assume such a distribution, even if μ≠0\\mu \\neq 0μ≠0, and add μ\\muμ back to the resulting function values after the prediction step. This process is also called _centering_ of the data. So configuring μ\\muμ is straight forward — it gets more interesting when we look at the other parameter of the distribution.

The clever step of Gaussian processes is how we set up the covariance matrix Σ\\SigmaΣ. The covariance matrix will not only describe the shape of our distribution, but ultimately determines the characteristics of the function that we want to predict. We generate the covariance matrix by evaluating the kernel kkk, which is often also called _covariance function_, pairwise on all the points. The kernel receives two points t,t′∈Rnt,t’ \\in \\mathbb{R}^nt,t′∈Rn as an input and returns a similarity measure between those points in the form of a scalar:

k:Rn×Rn→R,Σ\=Cov(X,X′)\=k(t,t′) k: \\mathbb{R}^n \\times \\mathbb{R}^n \\rightarrow \\mathbb{R},\\quad \\Sigma = \\text{Cov}(X,X’) = k(t,t’) k:Rn×Rn→R,Σ\=Cov(X,X′)\=k(t,t′)
...
Again, we can use conditioning to derive the predictive distribution PX∣YP\_{X|Y}PX∣Y​. In this formulation, ψ\\psiψ is an additional parameter of our model.
...
Through the _marginalization_ of each random variable, we can extract the respective mean function value μi′\\mu’\_iμi′​ and standard deviation σi′\=Σii′\\sigma’\_i = \\Sigma’\_{ii}σi′​\=Σii′​ for the iii\-th test point. In contrast to the prior distribution, where we set the mean to μ\=0\\mu=0μ\=0, the result of conditioning the joint distribution of test and training data will most likely have a non-zero mean μ′≠0\\mu’ \\neq 0μ′≠0. Extracting μ′\\mu’μ′ and σ′\\sigma’σ′ does not only lead to a more meaningful prediction, it also allows us to make a statement about the confidence of the prediction.
...
Let’s consider two kernels, a linear kernel klink\_{\\text{lin}}klin​ and a periodic kernel kperk\_{\\text{per}}kper​, for example. This is how we would multiply the two:

k∗(t,t′)\=klin(t,t′)⋅kper(t,t′) k^{\\ast}(t,t’) = k\_{\\text{lin}}(t,t’) \\cdot k\_{\\text{per}}(t,t’) k∗(t,t′)\=klin​(t,t′)⋅kper​(t,t′)

However, combinations are not limited to the above example, and there are more possibilities such as concatenation or composition with a function. To show the impact of a kernel combination and how it might retain qualitative features of the individual kernels, take a look at the [figure below](#KernelCombinationsStatic). If we add a periodic and a linear kernel, the global trend of the linear kernel is incorporated into the combined kernel. The result is a periodic function that follows a linear trend. When combining the same kernels through multiplication instead, the result is a periodic function with a linearly growing amplitude away from linear kernel parameter ccc.
...
We are very grateful to Carla Avolio and Marc Spicker for their feedback on the manuscript. In addition, we want to thank Jonas Körner for helping with the implementation of the figure explaining the multivariate Gaussian distribution. Furthermore, we would like to thank the German Research Foundation (DFG) for financial support within project [A01](http://www.sfbtrr161.de/research/project_a01/index.html) of the [SFB-TRR 161](http://www.sfbtrr161.de) and within the [Research Unit FOR 2111](https://typo.uni-konstanz.de/questionsInterfaces/) with grant number DFG-431/16.

### Discussion and Review

[Review 1 - Anonymous](https://github.com/distillpub/post--visual-exploration-gaussian-processes/issues/1)  
[Review 2 - Anonymous](https://github.com/distillpub/post--visual-exploration-gaussian-processes/issues/3)  
[Review 3 - Austin Huang](https://github.com/distillpub/post--visual-exploration-gaussian-processes/issues/4)  
...
*   [Interactive visualization of Gaussian processes](http://www.infinitecuriosity.org/vizgp/) by ST John that joins together the different concepts introduced throughout this article.
*   [Gaussian process regression demo](http://www.tmpl.fi/gp/) by Tomi Peltola
*   [Gaussian Processes for Dummies](http://katbailey.github.io/post/gaussian-processes-for-dummies/) by Katherine Bailey
*   [Intuition behind Gaussian Processes](https://blog.sigopt.com/posts/intuition-behind-gaussian-processes) by Mike McCourt
*   [Fitting Gaussian Process Models in Python](https://blog.dominodatalab.com/fitting-gaussian-process-models-python/) by Chris Fonnesbeck
*   [A Practical Guide to Gaussian Processes](https://tinyurl.com/guide2gp) by Marc Peter Deisenroth, Yicheng Luo, and Mark van der Wilk: heuristics for initializing and optimizing Gaussian processes

If you want more of a hands-on experience, there are also many Python notebooks available:

*   [Fitting Gaussian Process Models in Python](https://blog.dominodatalab.com/fitting-gaussian-process-models-python/) by Chris Fonnesbeck
*   [Gaussian process lecture](http://nbviewer.jupyter.org/github/adamian/adamian.github.io/blob/master/talks/Brown2016.ipynb/) by Andreas Damianou

 

### References

1.  Gaussian Processes in Machine Learning  [\[PDF\]](http://www.gaussianprocess.org/gpml/chapters/RW.pdf)  
    Rasmussen, C.E., 2004. Advanced Lectures on Machine Learning, pp. 63--71. Springer Berlin Heidelberg. [DOI: 10.1007/978-3-540-28650-9\_4](https://doi.org/10.1007/978-3-540-28650-9_4)
2.  Gaussian Processes for Object Categorization  [\[link\]](https://doi.org/10.1007/s11263-009-0268-3)  
    Kapoor, A., Grauman, K., Urtasun, R. and Darrell, T., 2010. International Journal of Computer Vision, Vol 88(2), pp. 169--188. [DOI: 10.1007/s11263-009-0268-3](https://doi.org/10.1007/s11263-009-0268-3)
3.  Clustering Based on Gaussian Processes  [\[PDF\]](http://staging.csml.ucl.ac.uk/archive/talks/41e4e55330421a6e230d0ea2b89440ea/Paper_16.pdf)  
    Kim, H. and Lee, J., 2007. Neural Computation, Vol 19(11), pp. 3088--3107. MIT Press - Journals. [DOI: 10.1162/neco.2007.19.11.3088](https://doi.org/10.1162/neco.2007.19.11.3088)
4.  Covariance matrix (Encyclopedia of Mathematics)  
    1994\. Springer Science+Business Media B.V. / Kluwer Academic Publishers.
5.  The Nature of Statistical Learning Theory  
    Vapnik, V.N., 2000. Springer New York. [DOI: 10.1007/978-1-4757-3264-1](https://doi.org/10.1007/978-1-4757-3264-1)
6.  Automatic model construction with Gaussian processes  [\[link\]](https://www.cs.toronto.edu/~duvenaud/cookbook/)  
    Duvenaud, D., 2014. University of Cambridge. PhD Thesis.
7.  Information theory, inference and learning algorithms  
    MacKay, D.J., 2003. Cambridge University Press.
8.  Using Deep Belief Nets to Learn Covariance Kernels for Gaussian Processes  [\[PDF\]](http://papers.nips.cc/paper/3211-using-deep-belief-nets-to-learn-covariance-kernels-for-gaussian-processes.pdf)  
    Hinton, G.E. and Salakhutdinov, R.R., 2008. Advances in Neural Information Processing Systems 20, pp. 1249--1256. Curran Associates, Inc.
9.  Deep Kernel Learning  [\[PDF\]](http://proceedings.mlr.press/v51/wilson16.pdf)  
    Wilson, A.G., Hu, Z., Salakhutdinov, R. and Xing, E.P., 2016. Proceedings of the 19th International Conference on Artificial Intelligence and Statistics, Vol 51, pp. 370--378.
10.  Deep Neural Networks as Gaussian Processes  [\[PDF\]](https://arxiv.org/pdf/1711.00165v3.pdf)  
    Lee, J., Sohl-Dickstein, J., Pennington, J., Novak, R., Schoenholz, S. and Bahri, Y., 2018. International Conference on Learning Representations.
11.  Deep Gaussian Processes  [\[PDF\]](http://proceedings.mlr.press/v31/damianou13a.pdf)  
...
If you see mistakes or want to suggest changes, please [create an issue on GitHub](https://github.com/distillpub/post--visual-exploration-gaussian-processes/issues/new).

### Reuse

Diagrams and text are licensed under Creative Commons Attribution [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/) with the [source available on GitHub](https://github.com/distillpub/post--visual-exploration-gaussian-processes), unless noted otherwise. The figures that have been reused from other sources don’t fall under this license and can be recognized by a note in their caption: “Figure from …”.
