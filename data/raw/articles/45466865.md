...
### Open-ended automation evaluation for agents in Factorio

v0.3.0 SDK Release

  

[Code](https://github.com/JackHopkins/factorio-learning-environment) [Paper](https://arxiv.org/abs/2503.09617)  [![](../assets/images/twitch.png)Claude Plays Factorio](https://www.twitch.tv/playsfactorio)
...
Fabricate 16 Batteries  
per minute

Manufacture 16 Red Science  
per minute
...
Claude builds factories in lab-play
...
#### ![](../assets/images/gymnasium_black.svg)OpenAI Gym Compatibility
...
See the [GitHub repository](https://github.com/JackHopkins/factorio-learning-environment) for detailed documentation and configuration options.
...
✔ Power Setup ✔ Iron Mining ✔ Assembler Machine ✗ Logistics Attempt ✔ Logistics Complete
...
![Before state](../assets/0.3.0_vids/iron_gear_wheel/initial1.gif)

#### State After Action

![After animation](../assets/0.3.0_vids/iron_gear_wheel/15_17_engine_setup.gif)
...
print(f"Power generation verified! Steam engine producing {steam_engine.energy} energy")
```
...
![Before state](../assets/0.3.0_vids/iron_gear_wheel/15_17_engine_setup.gif)

#### State After Action

![After animation](../assets/0.3.0_vids/iron_gear_wheel/34_36_furnaces.gif)
...
![Before state](../assets/0.3.0_vids/iron_gear_wheel/48_50_furnaces.gif)

#### State After Action

![After animation](../assets/0.3.0_vids/iron_gear_wheel/68_70_assembler_inserters.gif)
...
print(f"Connected inserters to power")
```

### Attempting to Connect Mining to Assembler
...
![Before state](../assets/0.3.0_vids/iron_gear_wheel/68_70_assembler_inserters.gif)

#### State After Action

![After animation](../assets/0.3.0_vids/iron_gear_wheel/86_88_inserter_nochests.gif)
...
![Before state](../assets/0.3.0_vids/iron_gear_wheel/86_88_inserter_nochests.gif)

#### State After Action

![After animation](../assets/0.3.0_vids/iron_gear_wheel/95_97_running.gif)
...
print("Belt connections complete - waiting for iron flow")
```
...
We replicate the methodology from the [original FLE paper](https://arxiv.org/pdf/2503.09617) for the **lab-play** setting to evaluate the strongest models as of September 2025.
...
*   **Reasoning:** default settings (`{"enabled": true}`) for models that support reasoning.

## Model Performance on Lab-Play Tasks (Pass@8 - Throughput Level 1)

![FLE 0.3.0 Combined Model Performance Heatmap](../assets/images/fle_0_3_0_combined_dependency_sorted_heatmap_v3.png?v=5)
...
FLE lab-play clearly differentiates between the capabilities of the frontier models. Notably, the rank and performance gaps between the most advanced models (Claude > GPT > Gemini > Grok) is most similar to [GDPVal](https://openai.com/index/gdpval/), a novel benchmark recently released to measure progress in automating economically valuable tasks. This is in contrast to many other static exam-like benchmarks including Humanity's Last Exam, AIME 25, GPQA and MMMU where weaker models in FLE achieve higher performance.

While successful agents achieve their throughput goals, many rely on semi-manual strategies rather than building robust automation for more complex tasks. This manifests in agents shuttling resources manually and using storage chests as resource buffers, instead of constructing fully automated logistics chains. Although this _does_ achieve progress toward the target, it creates a local optimum where agents shortcut the more difficult but necessary step of full automation. Throughput is challenging to measure consistently due to these buffering effects. Agents can store items in intermediate buffers (like chests or belts) which temporarily satisfy throughput checks without true sustained production. We mitigate this by enforcing a holdout period during eval, in which an agent must leave their factory alone for 60 seconds before we test whether quotas are met. Higher throughput targets would make it infeasible to pass with manual logistics, forcing agents to build proper automation.
...
![Mean Error Rate for Step Number by Model](../assets/images/error_rate_heatmap_by_program_index.png)
...
*   **Planning and Control Errors:** Even when primitives are known, agents fail to chain actions coherently, resulting in inefficient or incomplete trajectories. _Note: This category cannot be reliably quantified through automated trajectory analysis, as it requires evaluating higher-level strategic coherence rather than individual error types._

  

## Failure modes vary across frontier models

![Error Category Distribution by Model](../assets/images/fle_0_3_0_error_types_by_model.png)
...
[![Claude Code Plays Factorio on Twitch](../assets/images/claude-code-plays.png)](http://twitch.tv/playsfactorio)
...
](https://www.reddit.com/r/factorio/comments/kmo1m0/10_spm_base/#lightbox)[30 Winning the game in 1h 18m

launch a 🚀



](https://www.youtube.com/watch?v=79T4Gp0RlfY)[100 100 SPM Base YouTube Showcase](https://www.youtube.com/watch?v=11IFvIMz2EM) [300 300 SPM Factory Tour](https://www.youtube.com/watch?v=Qy3Jz8uZSuA) [1K 1K SPM Megabase Tour](https://www.youtube.com/watch?v=hWOZiN1kaAc) [3K 3K SPM Megabase Showcase](https://www.youtube.com/watch?v=jSSRdt5ieGo) [10K 10K SPM Megabase YouTube Tour](https://www.youtube.com/watch?v=zRYQcVb_5W0)
...
Factorio is an exceptional platform for evaluating agents, due to its emphasis on system engineering and unbounded sandbox nature. FLE presents numerous research opportunities for evaluating agents with valuable system-level problem-solving abilities (our perspective is detailed in this [position paper](https://arxiv.org/abs/2502.01492)) - including:
...
1.  **Researchers** exploring novel architectures for long-horizon planning and spatial reasoning
2.  **Engineers** optimizing infrastructure for large-scale evaluation and training
3.  **Modders** designing new challenge domains

If you are interested in joining our team, you can find us on [Discord](https://discord.gg/zKaV2skewa). We look forward to seeing you!

This release was assembled by [Neel Kant](http://neelkant.io), [Jack Hopkins](https://www.unicef.org.uk/donate/donate-now-to-protect-children-in-ukraine/), [Harshit Sharma](https://www.linkedin.com/in/hrshtt/), [Kian Kyars](https://x.com/neuralkian).

With thanks to Yasaman Ansari[,](https://www.linkedin.com/in/yasaman-ansari/) [Mart Bakler](https://www.linkedin.com/in/m%C3%A4rt-bakler/), [Dipika Khullar](https://www.linkedin.com/in/dipikakhullar), [Akbir Khan](https://akbir.dev/) for their contributions, and to the [Factorio team](https://www.factorio.com/).

Website template borrowed from [NeRFies](https://github.com/nerfies/nerfies.github.io), [CLIPort](https://github.com/cliport/cliport.github.io), [Voyager](https://voyager.minedojo.org/) and [VIMA](https://github.com/vimalabs/vimalabs.github.io).