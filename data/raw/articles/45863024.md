GitHub - chochain/eforth: eForth in C/C++ - multi-platform (Linux,MacOS,Windows,ESP32,WASM)                                            
...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fchochain%2Feforth)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E&source=header-repo&source_repo=chochain%2Feforth)
...
[chochain](/chochain) / **[eforth](/chochain/eforth)** Public

*   [Notifications](/login?return_to=%2Fchochain%2Feforth) You must be signed in to change notification settings
*   [Fork 8](/login?return_to=%2Fchochain%2Feforth)
*   [Star 37](/login?return_to=%2Fchochain%2Feforth)
    

eForth in C/C++ - multi-platform (Linux,MacOS,Windows,ESP32,WASM)

[37 stars](/chochain/eforth/stargazers) [8 forks](/chochain/eforth/forks) [Branches](/chochain/eforth/branches) [Tags](/chochain/eforth/tags) [Activity](/chochain/eforth/activity)

[Star](/login?return_to=%2Fchochain%2Feforth)

[Notifications](/login?return_to=%2Fchochain%2Feforth) You must be signed in to change notification settings

*   [Code](/chochain/eforth)
*   [Issues 1](/chochain/eforth/issues)
*   [Pull requests 0](/chochain/eforth/pulls)
*   [Actions](/chochain/eforth/actions)
*   [Projects 0](/chochain/eforth/projects)
*   [Security](/chochain/eforth/security)
    
    [](/chochain/eforth/security)
    ...
    [There was an error while loading.](/chochain/eforth/security) Please reload this page.
    
*   [Insights](/chochain/eforth/pulse)

Additional navigation options

*   [Code](/chochain/eforth)
*   [Issues](/chochain/eforth/issues)
*   [Pull requests](/chochain/eforth/pulls)
*   [Actions](/chochain/eforth/actions)
*   [Projects](/chochain/eforth/projects)
*   [Security](/chochain/eforth/security)
*   [Insights](/chochain/eforth/pulse)
...
[Branches](/chochain/eforth/branches)[Tags](/chochain/eforth/tags)

[](/chochain/eforth/branches)[](/chochain/eforth/tags)
...
[1,192 Commits](/chochain/eforth/commits/master/)

[](/chochain/eforth/commits/master/)

[docs](/chochain/eforth/tree/master/docs "docs")

[docs](/chochain/eforth/tree/master/docs "docs")

[orig](/chochain/eforth/tree/master/orig "orig")

[orig](/chochain/eforth/tree/master/orig "orig")

[platform](/chochain/eforth/tree/master/platform "platform")

[platform](/chochain/eforth/tree/master/platform "platform")

[src](/chochain/eforth/tree/master/src "src")

[src](/chochain/eforth/tree/master/src "src")

[tests](/chochain/eforth/tree/master/tests "tests")

[tests](/chochain/eforth/tree/master/tests "tests")

[.gitignore](/chochain/eforth/blob/master/.gitignore ".gitignore")

[.gitignore](/chochain/eforth/blob/master/.gitignore ".gitignore")

[Makefile](/chochain/eforth/blob/master/Makefile "Makefile")

[Makefile](/chochain/eforth/blob/master/Makefile "Makefile")

[README.md](/chochain/eforth/blob/master/README.md "README.md")

[README.md](/chochain/eforth/blob/master/README.md "README.md")

[eforth.ino](/chochain/eforth/blob/master/eforth.ino "eforth.ino")

[eforth.ino](/chochain/eforth/blob/master/eforth.ino "eforth.ino")

[eforth.sln](/chochain/eforth/blob/master/eforth.sln "eforth.sln")
...
With all the advantages, it is unfortunate that Forth lost out to C language over the years and have been reduced to a niche. Per ChatGPT: _due to C's broader appeal, standardization, and support ecosystem likely contributed to its greater adoption and use in mainstream computing_.
...
1.  **100% C/C++ with multi-platform support**. Though classic implementation of primitives in assembly language and scripted high-level words gave the power to Forth, it also became the hurtle for newbies. Because they have to learn the assembly and Forth syntax before peeking into the internal beauty of Forth.
    
2.  **Dictionary is just an array**. It's remodeled from linear memory linked-list to an array (or a vector in C++'s term) of words.
...
3.  **Data and Return Stacks are also arrays**. With push, pop and \[\] methods to clarify intentions.
    
4.  **Parameter fields are all arrays**. Why not!
    
5.  **No vocabulary, or meta-compilation**. Except CREATE..DOES>, and POSTPONE, these black-belt skills of Forth greatness are dropped to keep the focus on core concepts.
    
6.  **Multi-threading and message passing are available** From v5.0 and on, multi-core platform can utilize Forth VMs running in parallel. see the multi-threading section below for details
...
In the release, a heavily commented _ceforth.cpp_, the companion _ceforth.h_, and a _config.h_. Altogether, about 800 lines. Check them out!
...
2.  **Lit, Var, Str, Bran, Tmp** - the polymorphic classes extended from the base class Code which serve the functionalities of primitive words of classic Forth.
...
3.  **Dictionary** - an array of _Code_ objects
    
     + build-it words - constructed by initializer\_list at start up, before main is called, degenerated lambdas become function pointers stored in Code.xt
...
4.  **Inner Interpreter** - _Code.exec()_ is self-explanatory
...
5.  **Outer Interpreter** - _forth\_core()_ is self-explanatory
    ...
    DU n \= parse\_number(idiom);           // word not found, try as a number
...
While traditional Forths uses `narr 2 cells +` to get the memory address of `narr[2]`, eforth `narr` returns its index (or defining order) in the dictionary. So, `narr 2 cells +` will actually get you the index of the second word defined after `narr`. You'll be storing the value into that word's empty qf field. To access the n_th_ element of `narr`, use `th` instead
...
To enable multi-threading, of v5, update the followings in ~/src/config.h
    ...
    #define DO\_MULTITASK   1
    #define E4\_VM\_POOL\_SZ  8
...
Note: For multi-threading to work, browser needs to receive Cross-Origin policies [here for detail](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cross-Origin-Opener-Policy) in the response header. A Python script _~/tests/cors.py_ is provided to solve the issue. The same needed to be provided if you use other web server.
...
[](#multi-threading---for-release-v50-and-after)
...
### Built-in words (available only when DO\_MULTITASK is enabled)
...
create a task (tid is index to thread pool entry)  
a free VM from pool is chosen for the task
...
start a task  
The VM is added to event\_queue and kick started when picked up by event\_loop
...
wait until the given task is completed
...
send n elements on current stack to designated task's stack (use stack as message queue)
...
forced fetch stack elements from a completed task
...
fetch microsecond since Epoch, useful for timing

#### Example1 - parallel jobs (~/tests/demo/mtask.fs)

[](#example1---parallel-jobs-testsdemomtaskfs)
...
[](#example2---producer-consumer-testsdemompifs)
...
#### Example3 - fetch result(s) from completed task (~/tests/demo/mpi\_pull.fs)

[](#example3---fetch-results-from-completed-task-testsdemompi_pullfs)
...
[](#desktop-pc---10k10k-cycles-on-32ghz-amd)
...
[](#v5x-srcceforth-multi-threading-capable-dynamic-vector-object-threading)
...
[](#experimental-orig50xceforth-multi-threading-capable-linear-memory-32-bit-iu)
...
[](#esp32---1k1k-cycles-on-240mhz-nodemcu)
...
Though syntax clarity is pretty much the same, lambda being function pointers takes an extra jump and the cost of stack-frame setup/teardown. It takes more space and about 15% slower in tight loops. However, with the advance of compilers,
...
### 50x, 50x32 - back to classic linear-memory model
...
It works better with WASM's memory model. It is used as the foundation for [weForth](https://github.com/chochain/weForth). So far, it is stable but tweaked from time to time and
...
Hinted by Sean Pringle's [Rethinking Forth](https://github.com/seanpringle/reforth) and Travis Bemann's wornderful [zeptoforth](https://github.com/tabemann/zeptoforth). Nested module (or sub-words), simplified control structures are attemped. Now, moved to eForthX
...
*   Dr. Ting's work on eForth between 1995~2011 [eForth references](http://forth.org/library/eforth_SOC) and their [Source Code Repo](http://forth.org/library/eforth_SOC/eforth_SOC_source)
    
*   CC 20210314: Initial
    
    *   Started with ~orig/33b code-base, refactor with enum and VA\_ARGS macros targeting 100% C/C++.
...
*   CC 20221118: Refactor
    ...
    *   WASM function pointer is U32 (index). Token-indirect worked but the two indirect look-up is even slower. Since WASM uses 64K linear memory block, 16-bit pointer offset is a better option. However, the xt "function pointer" in code space is simply an index to the shared \_indirect\_function\_table. Since LSB is used, so we are forced to use MSB to differentiate primitive word from colon word. This left us 15-bit, i.e. 32K, parameter offset available.
*   CC 20231011: Review
    
    *   Since the original intention of having a pre-compiled ROM dictionary still end up in C++ static initialization run before main(), moved dictionary compilation into dict\_compile as function calls gives a little more debugging control and opportunity for fine tuning.
    *   LAMBDA\_OK option was originally intended for full VM implementation but 2x slower. Dropped to reduce source clutter.
*   CC 20240308: Refactor for multi-platform, accept dynamic vectors
    ...
    *   To support cross-platform, i.g. Linux/Cygwin, Arduino/ESP32, Win32, and WASM, there were many conditional compilation branches which make the code really messy. The following were done
        ...
        *   rename 'next\_idiom' to 'word', per Forth standard
*   CC 20241001: Add multi-threading support
    
    *   Shared dictionary and code space amount threads.
    *   Refactor source into ceforth, ceforth\_sys, and ceforth\_task for their specific functions.
    *   Introduce VM, states
        *   local ss, rs, tos, and user area
        *   align to cache-line width
        *   pass VM& to all lambda and static functions
    *   Add thread pool and event\_loop with affinity to physical cores.
...
*   CC: 20250610: maintenance and memory leak check
    ...
    *   Refactor
        ...
        *   Group IO functions to forth\_sys module
        *   Macros to clarify intention, i.e. NEST, BASE, ADD\_W
    ...
    *   Enhance multi-threading
        ...
        *   Refined forth\_vm state machine transition (QUERY, HOLD, NEST, STOP)
    *   Enhance debugging
...
[esp32](/topics/esp32 "Topic: esp32") [wasm](/topics/wasm "Topic: wasm") [forth](/topics/forth "Topic: forth") [eforth](/topics/eforth "Topic: eforth")
...
[Activity](/chochain/eforth/activity)

### Stars

[**37** stars](/chochain/eforth/stargazers)

### Watchers

[**2** watching](/chochain/eforth/watchers)

### Forks

[**8** forks](/chochain/eforth/forks)

[Report repository](/contact/report-content?content_url=https%3A%2F%2Fgithub.com%2Fchochain%2Feforth&report=chochain+%28user%29)

## [Releases 2](/chochain/eforth/releases)
...
](/chochain/eforth/releases/tag/v4.2.2)

[\+ 1 release](/chochain/eforth/releases)

## [Packages 0](/users/chochain/packages?repo_name=eforth)

No packages published  

## [Contributors 3](/chochain/eforth/graphs/contributors)
...
*   [C 50.6%](/chochain/eforth/search?l=c)
*   [C++ 47.6%](/chochain/eforth/search?l=c%2B%2B)
*   [Forth 1.5%](/chochain/eforth/search?l=forth)
*   [HTML 0.1%](/chochain/eforth/search?l=html)
*   [Makefile 0.1%](/chochain/eforth/search?l=makefile)
*   [JavaScript 0.1%](/chochain/eforth/search?l=javascript)
