...
For instance, consider the nested clinical trial form structure below, which uses `Context` to manage state. The four-level "event → form → field group → field" hierarchy mirrors that of the schema we use to model clinical trial data internally. When a node rerenders in the demo, the component flashes (the color depends on the depth of the node in the tree). You'll notice that marking _any_ node as completed will cause the entire tree to rerender: try clicking "Mark Completed" on a node below.
...
clinical-trial-tree.js

use-flash-on-render.js
...
// Rerenders whenever `completedNodeIds` changes
const [completedNodeIds, setCompletedNodeIds] = useContext(
  CompletedNodeIdsContext,
)
```
...
// Only rerenders when the count of completed nodes has changed
const completedCountAtom = atom((get) => {
  const completedNodeIds = get(completedNodeIdsAtom)
  return completedNodeIds.size
})

// Only rerenders when the completion status of a specific node has changed
...
// The setter is stable, so this never causes a rerender on its own
...
1.  [react-hook-form](https://github.com/react-hook-form/react-hook-form) famously takes the [uncontrolled approach](https://react-hook-form.com/faqs#PerformanceofReactHookForm) and achieves impressive performance. We actually do use react-hook-form in many parts of our application; for instance, on our settings and sign up pages. But for the actual clinical trial data capture UI — which requires precise control over data flow, dependent validation, conditional visibility, etc. — it made more sense for us to handle form state management ourselves. [↩](#user-content-fnref-1)
    
2.  (in contrast to the increased boilerplate and more significant shift in mental models that would've been required if we'd used a more cross-cutting, opinionated library like [Redux](https://github.com/reduxjs/redux)) [↩](#user-content-fnref-2)
    
3.  To be completely fair, it's true that it's possible to achieve similar behavior in vanilla React — by aggressively splitting contexts, memoizing everything, etc. But at that point, why not just use a tool that was built for the job, that enables this behavior with better ergonomics and less code? [↩](#user-content-fnref-3)
...
### Stop fighting your software. Start accelerating your research.

See how Harbor can transform your next clinical trial.

Book a Demo

Study setup free for qualified startups.
...
Clinical trial software built for compliance and designed for humans. Get better data, quicker answers, and faster submissions.

[![Google Cloud Partner](/_next/static/media/google-cloud-partner-logo.15dbfc34.png)](https://cloud.google.com/partners)[![Octane Launchpad](/_next/static/media/octane-logo.f2999f42.png)](https://octaneoc.org/launchpad/)
...
*   [Company](/company)
*   [Blog](/blog)
*   [Contact Us](mailto:hello@runharbor.com)
*   [Privacy Policy](/privacy)

[](https://x.com/runharbor)[](http://linkedin.com/company/runharbor)[](http://github.com/runharbor)

Made in San Diego, California  
© 2025 Harbor Labs LLC

Get started with Harbor today
