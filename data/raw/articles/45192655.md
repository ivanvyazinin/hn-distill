...
![Cookie says: 'Oh my gosh, Josh :)! I just had the weirdest dream, like, everything we do is a game! Arfer!'](/assets/images/ac/Modern%20AI%20in%2024%20Year%20Old%20Game%20Screenshot%20(3).png)
...
graph TD A\[Python Script\] -- "Writes LLM response" --> B{Memory Mailbox @ 0x81298360} B -- "Game reads new dialogue" --> C\[Animal Crossing on Dolphin Emulator\] C -- "Writes current speaker & context" --> B
...
graph LR subgraph Option A: BBA Network Shim AC\[Animal Crossing\] --> Hooks\[Net Shim Hooks\] Hooks --> BBA\[BBA Driver\] BBA --> LAN\[(LAN)\] LAN --> Host\[Host Bridge Server\] end subgraph Option B: RAM Mailbox AC2\[Animal Crossing\] --> Mailbox\[RAM Mailbox\] Mailbox --> Py\[Python Watcher\] Py --> LLM\[LLM\] end
...
graph LR subgraph Game World Dolphin(Dolphin Emulator) end subgraph Python Bridge Watcher(watch\_dialogue.py) Encoder(Encoder/Decoder) end subgraph AI Core Writer(Writer LLM) Director(Director LLM) end subgraph External Data Wiki(Fan Wiki) News(RSS Feeds) end Dolphin <--> |IPC via RAM| Watcher Watcher --> |Context| Writer Wiki --> |Character Sheets| Writer News --> |Current Events| Writer Writer --> |Raw Dialogue| Director Director --> |Decorated Dialogue| Encoder Encoder --> |Encoded Bytes| Watcher
...
![Mitzi mentions European leaders meeting Trump and Zelenskyy](/assets/images/ac/Modern%20AI%20in%2024%20Year%20Old%20Game%20Screenshot%20(2).png)
...
![Cookie says Tom Nook is taking all the bells](/assets/images/ac/Modern%20AI%20in%2024%20Year%20Old%20Game%20Screenshot%20(1).png)
...
![Cookie says a woman was killed in a robbery in a blue city](/assets/images/ac/Modern%20AI%20in%2024%20Year%20Old%20Game%20Screenshot.png)
