...
*   [![](/static/logo-lockup.svg "Ink & Switch Logo")](/ "Visit Ink & Switch Home Page")
*   [Alex Good](https://patternist.xyz/)
*   [John Mumm](https://blog.jtfmumm.com/)
...
The entries start from the beginning, but you can jump to the most recent post: [05 · Syncing Keyhive](/keyhive/notebook/05/)

## [00 · Keyhive Background](/keyhive/notebook/00/)
...
![GitHub Repo Membership Page](/keyhive/notebook/static/00/github-rbac.png)
...
![Ranges of authorization over time](/keyhive/notebook/static/00/history.png)
...
![Comparing how authorization is layered in centralized and local-first applications](/keyhive/notebook/static/01/stack-changes.png)
...
![Dependency Stack](/keyhive/notebook/static/01/block-diagram.png)
...
### [Convergent Capabilities](#convergent-capabilities)

![](/keyhive/notebook/static/01/granovetter.png)
...
![](/keyhive/notebook/static/01/doc-group.png)
...
### [Transitive Groups](#transitive-groups)

![](/keyhive/notebook/static/01/device-management.png)
...
![](/keyhive/notebook/static/01/effects.png)

#### Access Effects

An example of delegation across the Keyhive access effect types

## [Trust Minimized Sync Servers](#trust-minimized-sync-servers)
...
## [02 · Group Key Agreement with BeeKEM](/keyhive/notebook/02/)
...
## [Continuous Group Key Agreement](#continuous-group-key-agreement)
...
![Forward secrecy and post-compromise security](/keyhive/notebook/static/03/forward_secrecy_and_post-compromise_security.png)
...
For these reasons, we’ve proposed our own alternative[3](#fn3) for Keyhive that we call “BeeKEM”. It is closely modelled on TreeKEM with insights from Causal TreeKEM. It requires no central server and only a causal order of operations. It provides logarithmic performance (with linear worst case). And like the other TreeKEM variants, it provides forward secrecy and post-compromise secrecy[4](#fn4). Furthermore, it relies exclusively on standard crypto, such as [Diffie Hellman key exchange](https://en.wikipedia.org/wiki/Diffie%E2%80%93Hellman_key_exchange) and [BLAKE3](https://github.com/BLAKE3-team/BLAKE3-specs/blob/master/blake3.pdf) hashing.

## [BeeKEM](#beekem)

In this section, we’ll see how BeeKEM works in more detail.

In BeeKEM (as in TreeKEM), the current group secret is stored encrypted at the root node of a binary tree. We’ll call this the “root secret”. The root secret is used for encrypting and decrypting document chunks shared with our group over the network[5](#fn5).

![Basic BeeKEM tree](/keyhive/notebook/static/03/basic_beekem_tree.png)
...
![BeeKEM inner nodes](/keyhive/notebook/static/03/beekem_inner_nodes.png)

Each node in a binary tree has a single sibling node, as illustrated in the following diagram:

![Node sibling](/keyhive/notebook/static/03/siblings.png)
...
![Diffie Hellman basics](/keyhive/notebook/static/03/diffie_hellman_basics.png)
...
![BeeKEM Diffie Hellman example](/keyhive/notebook/static/03/beekem_diffie_hellman_example.png)
...
![BeeKEM path](/keyhive/notebook/static/03/beekem_path.png)
...
![Blank nodes](/keyhive/notebook/static/03/blank_nodes.png)
...
![Remove member](/keyhive/notebook/static/03/remove_member.png)
...
![Add member](/keyhive/notebook/static/03/add_member.png)
...
![Resolving add conflicts](/keyhive/notebook/static/03/merging_concurrent_adds.png)

### [Handling Blank Nodes on Update and Decryption](#handling-blank-nodes-on-update-and-decryption)
...
![Node resolution example](/keyhive/notebook/static/03/node_resolution_examples.png)
...
![Blank sibling](/keyhive/notebook/static/03/blank_sibling.png)
...
![Skipping blanks](/keyhive/notebook/static/03/skipping_blanks.png)

### [Handling Concurrent Updates with “Conflict Keys”](#handling-concurrent-updates-with-conflict-keys)
...
![Merging conflict keys](/keyhive/notebook/static/03/merging_concurrent_updates.png)
...
1.  TreeKEM was inspired by earlier work on [Asynchronous Ratcheting Trees (ART)](https://eprint.iacr.org/2017/666.pdf). [↩︎](#fnref1)
...
Fun fact: previous internal names for this project included “authnz” and “Auth-o-merge”.

At the time that we decided on “Beehive”, the team was aware of namespace conflicts in the academic distributed systems literature[1](#fn1). Over time it’s become clear that we also have this problem with packages in more than one language ecosystem. Since we don’t want to tie the project to [Automerge](https://automerge.org/) exclusively, prefixing the core project with `automerge-*` was not appropriate.

We are retaining our apian naming for other parts of the project. [BeeKEM](/keyhive/notebook/#beekem) maintains it’s pun on TreeKEM, and Beelay is the Keyhive-enabled relay.

Beehive is dead. Long live Keyhive!

* * *

1.  Some examples include [Beehive: Exploiting Power Law Query Distributions for O(1) Lookup Performance in Peer to Peer Overlays](https://www.cs.cornell.edu/people/egs/615/beehive.pdf), [Beehive: Erasure Codes for Fixing Multiple Failures in Distributed Storage Systems](https://iqua.ece.toronto.edu/papers/junli-tpds17.pdf), and [Beehive: Simple Distributed Programming in Software-Defined Networks](https://courses.cs.duke.edu/spring17/compsci590.7/Papers/Beehive16.pdf). [↩︎](#fnref1)
    

## [04 · Opening the Pre-Alpha](/keyhive/notebook/04/)
...
## [05 · Syncing Keyhive](/keyhive/notebook/05/)
...
“Authentication” here is used in the technical sense, and refers to the task of ensuring that the remote peer controls some public key. It _does not_ refer to the much more complicated task of binding a physical person or organisation to a Keyhive principal. There are many ways of achieving such a binding with different tradeoffs, but we will not be discussing them here.

### [What are we authenticating?](#what-are-we-authenticating)
...
![A diagram of a group representing a person, with three nodes representing devices called 'phon', 'laptop', and 'table' all of which have arrows pointing to the person node](/keyhive/notebook/static/05/person-group.png)
...
![Another diagram, this time with a node labelled 'Ink & Switch' which represents an organisation with the original 'person' diagram being a member of that group](/keyhive/notebook/static/05/person-doc-group.png)
...
### [Person in the middle attacks](#person-in-the-middle-attacks)
...
![A diagram with three nodes connnected in a line, the right most is labelled 'browser', the middle is labelled 'sync.automerge.org (attacker)' and the rightmost is labelled 'sync.automerge.org'](/keyhive/notebook/static/05/pitm.png)
...
In practice we actually use the hash of the URL as this makes the message size a fixed length.
...
## [Syncing the membership graph](#syncing-the-membership-graph)
...
![set reconciliation](/keyhive/notebook/static/05/set-reconciliation-1.png)
...
![an image of a set of boxes labeled thing1, thing2, and thing3, with each box pointing to another box labelled hash(thing1), hash(thing2), hash(thing3). A larger box surrounds the entire set of boxese pointing to a sequence of boxes labelled symbol1, symbol2, symbol3](/keyhive/notebook/static/05/riblt-things-to-hashes-to-symbols.png)
...
![an image of three boxes labeled symbol1, symbol2, symbol3 with an arrow - labelled decode - pointing to two bases labeled things we have which they dont and things they have which we dont containg a box labelled hash(thing1) and hash(thing2) respectively](/keyhive/notebook/static/05/riblt-symbols-decoded.png)
...
![begin sync](/keyhive/notebook/static/05/beginsync-1.png)

## [Document Collection Sync](#document-collection-sync)
...
Keep up-to-date with the lab's latest findings, appearances, and happenings by subscribing to our newsletter. For a sneak peek, [browse the archive](/newsletter).
