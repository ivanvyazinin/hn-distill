...
August 6, 2025 / [#WSL](/news/tag/wsl/)

# How to Free Up and Automatically Manage Disk Space for WSL on Windows 10/11

![brooklyn](https://cdn.hashnode.com/res/hashnode/image/upload/v1741178393492/823d8c4d-c927-4c49-a348-7148430f6e7f.jpeg?w=500&h=500&fit=crop&crop=entropy&auto=compress,format&format=webp)

[brooklyn](/news/author/brkln/)

  ![How to Free Up and Automatically Manage Disk Space for WSL on Windows 10/11](https://cdn.hashnode.com/res/hashnode/image/upload/v1754523230294/70893973-fddf-42a9-b41a-2a8f94a47e22.png)

Windows Subsystem for Linux ([WSL](https://learn.microsoft.com/en-us/windows/wsl/install)) lets you run a Linux environment directly on Windows. This is particularly useful for web development where you can develop and test applications in a Linux environment without leaving Windows. You can even run [freeCodeCamp locally](https://contribute.freecodecamp.org/how-to-setup-wsl/) with it!
...
*   [Part 1: How to Manually Compact Your Virtual Hard Disk](#heading-part-1-how-to-manually-compact-your-virtual-hard-disk)
    
    *   [Prerequisites](#heading-prerequisites)
        
    *   [Step 1: Verify your WSL version and status](#heading-step-1-verify-your-wsl-version-and-status)
        
    *   [Step 2: List all installed distributions verbosely](#heading-step-2-list-all-installed-distributions-verbosely)
        
    *   [Step 3: Locate your linux Virtual Hard Drive (VHDX) path](#heading-step-3-locate-your-linux-virtual-hard-drive-vhdx-path)
        
    *   [Step 4: Shut down all WSL instances](#heading-step-4-shut-down-all-wsl-instances)
        
    *   [Step 5: Compact the Linux virtual hard drive using DiskPart](#heading-step-5-compact-the-linux-virtual-hard-drive-using-diskpart)
        
    *   [Step 6: Restart WSL and verify](#heading-step-6-restart-wsl-and-verify)
        
*   [Part 2: How to Make Your Life Easier with Automation](#heading-part-2-how-to-make-your-life-easier-with-automation)
    
    *   [Prerequisites](#heading-prerequisites-1)
        
    *   [Step 1: Find out installed WSL2 distributions](#heading-step-1-find-out-installed-wsl2-distributions)
        
    *   [Step 2: Select a distro to compact](#heading-step-2-select-a-distro-to-compact)
        
    *   [Step 3: Locate the ext4.vhdx File](#heading-step-3-locate-the-ext4vhdx-file)
        
    *   [Step 4: The confirmation prompt](#heading-step-4-the-confirmation-prompt)
        
    *   [Step 5: Shut Down WSL and compact](#heading-step-5-shut-down-wsl-and-compact)
        
    *   [Step 6: Run a DiskPart script](#heading-step-6-run-a-diskpart-script)
...
Look for your distro name (for example, “_Ubuntu_”) and note its WSL version. If it shows “Version 2”, you can proceed with compaction.

### **Step 3: Locate your linux Virtual Hard Drive (VHDX) path**

Each WSL distro’s files live in a [VHDX file](https://en.wikipedia.org/wiki/VHD_\\(file_format\\)) on your Windows drive. To find the path for any Linux distribution, use this PowerShell snippet:
...
1.  Launch DiskPart in the same elevated (_admin_) shell:
...
2.  In the DiskPart prompt, select the VHDX file you found earlier. Replace the path as displayed below with your actual path (the line you copied before):
...
3.  Attach the virtual drive in read-only mode:
...
To test the script, download it from [GitHub](https://github.com/brooks-code/WSL-VHDX-Compact/blob/main/wsl_compactor.ps1). Open an elevated PowerShell or Command Prompt and navigate to the script’s folder. With just the command below, you will be able to run it and free up some disk space:
...
In the first step, we collected the information we need about the available distributions and where they are stored on the Windows system. By choosing an entry (either manually or automatically), we can find the correct file. Sometimes, the ext4 file is located between the base path and a _LocalState_ folder. This script manages both situations. It builds the usual locations where the file can be found.
...
2.  Compacting can take a while, especially if you’ve never de-cluttered your virtual drive before. It’s wise to show a warning before proceeding:
...
Okay, that’s it for the scripting! If you collected all the snippets so far, just save them with a `.ps1` file extension, or download the full example from this [GitHub repository](https://github.com/brooks-code/WSL-VHDX-Compact).
...
You can find a list of my current projects on [GitHub](https://github.com/brooks-code?tab=repositories).
...
url = "github.com/brooks-code" if deep\_learning\_engineering: np.where( data\_scientist == True, "LLM alchemist by day, RL innovator by night")
