Paying attention to feature distribution alignment | Alex Shtoff                     
...
[⟪ Off with the polynomial's tail!](/2025/04/17/Polynomial-Pruning.html)

* * *

#### This post is part of the series "_Polynomial features in machine learning"_, and is not self-contained.

#### Series posts:

#### 

1.  [“Are polynomial features the root of all evil?"](/2024/01/21/Bernstein.html)
2.  [“Keeping the polynomial monster under control"](/2024/01/25/Bernstein-Basis.html)
3.  [“SkLearning with Bernstein Polynomials"](/2024/02/11/Bernstein-Sklearn.html)
4.  [SkLearning with Bernstein Polynomials - continued](/2024/05/13/Bernstein-Pairwise-Sklearn.html)
5.  [A Bernstein SkLearn model calibrator](/2024/05/19/BernsteinCalibration.html)
6.  [Regularization properties of polynomial bases](/2024/06/03/PolynomialBasesRegProps.html)
7.  [Let the polynomial monster free](/2025/03/27/Free-Poly.html)
8.  [Off with the polynomial's tail!](/2025/04/17/Polynomial-Pruning.html)
9.  Paying attention to feature distribution alignment _(this post)_
...
So that we all are on the same page, let’s recall why orthogonal bases produce uncorrelated features. Recall, the two polynomials \\(P\_i\\) and \\(P\_j\\) defined on \\(\[-1, 1\]\\) are orthogonal if

\\\[\\langle P\_i, P\_j \\rangle = \\int\_{-1}^1 P\_i(x) P\_j(x) dx = 0,\\\]

just like two orthogonal vectors - their inner product is zero. But here the inner product is an integral rather than a sum. But an integral is also an expectation, and empirical averages approximate expectations. So if our data points \\(x\_1, \\dots, x\_n\\) are approximately uniform in \\(\[-1, 1\]\\), then

\\\[0 = \\int\_{-1}^1 P\_i(x) P\_j(x) dx \\sim \\frac{2}{n} \\sum\_{k=1}^n P\_i(x\_k) P\_j(x\_k).\\\]
...
\\\[\\langle \\mathbf{x}, \\mathbf{y} \\rangle\_{\\mathbf{w}} = \\sum\_{i=1}^n x\_i y\_i w\_i.\\\]

The contribution of every two components at index \\(i\\) is weighted by the weight \\(w\_i\\).
...
\\\[\\langle f, g\\rangle\_w = \\int\_{D} f(x)g(x)w(x)dx\\\]
...
Now let’s see what does it mean in terms of informativeness. Suppose without loss of generality that \\(w(x)\\) is normalized such that \\(\\int\_{D} w(x) = 1\\). If it’s not, we can always divide it by its integral. Now, it can be thought of as PDF of some probability distribution over \\(x\\), and therefore the inner product is just an expectation. Therefore if our data points \\(x\_1, \\dots, x\_n\\) come from that distribution, then

\\\[\\langle f, g \\rangle\_w = \\mathbb{E}\_x \\left\[ f(x) g(x) \\right\] \\sim \\frac{1}{n} \\sum\_{i=1}^n f(x\_i) g(x\_i).\\\]
...
So ideally we would like to devise orthogonal bases w.r.t the PDF of our data distribution. The differential equations community has been designing orthogonal bases w.r.t various weights for a long time, and have come up with plenty of methods and an enormous body of literature. But there are two extremely simple tricks we can adopt from them for ML, which I’d like to discuss in this pose. Both are also discussed in the context of approximation theory and differential equations in the recent survey paper by Shen and Wang[2](#fn:1). One of them appears extremely intuitive, easy to implement, and is indeed useful in practice. The other one requires some careful math before coding, harder to apply in practice, and I’d like to present it and discuss when I believe it may be useful to.
...
\\\[0 = \\int\_{-1}^1 P\_i(y) P\_j(y) dy = \\int\_{D} P\_i(\\phi(x)) P\_j(\\phi(x)) \\phi'(x) dx = \\langle Q\_i, Q\_j \\rangle\_{\\phi'}\\\]

The conclusion is simple - mapping with \\(\\phi\\) results in orthogonal functions weighted by \\(\\phi'\\). In particular, if \\(\\phi\\) is a CDF of some distribution, then using the basis \\(Q\_0, Q\_1, ...\\) will result in uncorrelated features!
...
![orthogonality_norm_std](https://alexshtf.github.io/assets/orthogonality_norm_std.png)
...
![orthogonality_norm_std](https://alexshtf.github.io/assets/orthogonality_norm_nonstd.png)
...
![orthogonality_norm_std](https://alexshtf.github.io/assets/orthogonality_norm_gamma.png)
...
![orthogonality_test_pipeline](https://alexshtf.github.io/assets/orthogonality_test_pipeline.png)
...
\\\[\\underbrace{\\frac{P\_0(T(x))}{1+x}}\_{Q\_0(x)}, \\underbrace{\\frac{P\_1(T(x))}{1+x}}\_{Q\_1(x)}, ...\\\]

According to what we saw now, \\(Q\_0, Q\_1, ...\\) is orthogonal w.r.t the weight function \\(w(x) = T'(x) (1 + x)^2\\).

So now, we want to design the mapping \\(T\\) such that the weight aligns with the data distribution, to get informative features from our basis. This means we want the weight to be proportional to the PDF: \\(T'(x) (1 + x^2) = a U'(x), \\qquad a > 0.\\)
...
1.  Muthukumar, V., Vodrahalli, K., Subramanian, V. and Sahai, A., 2019. Harmless interpolation of noisy data in regression. _arXiv preprint arXiv:1903.09139_. [↩](#fnref:2)
    
2.  Shen, J. and Wang, L.L., 2009. Some recent advances on spectral methods for unbounded domains. _Communications in computational physics_, _5_(2-4), pp.195-241. [↩](#fnref:1)
...
[](/2025/08/19/Orthogonality.html)

*   Alex Shtoff
*   [alex.shtf@gmail.com](mailto:alex.shtf@gmail.com)

Blog on optimization, machine learning, and software development.

*   [](https://github.com/alexshtf "GitHub")
*   [](https://x.com/AlexShtf "X")
*   [](https://www.linkedin.com/in/alexshtf "LinkedIn")
*   [](https://stackoverflow.com/users/532057/alex-shtoff "Stack Overflow")
*   [](https://scholar.google.com/citations?user=XcpQMwwAAAAJ "Google Scholar")
*   [](https://alexshtf.github.io/feed.xml "Subscribe to syndication feed")