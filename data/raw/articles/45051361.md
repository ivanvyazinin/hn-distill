...
There are a few possible types that come up often, hiding as booleans. Let's take a look at each of these, as well as the case where using a boolean _does_ make sense. This isn't exhaustive—[\[1\]](#fn-em-dash)there are surely other types that can make sense, too.
...
But when _should_ we use a boolean? I've mainly run into one case where it makes sense: when you're (temporarily) storing the result of a conditional expression for evaluation. This is in some ways an optimization, either for the computer (reuse a variable[\[2\]](#fn-optimization)) or for the programmer (make it more comprehensible by giving a name to a big conditional) by storing an intermediate value.
...
1.  I know that using an em-dash is treated as a sign of using LLMs. LLMs are _never_ used for my writing. I just really like em-dashes and have a dedicated key for them on one of my keyboard layers. [↩](#fr-em-dash-1)
    
2.  This one is _probably_ best left to the compiler. [↩](#fr-optimization-1)
