...
For Odin since it has no concept of a closure, being that it is a language with manual-memory-management[1](#fn:1), it would not be possible to add the same approach that Go does. However there is a way of achieving this in a similar way which requires no closures, and would produce very good code due to its imperative nature.
