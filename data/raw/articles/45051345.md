...
[RSS](/rss.xml) [Github](https://github.com/ValeLang/Vale) [r/Vale](https://reddit.com/r/Vale) [Twitter](https://twitter.com/verdagon) [Discord](https://discord.gg/SNB8yGH)

# Group Borrowing: Zero-Cost Memory Safety with Fewer Restrictions

Aug 28, 2025  —  Evan Ovadia, with help from Nick Smith
...
A few of us have been approaching the problem by starting with reference counting or garbage collection (or generational references!) and allowing functions to "borrow" those objects with much less--or zero--overhead. [0](#note0)

In my biased [1](#note1) opinion, these approaches have some strong benefits. But they're not a panacea, and the world needs more approaches here.

And luckily, my good friend [Nick Smith](https://github.com/nmsmith) (from the [Mojo](https://www.modular.com/mojo) community [2](#note2)) has been exploring exactly that for the past few years.

I think he's found a way to add mutable aliasing directly into a borrow checker _without_ building on a foundation of reference counting, garbage collection, or generational references. In other words, an approach for zero-overhead mutable aliasing, which is a _big deal_. [3](#note3)
...
*   [Foundation: Builds on C++'s "Single Ownership"](#foundation-builds-on-cs-single-ownership)
*   [Recap of Rust Borrowing](#recap-of-rust-borrowing)

*   [Context and Comments on Borrow Checking Limitations](#context-and-comments-on-borrow-checking-limitations)
*   [An example](#an-example)

*   [Nick's Borrowing System](#nicks-borrowing-system)

*   [Basic Mutable Aliasing](#basic-mutable-aliasing)
*   [When to Invalidate References to Contents](#when-to-invalidate-references-to-contents)

*   [A More Complex Example](#a-more-complex-example)

*   [Child groups](#child-groups)

*   [print(hp\_ref)](#printhpref)
*   [print(ring\_ref)](#printringref)
*   [print(len(rings\_list\_ref))](#printlenringslistref)
*   [print(s.durability)](#printsdurability)
*   [Child Groups, Summarized](#child-groups-summarized)

*   [Where do groups come from?](#where-do-groups-come-from)
*   [Isolation](#isolation)
*   [Functions' Group Annotations](#functions-group-annotations)

*   [A More Complex Example](#a-more-complex-example)
*   [Paths](#paths)
*   [Syntax](#syntax)

*   [The approach, summarized](#the-approach-summarized)
*   [Does the approach really not have unique references?](#does-the-approach-really-not-have-unique-references)

*   [Comparison to Borrow Checking](#comparison-to-borrow-checking)
...
My long-time readers will recognize my cognitive dissonance here because I think such a thing is [a myth](https://verdagon.dev/blog/myth-zero-overhead-memory-safety). Nick's approach makes me question that, though. At the very least, we're _much closer_ to achieving the myth, if he hasn't solved it completely already.
...
*   Each Spaceship owns its Engine (via the unique\_ptr).
...
**TL;DR:** Rust's borrow checker has the "aliasing xor mutable" rule which makes it conservative. This means it rejects a lot of valid programs and useful patterns [4](#note4) and it causes accidental complexity for some use cases. [5](#note5)
...
Alas, .split\_at\_mut isn't always great in practice (reasons vary) [6](#note6) and besides, we sometimes _want_ to have two &mut referring to the same object.
...
This is using the slotmap crate (similar to generational\_arena), though you often see this pattern with HashMap instead (or one could also use raw indices into a Vec, though that risks use-after-release problems).
...
...because we're mutably borrowing entities twice: once in a's get\_mut call, and once in d's get\_mut call, and their usages overlap.
...
*   I'm taking some terminology liberties: the proposal calls them "regions", but here I'm describing them as "groups", mainly because I know that "regions" tends to get misinterpreted as "arenas". These are not arenas. [7](#note7)
...
But... we humans can easily conclude this is safe. After the evaluation of list\_ref\_a.push(5), my\_list is still there, and it's still in a valid state. So there is no risk of memory errors when evaluating the second call to push.

In any language, when we hand a function a reference to an object, that function can't destroy the object, [8](#note8) nor change its type--it just has a reference. The same is true here.
...
In the below example, the compiler should reject print(element\_ref) because append might have modified the List.
...
In other words, the approach should know that when we hand append a reference to List, it shouldn't invalidate the other reference list\_ref, but it should invalidate any references to its contents (like el\_ref).
...
*   Know that none of these methods delete any Entitys. [9](#note9)
...
All these methods don't delete any Entitys, so this attack function is completely memory safe. In fact, even though the use\_energy and damage methods modify Entitys, every line in attack is still memory-safe. [10](#note10)
...
![](/images/EntityWithHPAndRings3.png)
...
Now let's see what happens when we modify d with a call to damage and then try to use that ring\_ref:
...
The compiler shows an error, because one of the functions (like damage) might have deleted that first ring, so the compiler _should_ invalidate any references to the contents of all Entitys in the group.
...
The previous (invalid) program had a ring\_ref referring to an element in a ring array.

This new (correct) program has an hp\_ref that's pointing to a mere integer instead.
...
But wait, how? Wouldn't that violate our rule? We might have used a reference (damage may have used d) to mutate an object (the Entity that d is pointing to). So why didn't we invalidate all references to the Entity's contents, like that hp\_ref?
...
*   armor: Box\[IArmor\] [11](#note11)
*   hand: Variant\[Shield, Sword\] [12](#note12)

However, these would be in Entity's child groups:

*   The Rings _inside_ that rings list.
...
![](/images/EntityPossibleGroups.png)

Some observations:

*   d, hp\_ref, and rings\_list\_ref all point to the r group (in blue).
*   ring\_ref points to the r.rings.items\[\*\] group (in green). That group represents _all_ the rings, because the compiler doesn't know the index rand() % len(d.rings). This is different than the r.rings\[0\] from before.
*   armor\_ref points to the r.armor\[\] group (in red).
*   s points to the r.hand.Shield group (in yellow). [13](#note13)
...
Let's look at it piece-by-piece.
 ...
 Notes \[–\] Notes \[+\] 11 12 13

11

An owning pointer to heap, unique\_ptr in C++ speak.
...
Now consider ring\_ref which points to an item in d.rings.
...
#### print(len(rings\_list\_ref))

Let's do a harder example. Consider the rings\_list\_ref that points to the whole d.rings list, rather than an individual Ring.
...
That rings\_list\_ref is actually pointing at group r, not a child group, because the rings ArrayList isn't _in_ a collection (it _is_ the collection). It's in group r (not a child group), which wasn't invalidated, so the compiler can let us use our reference after the damage method.
...
That means rings\_list\_ref is still valid, and we can use it in that print call!
...
Local variables! Each local variable has its own group. [14](#note14)
...
What about heap allocations? For example, if we had a var x = Box\[Entity\](10, 10). In this case, the local variable x has a group. The Entity it's pointing to is a _child group._
...
Stated more accurately, d.damage(3) is modifying group r, so it invalidates all references that point into r's child groups (like contents\_ref).
...
And here's something similar to our attack from before, but with a new call to a new power\_up\_ring function:
...
As the comments say, power\_up\_ring is modifying one of a's rings, and it doesn't invalidate our armor\_ref.

To see how that's possible, let's see power\_up\_ring (note I'm taking some liberties with the syntax, a much shorter version is in a section below):
...
With this, the caller (attack) has enough information to know exactly what was modified. [15](#note15)

Specifically, attack knows that Entitys' .rings elements may have changed. Therefore, after the call to power\_up\_ring, attack should invalidate any references pointing into Entitys' .rings elements, but not invalidate anything else. Therefore, it should not invalidate that armor\_ref.

Inside the function, we see a a\_ring.power += entity.energy / 4. Note how it's:
...
Well not _exactly_. Technically, only the .power field is being modified, but power\_up\_ring is saying that _anything_ inside Ring might have changed.
...
This is the key that makes this entire approach work across function calls. Whenever there's a callsite, like attack's call to power\_up\_ring(a, a.rings\[0\]), it can assemble a full picture of whether that call is valid, and how it affects the code around it.

When compiling attack, the compiler thinks this:

*   The caller (attack) calling power\_up\_ring(a, a.rings\[0\]).
...
When I was learning about the approach, I was kind of surprised that it had no unique references. They seemed inevitable. [16](#note16) In his proposal, Nick even mentions this example:
...
*   Then again, Rust kind of has this too, in the form of .split\_at\_mut. It might be easier here?
...
*   Maybe we'll discover some ways we can decompose it into multiple orthogonal mechanisms, like how implementation inheritance (Java's extends) is really just implements+delegation+composition. [18](#note18)
...
*   Maybe someone will find a way to make these groups (mutably) alias each other! [19](#note19)
...
Of course, it pains me to say that "it's simple", because it makes it seem like it was easy to discover. I know from personal experience just how hard it is to come up with something like this... it takes a _lot_ of thinking, trial and error, and bumping into dead ends. [20](#note20)
