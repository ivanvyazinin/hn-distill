...
The system is very simple: A _task_ dispatches onto a given _queue_ from where a _worker_ picks it up to work on. Tasks are subdivided into _steps_, which are executed in sequence by the worker. Tasks can be suspended or fail, and when that happens, they execute again (a _run_). The result of a step is stored in the database (a _checkpoint_). To avoid repeating work, checkpoints are automatically loaded from the state storage in Postgres again.

Additionally, tasks can _sleep_ or _suspend for events_ and wait until they are emitted. Events are cached, which means they are race-free.
...
1.  It’s named Absurd because durable workflows are absurdly simple, but have been overcomplicated in recent years.[↩](#fnref-1)
    

This entry was tagged [ai](/tags/ai/) and [announcements](/tags/announcements/)

[copy as](/2025/11/3/absurd-workflows.md) / [view](/2025/11/3/absurd-workflows.md) markdown
...
More info: [imprint](/about#imprint) & [AI transparency](/ai-transparency). Subscribe [to the atom feed](/feed.atom "Armin Ronacher's
