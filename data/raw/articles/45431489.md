JP's Website · 2025-09-28 · Pre-emptive Multi-Tasking on Arm Cortex-M             

### [JP's Website](https://thejpster.org.uk "Home")  

* * *

Scroll through the blog: ‹ Newer | [List All](..) | [Older ›](/blog/blog-2025-08-23/)
...
*   [Background](https://thejpster.org.uk/blog/blog-2025-09-28/#background)
*   [C-language RTOSes](https://thejpster.org.uk/blog/blog-2025-09-28/#c-language-rtoses)
*   [Rust-language RTOSes](https://thejpster.org.uk/blog/blog-2025-09-28/#rust-language-rtoses)
*   [But what is an RTOS?](https://thejpster.org.uk/blog/blog-2025-09-28/#but-what-is-an-rtos)
*   [Arm Cortex-M](https://thejpster.org.uk/blog/blog-2025-09-28/#arm-cortex-m)
*   [Writing an RTOS in Rust](https://thejpster.org.uk/blog/blog-2025-09-28/#writing-an-rtos-in-rust)
    *   [Representing a Stack](https://thejpster.org.uk/blog/blog-2025-09-28/#representing-a-stack)
    *   [Pushing onto a Stack](https://thejpster.org.uk/blog/blog-2025-09-28/#pushing-onto-a-stack)
    *   [Tasks](https://thejpster.org.uk/blog/blog-2025-09-28/#tasks)
    *   [The Scheduler](https://thejpster.org.uk/blog/blog-2025-09-28/#the-scheduler)
    *   [Picking a new task](https://thejpster.org.uk/blog/blog-2025-09-28/#picking-a-new-task)
    *   [Actually switching tasks](https://thejpster.org.uk/blog/blog-2025-09-28/#actually-switching-tasks)
    *   [Userspace API](https://thejpster.org.uk/blog/blog-2025-09-28/#userspace-api)
*   [An example](https://thejpster.org.uk/blog/blog-2025-09-28/#an-example)
*   [Conclusions](https://thejpster.org.uk/blog/blog-2025-09-28/#conclusions)
...
*   [Eclipse ThreadX](https://threadx.io/) (formerly Microsoft Azure RTOS, which was formerly Express Logic ThreadX)
...
_An aside on naming: If you're more familiar with Windows, macOS or Linux, what I call a Task maps closely to what you might know as a Thread. It should also be noted that what I call a Task isn't what Rust calls a [Task](https://doc.rust-lang.org/stable/std/task/index.html), as those are specifically asynchronous tasks, which is something different. You might say, "But why didn't you just call them Threads?" and the answer is FreeRTOS [also calls them Tasks](https://www.freertos.org/Documentation/02-Kernel/02-Kernel-features/01-Tasks-and-co-routines/00-Tasks-and-co-routines) and because to me, Threads are short-lived things you can spawn and then wait on, whilst Tasks are things that are created at start-up. Anyway, naming things is one of the hardest problems in computer science._
...
However, in a major departure from both Legacy Arm Architectures and the R- and A-Profiles of the current Arm Architectures, the M-Profile specifications state that the hardware will _automatically push those registers to the stack_. This means that you _can_ write Armv7-M (et al) exception handlers in C. Or in Rust. It also gives us a leg-up when we want to write an RTOS, as _part_ of our work is done for us.
...
On Arm, stacks are typically what is known as _Full Descending_. That is, the stack pointer points at the most recent item added to the stack (it points to a 'full' location), and when a new item is pushed onto the stack, the pointer moves downwards. You could also have a _Empty Descending_ stack, or even an _Empty Ascending_ stack, but C and Rust agree to use the Arm Embedded ABI, and that says that stacks are _Full Descending_ (and you can imagine the mess if not all the code in the system agrees on this convention).

So, we need a memory region, and we need to know the address just above the top-most location in that region.

First, please note that examples on this page are Copyright (c) 2025 Ferrous Systems, and licensed under GPL-3.0-or-later. You can find a full working version at [https://github.com/jonathanpallant/pets/tree/v0.1.0](https://github.com/jonathanpallant/pets/tree/v0.1.0), which includes all the lovely comments that I stripped out of this blog page in the name of brevity (and because there's all this explanatory text anyway).
...
There's also a curious issue with `set_pendsv()`. Setting the bit doesn't _immediately_ cause the `PendSV` handler to fire. Because Arm processors are _pipelined_, they are loading the _next_ instruction whilst simultaneously _executing_ the current instruction (and perhaps _retiring_ the previous instruction). So there may be a delay of a clock cycle or two whilst the processor deals any instructions it started but has not finished, before it jumps to the `PendSV` handler. An _Instruction Synchronization Barrier_ is what we want here, to block the CPU until the pipeline is empty, using the `isb()` function from the `cortex-m` crate.
...
Not a very _good_ RTOS of course, and possibly not very real-time. To make it better, we should give each task a Priority, and select the task with the highest priority to run. We also need a mechanism to block a task on something other than the current time - say, to wait for something to arrive in a mailbox. I'll leave these as exercises for the reader for now, along with FPU support (because you'll note we haven't saved any of the 33 FPU registers when switching tasks).
...
[About Me](https://thejpster.org.uk/about) · [Blog](https://thejpster.org.uk/blog) · [RSS](https://thejpster.org.uk/atom.xml) · [My CV](https://thejpster.org.uk/cv)
