Digital echoes: open bus behavior on the compact Macintosh | Thomas                    

[!['s Picture](/assets/images/thomas.png)](/)
...
![By NetworkTopologies.png: Maksim / derivative work: Malyszkz - Own work based on: NetworkTopologies.png, Public Domain, https://commons.wikimedia.org/w/index.php?curid=15006915](/assets/posts/mac-openbus/NetworkTopologies.svg)
...
![MC68000 Read and Write-Cycle Timing Diagram](/assets/posts/mac-openbus/m68000timing.png)
...
![Macintosh Plus address map](/assets/posts/mac-openbus/plusmap.png)
...
The MC68000 has a mechanism for devices to acknowledge bus transfers, using the _/DTACK_ and _/VPA_ pins, as seen in the timing diagram above as well. If a device does not acknowledge the transfer, the CPU can generate an exception which the software can act upon.

However, on the compact Macintosh, the electronics around the CPU contain logic that generates these signals automatically, for _every address_:

![DTACK/VPA note in guide to the Macintosh Family Hardware](/assets/posts/mac-openbus/dtackvpa.png)
...
![MC68000 input DC characteristics](/assets/posts/mac-openbus/inputdc.png)
...
One last characteristic of any piece of wire or trace we need to take into account is _parasitic capacitance_. A copper signal trace on a circuit board generally has some capacitance (it can store an electric charge, like a capacitor, for a short amount of time). This means that when the signal is tri-stated, it may still retain some voltage if it was driven high shortly before!
...
![Macintosh Classic II Programmers key - by ksfan, CC BY 2.5](/assets/posts/mac-openbus/progkey.jpg)
...
![DM FFFFC0 in Microbug on the Macintosh 512K](/assets/posts/mac-openbus/512k.jpg)
...
But why `6116`? You’ll notice that `6116` is actually the encoded instruction on address `4015D0`, which comes _after_ the move instruction we’ve been looking at at `4015CE`. The reason why the next instruction shows up here is because the MC68000 [has a prefetch queue](https://pasti.fxatari.com/68kdocs/68kPrefetch.html). The prefetch queue is two words long and the instructions at `4015CE` and `4015D0` are both one word long, so the `Bsr.B` instruction at `4015D0` was read into the queue just before the `Move` instruction began executing, explaining why we’re seeing `6116` as open bus data.

## How can this be emulated?

First of all, to properly emulate this behavior, an emulator should emulate the MC68000 with bus cycle accuracy - meaning the emulator needs to execute every bus transaction of every instruction in the exact same sequence and timing as a real MC68000. Prefetch also needs to be emulated accurately. Fortunately, Snow already achieves this level of accuracy and its CPU core passes the [test suite based on a MC68000 microcode simulation](https://github.com/SingleStepTests/m68000).
...
![Open bus emulation in Snow](/assets/posts/mac-openbus/snowopenbus.png)

# The effect on software

There are many documented examples of software on various systems that rely on open bus behavior, either intentionally or unintentionally. A known example is [Donkey Kong Country 2](https://jsgroth.dev/blog/posts/dkc2-open-bus/) which broke on the ZSNES emulator due to incorrectly emulated open bus behavior.
...
![The Print Shop 1.0, running in Snow](/assets/posts/mac-openbus/printshop.png)
...
The copy protection on this disk reads a specially crafted track on the floppy which is written with flux transitions at a faster interval. This track looks like this when viewed in [Applesauce](https://applesaucefdc.com/):

![Print Shop 1.0 protection track](/assets/posts/mac-openbus/psprottrack2.png)

For reference, a normal track:

![Print Shop 1.0 normal track](/assets/posts/mac-openbus/psprottrack.png)
...
Eventually, I noticed the following code running _after_ the copy protection check:
...
![Animation Toolkit 1.0, running in Snow](/assets/posts/mac-openbus/atk.png)
...
![Snow showing an address error](/assets/posts/mac-openbus/atk-crash-snow.png)
...
![NewHandle, Inside Macintosh](/assets/posts/mac-openbus/newhandle.png)
...
![F80000 debugger dump](/assets/posts/mac-openbus/f80000.jpg)

Looks like it! Let’s also look at the reference for `HLock`:

![HLock, Inside Macintosh](/assets/posts/mac-openbus/hlock.png)
...
![Macintosh Plus showing an address error](/assets/posts/mac-openbus/atk-crash-hw.jpg)
...
_Thanks to Doctorgianlu, Reza Fouladian, KenDesigns and Andy for providing bug reports, input and testing_

Share: [Twitter](https://twitter.com/share?text=Digital+echoes%3A+open+bus+behavior+on+the+compact+Macintosh&url=post/compact-mac-openbus/) [Facebook](https://www.facebook.com/sharer/sharer.php?u=post/compact-mac-openbus/)

## Read Next

August 23, 2024

### [Emulating the early Macintosh floppy drive](/post/mac-floppy-emu/)

[macintosh](/tag/macintosh/) [emulation](/tag/emulation/) [floppy](/tag/floppy/)

[Thomas](#) © 2025. Royce theme by [JustGoodThemes](https://justgoodthemes.com/).  
