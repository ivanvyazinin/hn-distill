...
![](me.jpeg) Philippe Gaultier

*   [Body of work](/blog/body_of_work.html)
*   [Tags](/blog/articles-by-tag.html)
*   [Resume](https://github.com/gaultier/resume/raw/master/Philippe_Gaultier_resume_en.pdf)
*   [](/blog/feed.xml)
*   [](https://www.linkedin.com/in/philippegaultier/)
*   [](https://github.com/gaultier)
*   [](https://hachyderm.io/@pg)
*   [](https://bsky.app/profile/pgaultier.bsky.social)
...
[GUI](/blog/articles-by-tag.html#gui) [x86\_64](/blog/articles-by-tag.html#x86-64) [X11](/blog/articles-by-tag.html#x11) [Optimization](/blog/articles-by-tag.html#optimization)
...
_Discussions: [Hacker News](https://news.ycombinator.com/item?id=36153237), [r/programming](https://old.reddit.com/r/programming/comments/13xgbk6/learn_x8664_assembly_by_writing_a_gui_from_scratch/), [Lobsters](https://lobste.rs/s/dvtzfl/learn_x86_64_assembly_by_writing_gui_from)._
...
_Note: Authentication is optional in the X11 protocol, but some X11 servers e.g. XWayland require it. Authentication is skipped here and is handled in a separate [article](/blog/write_a_video_game_from_scratch_like_1987.html#authentication)._

## [What do we need?](#what-do-we-need)[](#what-do-we-need)

I will be using the `nasm` assembler which is simple, cross-platform, fast, and has quite a readable syntax.

For the GUI, I will be using X11 since I am based on Linux and it has some interesting properties that make it easy to do without external libraries. If you are running Wayland, it should work with XWayland out of the box (_EDIT: After testing it, I can confirm it does work_), and perhaps also on macOS with XQuartz, but I have not tested those (for macOS, remember to tell `nasm` to use the `macho64` format, since macOS does not use the ELF format! Also, the stock linker on macOS does not support `-static`.).
...
## [X11 basics](#x11-basics)[](#x11-basics)
...
The [official documentation](https://www.x.org/releases/X11R7.7/doc/xproto/x11protocol.html) is pretty good, so when in doubt we can refer to it.

## [Main in x64 assembly](#main-in-x64-assembly)[](#main-in-x64-assembly)

Let's start slow with minimal program that simply exits with 0, and build from there.

First, we tell nasm we are writing a 64 bit program and that we target x86\_64. Then, we need a main function, which we call `_start` and needs to be visible since this is the entry point of our program (hence the `global` keyword):
...
_Note that Linux (and perhaps other Unices?) has a 'fun' difference, which is that the fourth parameter of a system call is actually passed using the register `r10`._

> Astute readers have pointed out that this is the case across all OSes and documented in the x86\_64 architecture supplement of the System V ABI. The more you know! That's only for system calls, though, regular functions still use `rcx` for the fourth parameter.
...
## [A stack primer](#a-stack-primer)[](#a-stack-primer)
...
### [A small stack example](#a-small-stack-example)[](#a-small-stack-example)
...
## [Opening a socket](#opening-a-socket)[](#opening-a-socket)
...
## [Connecting to the server](#connecting-to-the-server)[](#connecting-to-the-server)
...
## [Sending data over the socket](#sending-data-over-the-socket)[](#sending-data-over-the-socket)
...
For our handshake, we need to set the `order` to be `l`, that is, little-endian, since X11 can be told to interpret message as big or little endian. Since x64 is little-endian, we do not want to have an endianness translation layer and so we stick to little-endian.
...
This structure is only 12 bytes long, since we do not use authorization (we leave all subsequent fields after the `minor_version` as 0).
...
## [Generating ids](#generating-ids)[](#generating-ids)
...
## [Opening a font](#opening-a-font)[](#opening-a-font)
...
## [Creating a graphical context](#creating-a-graphical-context)[](#creating-a-graphical-context)
...
## [Creating the window](#creating-the-window)[](#creating-the-window)
...
## [Mapping the window](#mapping-the-window)[](#mapping-the-window)
...
![Black window](x11_x64_black_window.png)

Yay!

## [Polling for server messages](#polling-for-server-messages)[](#polling-for-server-messages)
...
## [Addendum: the full code](#addendum-the-full-code)[](#addendum-the-full-code)
...
> If you enjoy what you're reading, you want to support me, and can afford it: [Support me](https://paypal.me/philigaultier?country.x=DE&locale.x=en_US). That allows me to write more cool articles!
> 
> This blog is [open-source](https://github.com/gaultier/blog)! If you find a problem, please open a Github issue. The content of this blog as well as the code snippets are under the [BSD-3 License](https://en.wikipedia.org/wiki/BSD_licenses#3-clause_license_\(%22BSD_License_2.0%22,_%22Revised_BSD_License%22,_%22New_BSD_License%22,_or_%22Modified_BSD_License%22\)) which I also usually use for all my personal projects. It's basically free for every use but you have to mention me as the original author.