...
Hello! Earlier this summer I was talking to a friend about how much I [love using fish](https://jvns.ca/blog/2024/09/12/reasons-i--still--love-fish/), and how I love that I don’t have to configure it. They said that they feel the same way about the [helix](https://helix-editor.com/) text editor, and so I decided to give it a try.

I’ve been using it for 3 months now and here are a few notes.

### [why helix: language servers](#why-helix-language-servers)
...
### [the search is great](#the-search-is-great)
...
### [the quick reference is nice](#the-quick-reference-is-nice)
...
### [some vim -> helix translations](#some-vim-helix-translations)

*   Helix doesn’t have marks like `ma`, `'a`, instead I’ve been using `Ctrl+O` and `Ctrl+I` to go back (or forward) to the last cursor locarion
...
### [some helix annoyances](#some-helix-annoyances)

Here’s everything that’s annoyed me about Helix so far.

*   I like the way Helix’s `:reflow` works much less than how vim reflows text with `gq`. It doesn’t work as well with lists. ([github issue](https://github.com/helix-editor/helix/issues/3332))
*   If I’m making a Markdown list, pressing “enter” at the end of a list item won’t continue the list. There’s a [partial workaround](https://github.com/helix-editor/helix/wiki/Recipes#continue-markdown-lists--quotes) for bulleted lists but I don’t know one for numbered lists.
*   No persistent undo yet: in vim I could use an [undofile](https://vimdoc.sourceforge.net/htmldoc/options.html#'undofile') so that I could undo changes even after quitting. Helix doesn’t have that feature yet. ([github PR](https://github.com/helix-editor/helix/pull/9154))
...
*   Sometimes it crashes: every week or so there’s a segfault and the editor crashes. Someone mentioned that this might have something to do with the fact that I edit a lot of Markdown, not sure. This doesn’t bother me that much though, I can just reopen it.
...
### [switching was easier than I thought](#switching-was-easier-than-i-thought)
...
### [using a terminal-based text editor](#using-a-terminal-based-text-editor)
...
Three months is not that long, and it’s possible that I’ll decide to go back to Vim at some point. For example, I wrote a [post about switching to nix](https://jvns.ca/blog/2023/02/28/some-notes-on-using-nix/) a while back but after maybe 8 months I switched back to Homebrew (though I’m still using NixOS to manage one little server, and I’m still satisfied with that).

Want a weekly digest of this blog?  Subscribe

[New zine: The Secret Rules of the Terminal](https://jvns.ca/blog/2025/06/24/new-zine--the-secret-rules-of-the-terminal/ "Previous Post: New zine: The Secret Rules of the Terminal")

[Archives](/)

© Julia Evans. If you like this, you may like [Ulia Ea](https://web.archive.org/web/20181228051203/http://www.uliaea.ca/) or, more seriously, this list of [blogs I love](https://jvns.ca/blogroll) or some [books I've read](https://jvns.ca/bookshelf).  
