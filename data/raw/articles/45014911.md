...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fantirez%2Fsds)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E&source=header-repo&source_repo=antirez%2Fsds)
...
[antirez](/antirez) / **[sds](/antirez/sds)** Public

*   [Notifications](/login?return_to=%2Fantirez%2Fsds) You must be signed in to change notification settings
*   [Fork 491](/login?return_to=%2Fantirez%2Fsds)
*   [Star 5.2k](/login?return_to=%2Fantirez%2Fsds)
...
[BSD-2-Clause license](/antirez/sds/blob/master/LICENSE)

[5.2k stars](/antirez/sds/stargazers) [491 forks](/antirez/sds/forks) [Branches](/antirez/sds/branches) [Tags](/antirez/sds/tags) [Activity](/antirez/sds/activity)

[Star](/login?return_to=%2Fantirez%2Fsds)

[Notifications](/login?return_to=%2Fantirez%2Fsds) You must be signed in to change notification settings

*   [Code](/antirez/sds)
*   [Issues 38](/antirez/sds/issues)
*   [Pull requests 67](/antirez/sds/pulls)
*   [Actions](/antirez/sds/actions)
*   [Projects 0](/antirez/sds/projects)
*   [Wiki](/antirez/sds/wiki)
*   [Security](/antirez/sds/security)
    
    [](/antirez/sds/security)
    ...
    [There was an error while loading.](/antirez/sds/security) Please reload this page.
    
*   [Insights](/antirez/sds/pulse)

Additional navigation options

*   [Code](/antirez/sds)
*   [Issues](/antirez/sds/issues)
*   [Pull requests](/antirez/sds/pulls)
*   [Actions](/antirez/sds/actions)
*   [Projects](/antirez/sds/projects)
*   [Wiki](/antirez/sds/wiki)
*   [Security](/antirez/sds/security)
*   [Insights](/antirez/sds/pulse)
...
[Branches](/antirez/sds/branches)[Tags](/antirez/sds/tags)

[](/antirez/sds/branches)[](/antirez/sds/tags)
...
[60 Commits](/antirez/sds/commits/master/)

[](/antirez/sds/commits/master/)

[.gitignore](/antirez/sds/blob/master/.gitignore ".gitignore")

[.gitignore](/antirez/sds/blob/master/.gitignore ".gitignore")

[Changelog](/antirez/sds/blob/master/Changelog "Changelog")

[Changelog](/antirez/sds/blob/master/Changelog "Changelog")

[LICENSE](/antirez/sds/blob/master/LICENSE "LICENSE")

[LICENSE](/antirez/sds/blob/master/LICENSE "LICENSE")

[Makefile](/antirez/sds/blob/master/Makefile "Makefile")

[Makefile](/antirez/sds/blob/master/Makefile "Makefile")

[README.md](/antirez/sds/blob/master/README.md "README.md")

[README.md](/antirez/sds/blob/master/README.md "README.md")

[sds.c](/antirez/sds/blob/master/sds.c "sds.c")

[sds.c](/antirez/sds/blob/master/sds.c "sds.c")

[sds.h](/antirez/sds/blob/master/sds.h "sds.h")

[sds.h](/antirez/sds/blob/master/sds.h "sds.h")

[sdsalloc.h](/antirez/sds/blob/master/sdsalloc.h "sdsalloc.h")

[sdsalloc.h](/antirez/sds/blob/master/sdsalloc.h "sdsalloc.h")

[testhelp.h](/antirez/sds/blob/master/testhelp.h "testhelp.h")
...
**Notes about version 2**: this is an updated version of SDS in an attempt to finally unify Redis, Disque, Hiredis, and the stand alone SDS versions. This version is __NOT_ binary compatible_\* with SDS verison 1, but the API is 99% compatible so switching to the new lib should be trivial.
...
SDS strings as already mentioned don't follow this schema, and are instead a single allocation with a prefix that lives _before_ the address actually returned for the string.
...
sds sdsgrowzero(sds s, size\_t len);
...
void clusterWriteHandler(..., int fd, void \*privdata, ...) {
    ...
    ssize\_t nwritten \= write(fd, link\->sndbuf, sdslen(link\->sndbuf));
    if (nwritten <= 0) {
        /\* Error handling... \*/
    }
...
sds sdscpylen(sds s, const char \*t, size\_t len);
...
sds sdscatrepr(sds s, const char \*p, size\_t len);

The `scscatrepr` (where `repr` means _representation_) follows the usualy SDS string function rules accepting a char pointer and a length, so you can use it with SDS strings, normal C strings by using strlen() as `len` argument, or binary data. The following is an example usage:
...
There is an SDS function that is able to perform the reverse conversion and is documented in the _Tokenization_ section below.
...
sds sdsjoin(char \*\*argv, int argc, char \*sep, size\_t seplen);
sds sdsjoinsds(sds \*argv, int argc, const char \*sep, size\_t seplen);
...
At the very beginning of this documentation it was explained how SDS strings are allocated, however the prefix stored before the pointer returned to the user was classified as an _header_ without further details. For an advanced usage it is better to dig more into the internals of SDS and show the structure implementing it:
...
This approach is a memory management technique called _reference counting_ and in the context of SDS has two advantages:
...
*   The popular Valgrind program will detect SDS strings are _possibly lost_ memory and never as _definitely lost_, so it is easy to tell if there is a leak or not. I used Valgrind with Redis for years and every real leak was consistently detected as "definitely lost".
...
oldlen \= sdslen(s);
s \= sdsMakeRoomFor(s, BUFFER\_SIZE);
nread \= read(fd, s+oldlen, BUFFER\_SIZE);
...
[Activity](/antirez/sds/activity)

### Stars

[**5.2k** stars](/antirez/sds/stargazers)

### Watchers

[**141** watching](/antirez/sds/watchers)

### Forks

[**491** forks](/antirez/sds/forks)

[Report repository](/contact/report-content?content_url=https%3A%2F%2Fgithub.com%2Fantirez%2Fsds&report=antirez+%28user%29)

## [Releases](/antirez/sds/releases)

[2 tags](/antirez/sds/tags)

## [Packages 0](/users/antirez/packages?repo_name=sds)
...
## [Contributors 7](/antirez/sds/graphs/contributors)

*   [![@antirez](https://avatars.githubusercontent.com/u/65632?s=64&v=4)](https://github.com/antirez)
*   [![@melo](https://avatars.githubusercontent.com/u/485?s=64&v=4)](https://github.com/melo)
*   [![@dchest](https://avatars.githubusercontent.com/u/52677?s=64&v=4)](https://github.com/dchest)
*   [![@gavinwahl](https://avatars.githubusercontent.com/u/421710?s=64&v=4)](https://github.com/gavinwahl)
*   [![@frodsan](https://avatars.githubusercontent.com/u/840464?s=64&v=4)](https://github.com/frodsan)
*   [![@JuanitoFatas](https://avatars.githubusercontent.com/u/1000669?s=64&v=4)](https://github.com/JuanitoFatas)
*   [![@Meiye-lj](https://avatars.githubusercontent.com/u/71107931?s=64&v=4)](https://github.com/Meiye-lj)

## Languages

*   [C 99.6%](/antirez/sds/search?l=c)
*   [Makefile 0.4%](/antirez/sds/search?l=makefile)
