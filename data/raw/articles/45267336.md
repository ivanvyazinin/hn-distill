...
[animations](https://css-tricks.com/tag/animations/) [math](https://css-tricks.com/tag/math/)
...
[Juan Diego Rodríguez](https://css-tricks.com/author/monknow/) on Sep 15, 2025

Get affordable and hassle-free WordPress hosting plans with Cloudways — [start your free trial today](https://www.cloudways.com/en/wordpress-hosting.php?id=1223312).

No feature is truly “the worst” in CSS, right? After all, it’s all based on opinion and personal experience, but if we had to reach a consensus, checking the [State of CSS 2025](https://2025.stateofcss.com/en-US/) results would be a good starting point. I did exactly that, jumped into the [awards section](https://2025.stateofcss.com/en-US/awards/), and there I found it: the “Most Hated Feature,” a title no CSS should have bear…
...
This shocks me, if I’m being honest. Are really trigonometric functions really _that_ hated? I know “hated” is not the same as saying something is “worst”, but it still has an awful ring to it. And I know I’m being a little dramatic here, since only “9.1% of respondents truly hate trigonometry.” But that’s still too much shade being thrown for my taste.
...
#### [](#aa-css-trigonometric-functions-the-most-hated-css-feature)CSS Trigonometric Functions: The “Most Hated” CSS Feature

1.  **`sin()` and `cos()`** _(You are here!)_
2.  **Tackling the CSS `tan()` Function** _(coming soon)_
3.  **Inverse functions: `asin()`, `acos()`, `atan()` and `atan2()`** _(coming soon)_

### [](#aa-what-the-heck-are-cos-and-sin-anyway)What the heck are `cos()` and `sin()` anyway?

This section is for those who `cos()` and `sin()` don’t quite click yet, or simply want a refresher. If you aced trigonometry quizzes in high school, feel free to skip ahead to the [next section](#aa-circular-layouts)!

What I find funny about `cos()` and `sin()`— and also why I think there is confusion around them — is the many ways we can describe them. We don’t have to look too hard. A quick glance at this [Wikipedia page](https://en.wikipedia.org/wiki/Sine_and_cosine) has an eye-watering number of super nuanced definitions.

This is a [learning problem in the web development field](https://css-tricks.com/now-you-see-it/). I feel like some of those definitions are far too general and lack detail about the essence of what trigonometric functions like `sin()` and `cos()` can do. Conversely, other definitions are overly complex and academic, making them tough to grok without an advanced degree.
...
### [](#aa-circular-layouts)Circular layouts
...
**Note:** This step will become _much_ easier and concise when the [`sibling-index()`](https://css-tricks.com/almanac/functions/s/sibling-index/) and `[sibling-count()](https://css-tricks.com/almanac/functions/s/sibling-count/)` functions gain support (and [they’re _really_ neat](https://css-tricks.com/how-to-wait-for-the-sibling-count-and-sibling-index-functions/)). I’m hardcoding the indexes with inline CSS variables in the meantime.
...
### [](#aa-damped-oscillatory-animations)Damped oscillatory animations
...
This is, of course, trivial since we can do it in a single [`animation`](https://css-tricks.com/almanac/properties/a/animation/) declaration:
...
This “back and forth” animation is called _oscillatory_ movement. And while `cos()` or `sin()` are used to model oscillations in CSS, it would be like reinventing the wheel (albeit a clunkier one).
...
There’s a specific term that describes this: _damped_ oscillatory movement. And guess what? We can model it in CSS with the `cos()` function! If we graph it over time, then we will see it goes back and forth while getting closer to the resting position[1](#footnote-1).
...
[Wikipedia has another animated example](https://en.wikipedia.org/wiki/File:Damped_Spring_Simulation.gif) that nicely demonstrates what damped oscillation looks like.
...
*   **cos(ωt−α):** This gives the movement its oscillation as time passes. Time is multiplied by frequency (ω), which determines an element’s oscillation speed[2](#footnote-2). We can also subtract from time α, which we can use to offset the initial oscillation of the system.
...
Given these variables, we can peek at what the animation would look like on a graph using a tool like [GeoGebra](https://www.geogebra.org/graphing):
...
Remember that to animate or transition a custom property, we’ve gotta register it with the [`@property`](https://css-tricks.com/almanac/rules/p/property/) at-rule.
...
This is even more satisfying! A circular _and_ damped motion, all thanks to `cos()` and `sin()`. Besides looking great, how could this be used in a real layout?
...
### [](#aa-more-trigonometry-to-come)More trigonometry to come!
...
#### [](#aa-footnotes)Footnotes

1.  This kind of damped oscillatory movement, where the back and forth is more visible, is called underdamped oscillation. There are also overdamped and critically damped oscillations, but we won’t focus on them here. [↪️](#fn-1)
2.  In reality, the damped constant and the frequency are closely related. You can read more about damped oscillation in [this paper](https://phys.libretexts.org/Bookshelves/University_Physics/University_Physics_\(OpenStax\)/Book%3A_University_Physics_I_-_Mechanics_Sound_Oscillations_and_Waves_\(OpenStax\)/15%3A_Oscillations/15.06%3A_Damped_Oscillations). [↪️](#fn-2)

## TOC

1.  [What the heck are cos() and sin() anyway?](#aa-what-the-heck-are-cos-and-sin-anyway)
2.  [Circular layouts](#aa-circular-layouts)
3.  [Wavy layouts](#aa-wavy-layouts)
4.  [Damped oscillatory animations](#aa-damped-oscillatory-animations)
5.  [More trigonometry to come!](#aa-more-trigonometry-to-come)
...
1.  Sacha Greif
    ...
    [Permalink to comment#](https://css-tricks.com/the-most-hated-css-feature-cos-and-sin/#comment-1883296) September 15, 2025
    
    Great article! I’m sure more CSS developers will learn to love trigonometry in time :)
    
    [Reply](#comment-1883296)
    
    *   Juan Diego Rodríguez
        
        [Permalink to comment#](https://css-tricks.com/the-most-hated-css-feature-cos-and-sin/#comment-1883299) September 16, 2025
        
        Heyy, Sacha! Thankss, I hope so. We all know the most hated one is `vertical-align`
        

### Leave a Reply [Cancel reply](/the-most-hated-css-feature-cos-and-sin/#respond)
