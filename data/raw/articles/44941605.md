...
[Introduction](#introduction)[The basic idea](#the-basic-idea-1)[Commands](#commands-2)[Move](#move-3)[Lines](#lines-4)[Bézier curves](#bzier-curves-5)[Arcs](#arcs-8)[Animations](#animations-14)[Lil’ extras](#lil-extras-15)[Closing paths](#closing-paths-16)[Relative commands](#relative-commands-17)[Chained curves](#chained-curves-18)
...
This blog post is written for web developers of all experience levels, though I do assume that you understand the basics of SVG.

If you’re not super comfortable with SVG fundamentals, I have another post that you should read first:

*   [A Friendly Introduction to SVG](/svg/friendly-introduction-to-svg/)
...
If you’ve ever inspected SVGs in the wild, you’ve likely found that the `<path>` commands are even more inscrutable than the one above. Everything is smushed together, without any delineation:
...
<path d="M4 12L12 4M6 4L12 4L12 10" />
```

The [W3C specification(opens in new tab)](https://www.w3.org/TR/SVG/paths.html#PathDataGeneralInformation) states that SVGs were designed to be very compact “in order to allow for minimal file size and efficient downloads”. Whitespace is only required between adjacent numbers. Separators like commas and even newlines are tolerated, but are considered “superfluous”.

**But here’s the thing:** The difference in file size is negligible, _especially_ when considering gzip compression (a standard feature built into all modern web servers). In fact, I did a quick test, and the version with commas/spaces was _smaller_ after compression!

I don’t want to start any internet beef with the W3C, but I _strongly_ encourage you to include commas and whitespace in your SVGs. They’re _so much easier_ to understand. Your future self and your co-workers will thank you for it, and it won’t negatively affect your users at all. ❤️
...
One thing to watch out for: SVG commands are case-sensitive. The lowercase `m` command works differently from the uppercase `M` command. We generally want to use uppercase commands.

I’ll share what the difference is later in this blog post, when we talk about relative commands.
...
If you’ve been working with CSS for a while, you’ve likely used Bézier curves to control the _easing_ of your [CSS transitions](/animation/css-transitions/). In SVG, we can create those same swoopy curves, but they’re drawn visually rather than used mathematically.
...
**The math of Bézier curves**

If you’re wondering how exactly the browser turns these three points into a curved line, check out this video from the incredibly talented Freya Holmér:

*   [The Beauty of Bézier Curves(opens in new tab)](https://youtu.be/aVwxzDHniEw?t=115)
    

This link will take you straight to the most important part of the video, for our purposes, but the entire video is worth watching.

That said, I don’t _actually_ find that understanding the math really provides any clarity. Personally, I prefer to imagine the control point pinching and pulling the line towards it.
...
![A mathematical compass, a V-shaped tool with a sharp end and a pencil end. By poking the sharp end into a paper, you can use it to draw a perfect circle centered around it](/_next/image/?url=%2Fimages%2Finteractive-guide-to-paths%2Fcompass.jpg&w=828&q=75)
...
![Golf ball](/images/interactive-guide-to-paths/golf-ball.png)
...
![Baseball](/images/interactive-guide-to-paths/baseball.png)

And if we chose an _enormous_ ball like a bowling ball, the arc would practically be flat:

![bowling](/images/interactive-guide-to-paths/bowlingball.png)

To bring this back to SVGs: when we draw an arc, we need to specify a horizontal radius (`rx`) and a vertical radius (`ry`). These value affect the size of the hypothetical ellipse, which in turn affects how deep/shallow the arc is. The smaller the radius, the deeper the arc.

**Impossible situations?**

One of the things that kept me from _really_ understanding arcs for years was that it just didn’t seem possible that any two points could always be connected by a dynamically-sized ellipse.

In my mind, I was always picturing the two points on opposite ends of the ellipse, like this:

In _this_ case, the ellipse has to be a very specific size, where the horizontal diameter is equal to the gap between points. But as we’ve seen above, the points don’t _have_ to be on opposite sides of the ellipse! If we provide a much larger radius, we’ll only use a small slice of the ellipse to connect our two points.

**That said, there’s still one weird case worth discussing.** What if the ellipse is _smaller_ than the gap?

?

The SVG specification has [some more math(opens in new tab)](https://www.w3.org/TR/SVG11/implnote.html#ArcCorrectionOutOfRangeRadii) that shows what should happen in this case, and honestly, I couldn’t tell you exactly what it does 😅. From what I’ve gathered through experimentation, the browser _scales up_ the ellipse, preserving the ratio between `rx` and `ry`.

And with that one exception, the problem is solved! We can use any positive values we want for `rx` and `ry` and the browser will always be able to conjure up an ellipse that works.
...
[![Whimsical Animations](/_next/image/?url=%2Fimages%2Fwhimsical-animations.jpg&w=1920&q=75)](https://whimsy.joshwcomeau.com/)
...
I’m planning on launching this course in _Early Access_ in late September. You can join the waitlist to be the first to know when registration opens:
...
In the demo above, you might’ve noticed that the first corner becomes beveled when you move the start point too far to the right. Specifically, it appears to flip below 30°:

angle:45

For a long time, I thought this was a rendering quirk, a bug. It turns out that this is actually a feature! 😅

The idea is that when angles are very acute, the corners become way too pointy, so the `stroke-linejoin` property automatically flips from the default `miter` value to `bevel`.

the [stroke-miterlimit property(opens in new tab)](https://developer.mozilla.org/en-US/docs/Web/SVG/Reference/Attribute/stroke-miterlimit) lets us adjust the breakpoint. It uses a rather complicated formula, but if we pick a large value like 100, our corners should almost always stay sharp:
...
![close-up screenshot of the SVG from the playground above, showing a kink in the curve](/_next/image/?url=%2Fimages%2Finteractive-guide-to-paths%2Fsvg-elbow-annotated.png&w=640&q=75)
