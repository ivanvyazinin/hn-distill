...
How would bound pointers work? Ideally you would initialize them with either a regular pointer-to-function or a reference to an object and a pointer-to-member-function. Once initialized, they would behave like normal pointer-to-functions. You could apply the function call `operator()` to them to invoke the function. In order to be suitable for a callback mechanism, the information about the type of the callee would \_not\_ be part of the type of the bound-pointer. It might look something like this:
...
class Button{
public:
	Button(void (\* \_\_bound uponClickDoThis)() )
...
public:
	...
	FunctorBase(const void \*c,const void \*f,size\_t sz)
		{
...
protected:
	...
	Functor1(Thunk t,const void \*c,const void \*f,size\_t sz):
...
Thanks to my fellow developers at RCS and to [Greg Comeau](http://www.comeaucomputing.com) for reviewing and commenting on this article.
...
[rhickey@bestweb.net](mailto:rhickey@bestweb.net)
