A Programmer-Friendly I/O Abstraction Over io\_uring and kqueue      [
...
[Docs](https://docs.tigerbeetle.com) [Blog](/blog) [Company](/company) [![Slack icon](/blog/img/icon-slack.svg) Slack ](https://slack.tigerbeetle.com/join)[![YouTube icon](/blog/img/icon-youtube.svg) 11.4K ](https://www.youtube.com/@tigerbeetledb)[![GitHub icon](/blog/img/icon-github.svg) 14.5K](https://github.com/tigerbeetle/tigerbeetle)
...
*   [![](/blog/img/authors/king.webp)
    
    King Butcher
    
    ](https://x.com/kingprotty)
*   [![](/blog/img/authors/phil.webp)
    
    Phil Eaton
    
    ](https://x.com/eatonphil)
...
## [Batching and readiness](#batching-and-readiness)
...
## [Central I/O dispatch](#central-io-dispatch)
...
## [Interacting with the submission and completion queue](#interacting-with-the-submission-and-completion-queue)
...
## [Callbacks and context](#callbacks-and-context)
...
## [Getting more expressive](#getting-more-expressive)
...
The overflow queue needs to be processed eventually, so we update our `flush` function (described in [Callbacks and context](#callbacks-and-context) above) to pull as many events from our overflow queue before submitting a batch to io\_uring or kqueue.
...
Let’s check out how the [Darwin version](https://github.com/tigerbeetle/tigerbeetle/blob/9d3552ba137a773d4b81106739e56cba6cd32a03/src/io/darwin.zig#L436) of TigerBeetle’s I/O library (with kqueue) differs from the [Linux version](https://github.com/tigerbeetle/tigerbeetle/blob/9d3552ba137a773d4b81106739e56cba6cd32a03/src/io/linux.zig#L656). As mentioned, the complete `send` call in the Darwin implementation waits for file descriptor readiness (through kqueue). Once ready, the actual `send` call is made back in userland:
...
Compare this to the [Linux version](https://github.com/tigerbeetle/tigerbeetle/blob/9d3552ba137a773d4b81106739e56cba6cd32a03/src/io/linux.zig#L656) (with io\_uring) where the kernel handles everything and there is no send system call in userland:
...
## [Standalone cross-platform evented I/O library?](#standalone-cross-platform-evented-io-library)

Hey, maybe we’ll split this out so you can use it too. It’s written in Zig so we can easily expose a C API. Any language with a C foreign function interface (i.e. every language) should work well with it. Keep an eye on [our GitHub](https://github.com/tigerbeetle). :)
...
![](../2023-01-30-series-seed-announcement/banner.webp)
...
](../2023-01-30-series-seed-announcement)[

![](../2022-10-12-a-database-without-dynamic-memory/banner.webp)
...
](../2022-10-12-a-database-without-dynamic-memory)
