...
> Note: If you are already aware of the UTF-8 encoding, you can explore the [UTF-8 Playground](https://utf8-playground.netlify.app/) utility that I built to visualize UTF-8 encoding.
...
### Example: Hindi Letter "अ" ([open in UTF-8 Playground](https://utf8-playground.netlify.app/0905))
...
The code point `U+0905` ([see official chart](https://www.unicode.org/charts/PDF/U0900.pdf)) represents the Hindi letter "अ" in the Unicode character set.
...
The text `Hey👋 Buddy` has both English characters and an emoji character on it. The [text file](/utf8-samples/text_with_emoji_character.txt) with this text saved on the disk will have the following **13 bytes** in it:
...
Starts with `0`, so it's a single-byte ASCII character. The remaining bits `1001000` represent the letter 'H'. ([open in playground](https://utf8-playground.netlify.app/0048))

01100101

Starts with `0`, so it's a single-byte ASCII character. The remaining bits `1100101` represent the letter 'e'. ([open in playground](https://utf8-playground.netlify.app/0065))

01111001

Starts with `0`, so it's a single-byte ASCII character. The remaining bits `1111001` represent the letter 'y'. ([open in playground](https://utf8-playground.netlify.app/0079))
...
The bits from these four bytes (excluding the leading bits) combine to form the binary sequence 00001 11110100 01001011, which is `1F44B` in hexadecimal, corresponds to the code point `U+1F44B`. This code point represents the waving hand emoji "👋" in the Unicode character set [(open in playground)](https://utf8-playground.netlify.app/1F44B).

00100000

Starts with `0`, so it's a single-byte ASCII character. The remaining bits `0100000` represent a whitespace character. ([open in playground](https://utf8-playground.netlify.app/0020))

01000010

Starts with `0`, so it's a single-byte ASCII character. The remaining bits `1000010` represent the letter 'B'. ([open in playground](https://utf8-playground.netlify.app/0042))

01110101

Starts with `0`, so it's a single-byte ASCII character. The remaining bits `1110101` represent the letter 'u'. ([open in playground](https://utf8-playground.netlify.app/0075))

01100100

Starts with `0`, so it's a single-byte ASCII character. The remaining bits `1100100` represent the letter 'd'. ([open in playground](https://utf8-playground.netlify.app/0064))
...
The [text file](/utf8-samples/text_with_ASCII_only.txt) doesn't have any non-ASCII characters. The file saved on the disk has the following **9 bytes** in it:
...
When I was exploring the UTF-8 encoding, I couldn't find any good tool to interactively visualize how UTF-8 encoding works. So I built **UTF-8 Playground** to visualize and play around with UTF-8 encoding. [Give it a try!](https://utf8-playground.netlify.app/1F44B).

[#tech](/tags/tech/) [#history](/tags/history/) [#programming](/tags/programming/)

© 2025 Vishnu Haridas • [RSS Feed](/rss.xml) • [All Tags](/tags/) • Built with [April⋅SSG](https://github.com/vishnuharidas/april-ssg)