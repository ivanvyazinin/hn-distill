...
*   [Memory Waste in Standard Vectors](#memory-waste-in-standard-vectors)
*   [Packing and Accessing Bits](#packing-and-accessing-bits)
*   [Crossing Word Boundaries](#crossing-word-boundaries)
*   [Faster Reads: Unaligned Access](#faster-reads-unaligned-access)
*   [Random Access Performance](#random-access-performance)
*   [Iterating Over Values](#iterating-over-values)
*   [Double-Ended Iteration](#double-ended-iteration)
*   [Writing bits](#writing-bits)
*   [In-Word Write](#in-word-write)
*   [Spanning Write](#spanning-write)
*   [Random Write Performance](#random-write-performance)
*   [Architecture](#architecture)
*   [Word Trait: The Physical Storage Layer](#word-trait-the-physical-storage-layer)
*   [Storable Trait: The Logical Type Layer](#storable-trait-the-logical-type-layer)
*   [Builder Pattern](#builder-pattern)
*   [Doing more than just reading](#doing-more-than-just-reading)
*   [Mutability: Proxy Objects](#mutability-proxy-objects)
...
Let’s trace an access with `bit_width = 10` for the element at `index = 7`. The starting bit position is `7 * 10 = 70`. This maps to `word_index = 1` and `bit_offset = 6`. Our 10-bit integer begins at the 6th bit of the _second_ `u64` word in our storage.
...
To correctly decode the value, we must read _two_ consecutive `u64` words and combine their bits. This splits our `get_unchecked` implementation into two paths. The first is the fast path we’ve already seen. The second is a new path for spanning values.
...
To get the upper bits of the value, we read the _next_ word. The bits we need are at the beginning of this word, so we shift them left to align them correctly.
...
Immediately following is `mov rdx, qword ptr [r8 + 8*rdx + 8]`. This is our second, distinct memory access. It loads the _next_ 64-bit word from memory by adding an 8-byte offset to the previous address. This corresponds to `limbs[word_index + 1]`.
...
We can try to implement a more aggressive access method. The strategy is to calculate the exact _byte_ address where our data begins and perform a single, unaligned read of a full `W` word from that position.
...
With the byte-level address, we get a raw `*const u8` pointer to our storage and perform the unaligned read. The [read\_unaligned](https://doc.rust-lang.org/std/ptr/fn.read_unaligned.html) intrinsic in Rust compiles down to a single machine instruction that the hardware can execute efficiently.
...
We can now benchmark the latency of 1 million random access operations on a vector containing 10 million elements. For each `bit_width`, we generate data with a uniform random distribution in the range `[0, 2^bit_width)`. The code for the benchmark is available here: [`bench-intvec`](https://github.com/lukefleed/compressed-intvec/blob/master/benches/fixed/bench_random_access.rs)

Our baseline is the smallest standard `Vec<T>` capable of holding the data (`Vec<u8>` for `bit_width <= 8`, etc.). We also include results from [`sux`](https://docs.rs/sux/latest/sux/bits/bit_field_vec/index.html), [`succinct`](https://docs.rs/succinct/latest/succinct/trait.IntVec.html), and [`simple-sds-sbwt`](https://docs.rs/simple-sds-sbwt/latest/simple_sds_sbwt/int_vector/struct.IntVector.html) for context. I am not aware of any other Rust crates that implement fixed-width bit-packed integer vectors, so if you know of any, please let me know!

We can see that for `bit_width` values below 32, the `get_unaligned_unchecked` of our `FixedVec` is almost always faster than the corresponding `Vec<T>` baseline. This is a result of improved cache locality. A 64-byte L1 cache line can hold 64 elements from a `Vec<u8>`. With a `bit_width` of 4, the same cache line holds `(64 * 8) / 4 = 128` elements from our `FixedVec`. This increased density improves the cache hit rate for random access patterns, and the latency reduction from avoiding DRAM access outweighs the instruction cost of the bitwise extraction. For values of `bit_width` above 32, the performance of `FixedVec` are _very slightly_ worse than the `Vec<T>` baseline, as the cache locality advantage diminishes. However, the memory savings remain.
...
We can see that the implementation of [`sux`](https://docs.rs/sux/latest/sux/bits/bit_field_vec/index.html) is almost on par with ours. The other two crates, [`succinct`](https://docs.rs/succinct/latest/succinct/trait.IntVec.html) and [`simple-sds-sbwt`](https://docs.rs/simple-sds-sbwt/latest/simple_sds_sbwt/int_vector/struct.IntVector.html), are significantly slower (note that the Y-axis is logarithmic). Tho, it’s worth noting that neither of these last two crates provides unchecked or unaligned access methods, so their implementations are inherently more conservative.
...
We can think then about a _stateful_ iterator. It should operate directly on the bitvector, maintaining its own position. Instead of thinking in terms of logical indices, it should think in terms of a “bit window”, a local `u64` register that holds the current chunk of bits being processed.
...
Initializing the front is easy: we load `limbs[0]` into `front_window`. The back is more complex. We must calculate the exact word index and the number of valid bits in the _last_ word that contains data. This requires a bit of arithmetic to handle cases where the data doesn’t perfectly fill the final word.
...
Next, we need to create a “hole” in our local copy where the new value will go. We do this by creating a mask that has ones _only_ in the bit positions we want to modify, and then inverting it to create a clearing mask.
...
First, we handle the `low_word`. We need to replace its high bits (from `bit_offset` onwards) with the low bits of our new value. The most direct way is to create a mask for the bits we want to _keep_. The expression `(1 << bit_offset) - 1` is a bit-twiddling trick to generate a mask with `bit_offset` ones at the least significant end.
...
As for the reads, the performance of our `FixedVec` is almost identical to that of [`sux`](https://docs.rs/sux/latest/sux/bits/bit_field_vec/index.html). The other two crates, [`succinct`](https://docs.rs/succinct/latest/succinct/trait.IntVec.html) and [`simple-sds-sbwt`](https://docs.rs/simple-sds-sbwt/latest/simple_sds_sbwt/int_vector/struct.IntVector.html), are again slower. It’s worth noting that also for the writes, neither of these last two crates provides unchecked methods.

> For the 64-bit width case, I honestly have no idea what is going on with [`sux`](https://docs.rs/sux/latest/sux/bits/bit_field_vec/index.html) being so much faster than everything else, even then `Vec<u64>`! Mybe some weird compiler optimization? If you have any insight, please let me know.
...
Once the structure logic is in place, we have to design an ergonomic way to construct it. A simple `new()` function isn’t sufficient because the vector’s memory layout depends on parameters that must be determined _before_ allocation, most critically the `bit_width`. This is a classic scenario for a builder pattern.

The central problem is that the optimal `bit_width` often depends on the data itself. We need a mechanism to specify the _strategy_ for determining this width. We can create the `BitWidth` enum:
...
*    [Rust](/tags/Rust)
*    [Succinct Data Structures](/tags/Succinct Data Structures)

* * *

[](https://github.com/lukefleed " Luca Lombardo on Github")[](https://www.linkedin.com/in/l-lombardo/ "Luca Lombardo on LinkedIn")[](mailto:l.lombardo@pm.me "Send an email to Luca Lombardo")
