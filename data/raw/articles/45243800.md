...
2025-09-13 :: Bogdan Ionescu:: 6 min read (1265 words)[source](https://github.com/BogdanTheGeek/blog/tree/main/content/projects/vapeserver.md "Source for this page") [report issue](https://github.com/BogdanTheGeek/blog/issues/new?template=corrections.md&title=[Correction]: Hosting%20a%20WebSite%20on%20a%20Disposable%20Vape "Submit Correction")

![Hosting a WebSite on a Disposable Vape](vapeserver.jpeg "Cover Image")

# Preface[#](#preface)

This article is served from a web server running on a disposable vape. Many thanks to [Mia Nowill](https://soundcloud.com/fkahardcore) for hosting it for me. If you want some eye candy, check out [the version on my blog](https://bogdanthegeek.github.io/blog/projects/vapeserver/). The content is otherwise identical.
...
The chip is marked `PUYA C642F15`, which wasn't very helpful. I was pretty sure it was a `PY32F002A`, but after poking around with [pyOCD](http://pyocd.io/), I noticed that the flash was 24k and we have 3k of RAM. The extra flash meant that it was more likely a `PY32F002B`, which is actually a very different chip.[1](#fn:1)
...
The idea of hosting a web server on a vape didn't come to me instantly. In fact, I have been playing around with them for a while, but after writing my post on [semihosting](/insights/jlink-rtt-for-the-masses/), the penny dropped.
...
For this blog however, I payed for none of the RAM, so I'll use all of the RAM.
...
1.  While getting things together for this post, I came across [this](https://github.com/grahamwhaley/py32c642_vape) project that correctly identified these MCUs as PY32C642, which are pretty much identical to the 002B. [↩︎](#fnref:1)
    
2.  Later modems used PPP (Point-to-Point Protocol) [↩︎](#fnref:2)
