...
[How To Scale Your Model](/scaling-book) Toggle navigation
...
*   [GPUs vs. TPUs at the chip level](#gpus-vs-tpus-at-the-chip-level)
...
A modern ML GPU (e.g. H100, B200) is basically a bunch of compute cores that specialize in matrix multiplication (called **Streaming Multiprocessors** or **SMs**) connected to a stick of fast memory (called **HBM**). Here’s a diagram:
 ...
 ![](/scaling-book/assets/gpu/gpu-diagram.png)

**Figure:** a diagram showing the abstract layout of an H100 or B200 GPU. An H100 has 132 SMs while a B200 has 148. We use the term 'Warp Scheduler' somewhat broadly to describe a set of 32 CUDA SIMD cores _and_ the scheduler that dispatches work to them. Note how much this looks like a TPU!
...
**Figure:** an example of warp divergence within a set of threads ([source](https://images.nvidia.com/content/volta-architecture/pdf/volta-architecture-whitepaper.pdf)). White spaces indicate stalls of at least some fraction of the physical CUDA cores
...
FLOPs/s/chip (bf16/fp16)

FLOPs/s/chip (fp8/int8)
...
Core “cell” that contains other units

Warp Scheduler

VPU

SIMD vector arithmetic unit
...
High bandwidth high capacity memory
...
\\\[T = \\max(T\_\\text{comms}, T\_\\text{math}) = \\frac{12 \\cdot N}{\\text{3.35e12}} = \\frac{N}{\\text{2.8e11}}\\\]
...
NVLink Bandwidth (GB/s, full-duplex)

NVLink Ports / GPU

Node GPU to GPU bandwidth (GB/s full-duplex)

Node size (NVLink domain)
...
Bandwidth per Unit (TB/s, full-duplex)

GPU-to-GPU Bandwidth (GB/s, full-duplex)

Fat Tree Bandwidth (GB/s, full-duplex)
...
\\\[T\_\\text{AG or RS comms} = \\frac{\\text{bytes} \\cdot (N - 1)}{N \\cdot \\text{GPU egress bandwidth}} \\rightarrow \\frac{\\text{bytes}}{\\text{GPU egress bandwidth}}\\\]
...
\\\[T\_\\text{AllToAll comms} = \\frac{B \\cdot (N - 1)}{W \\cdot N^2} \\approx \\frac{B}{W \\cdot N}\\\]
...
\\\[T\_\\text{SHARP AR comms} = \\frac{\\text{bytes}}{\\text{GPU egress bandwidth}}\\\]
...
\\\[T\_\\text{AG or RS comms} = \\frac{\\text{bytes}}{W\_\\text{node egress}} \\underset{H100}{=} \\frac{\\text{bytes}}{\\text{400e9}}\\\]
...
\\\[T\_\\text{total} = \\max(T\_\\text{comms at node}, T\_\\text{comms in scale-out network}) = \\max\\left\[\\frac{\\text{bytes}}{W\_\\text{GPU egress}}, \\frac{\\text{bytes}}{W\_\\text{node egress}}\\right\]\\\] You can see a more precise derivation here.
...
\\\[T\_\\text{AG or RS comms} = \\text{bytes} \\cdot max\_\\text{depth i}\\left\[\\frac{D\_i - 1}{D\_i \\cdot W\_\\text{link i}}\\right\]\\\]
...
\\\[T\_\\text{AllToAll comms} = \\frac{B \\cdot (M - 1)}{M^2 \\cdot W\_\\text{node egress}} \\approxeq \\frac{B}{M \\cdot W\_\\text{node egress}}\\\]
...
Switch Bandwidth (full-duplex, TB/s)

Cable Bandwidth (full-duplex, TB/s)

Collective Bandwidth (GB/s)
...
\\\[\\text{AllReduce}\_X(A\[I\_Y, J\]\\ \\{ U\_X \\})\\\]
...
\\\[T\_\\text{comms at node} = \\frac{\\text{bytes} \\cdot D\_\\text{node}}{\\min(Y, D\_\\text{node}) \\cdot W\_\\text{GPU egress}}\\\] \\\[T\_\\text{comms in scale-out network} = \\frac{\\text{bytes} \\cdot N}{Y \\cdot W\_\\text{node egress}}\\\] \\\[T\_\\text{total} = \\max(T\_\\text{comms at node}, T\_\\text{comms in scale-out network})\\\]
...
\\\[T\_\\text{AR or RS comms} = \\text{bytes} \\cdot \\max\_{\\text{depth } i}\\left\[\\frac{D\_i - 1}{D\_i \\cdot \\max(Y, S\_{i-1}) \\cdot W\_{\\text{link } i}}\\right\]\\\]
...
\\\[T\_\\text{comms} = \\frac{2 \\cdot D \\cdot F \\cdot 256}{Y \\cdot \\text{12.8e12}} = \\frac{2DF}{Y \\cdot \\text{50GB/s}}\\\]
...
\\\[T\_\\text{math} = \\frac{2 \\cdot 2 \\cdot 2 \\cdot BDF}{X \\cdot C}\\\] \\\[T\_\\text{comms} = \\frac{2 \\cdot 2 \\cdot 2 \\cdot DF}{W\_\\text{collective}}\\\]

Therefore, for $T\_\\text{math} > T\_\\text{comms}$, we need $B / (XC) > 1 / W\_\\text{collective}$ or

\\\[\\frac{B}{X} > \\frac{C}{W\_\\text{collective}}\\\]
...
\\\[T\_\\text{math} = \\frac{2 \\cdot 2 \\cdot 2 \\cdot k \\cdot BDF}{X \\cdot C}\\\] \\\[T\_\\text{comms} = \\frac{2 \\cdot 2 \\cdot 2 \\cdot EDF}{W\_\\text{collective}}\\\]

which inflates the per-GPU token batch size by a factor of $E/k$, i.e.

\\\[\\frac{B}{X} > \\frac{E}{k} \\frac{C}{W\_\\text{collective}}\\\]
...
\\\[T\_\\text{math} = \\frac{2 \\cdot 2 \\cdot 2 \\cdot BDF}{N \* C}\\\] \\\[T\_\\text{comms} = \\frac{2 \\cdot 2 \\cdot 2 \\cdot DF \\cdot (X-1)}{X \\cdot W\_\\text{collective}}\\\]
...
\\\[T\_\\text{math} = \\frac{2\\cdot 2 \\cdot BDF}{Y \\cdot C}\\\] \\\[T\_\\text{comms} = \\frac{2\\cdot 2 \\cdot BD}{W\_\\text{collective}}\\\]

which to be compute-bound gives us the rule

\\\[Y < \\frac{F \\cdot W\_\\text{collective}}{C}\\\]
...
As noted above, the cost of this AllToAllZ->k(\[B, D, k\]) if it spans multiple nodes is roughly $T\_\\text{AllToAll} = 2 \\cdot B \\cdot D \\cdot (Z-8)/Z \\min(8 \* k / Z, 1)$, so for pure expert parallelism we need

\\\[T\_\\text{math} = \\frac{4 \\cdot B \\cdot k \\cdot D \\cdot F}{Z \\cdot C}\\\] \\\[T\_\\text{comms} = \\frac{4 \\cdot B \\cdot D \\cdot (Z-8)}{W \\cdot Z} \\cdot \\min\\left(\\frac{8 \\cdot k}{Z}, 1\\right)\\\]
...
\\\[T\_\\text{total PP comms} = \\frac{2BD}{W \\cdot N\_\\text{microbatches}} \\cdot (N\_\\text{microbatches} + N\_\\text{stages} - 2)\\\] \\\[T\_\\text{per-layer comms} \\approx 1.5 \\cdot \\frac{2BD}{W \\cdot N\_\\text{layers}}\\\]
...
Please enable JavaScript to view the [comments powered by giscus.](http://giscus.app/?ref_noscript)

© Copyright 2025 . Powered by [Jekyll](https://jekyllrb.com/) with [al-folio](https://github.com/alshedivat/al-folio) theme. Hosted by [GitHub Pages](https://pages.github.com/).