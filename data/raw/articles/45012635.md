Linear scan with lifetime holes | Max Bernstein                      
...
In my [last post](/blog/linear-scan/), I explained a bit about how to retrofit SSA onto the original linear scan algorithm. I went over all of the details for how to go from low-level IR to register assignments—liveness analysis, scheduling, building intervals, and the actual linear scan algorithm.

Basically, we made it to 1997 linear scan, with small adaptations for allocating directly on SSA.

This time, we’re going to retrofit _lifetime holes_.
...
According to [Linear Scan Register Allocation on SSA Form](/assets/img/wimmer-linear-scan-ssa.pdf) (PDF, 2010):
...
Lifetime holes come from [Quality and Speed in Linear-scan Register Allocation](/assets/img/quality-speed-linear-scan-ra-clean.pdf) (PDF, 1998) by Traub, Holloway, and Smith. Figure 1, though not in SSA form, is a nice diagram for understanding how lifetime holes may occur. Unfortunately, the paper contains a rather sparse plaintext description of their algorithm that I did not understand how to apply to my concrete allocator.
...
**Even though** we are adding some gaps between ranges, each interval still gets assigned _one location for its entire life_. It’s just that in the gaps, we get to put other smaller intervals, like lichen growing between bricks.
...
We can change this to support multiple ranges by changing _just one character_!!!
...
It took a little bit of untangling, but the required modifications to support lifetime holes in the register assignment phase are not too invasive. To get an idea of the difference, I took the original [Poletto1999](/assets/img/linearscan-ra.pdf) (PDF) algorithm and rewrote it in the style of the [Mössenböck2002](/assets/img/linear-scan-ra-context-ssa.pdf) (PDF) algorithm.
...
Note that this begins to depart from strictly linear (time) linear scan: the `inactive` set is bounded not by the number of physical registers but instead by the number of virtual registers. Mössenböck2002 notes that the size of the `inactive` set is generally very small, though, so “linear in practice”.
...
Maybe next time we will add _interval splitting_, which will help us a) address ABI constraints more cleanly in function calls and b) remove the dependence on reserving a scratch register.

* * *

This blog is [open source](https://github.com/tekknolagi/tekknolagi.github.com). See an error? Go ahead and [propose a change](https://github.com/tekknolagi/tekknolagi.github.com/edit/main/_posts/2025-08-24-linear-scan-lifetime-holes.md "Help improve _posts/2025-08-24-linear-scan-lifetime-holes.md").  
