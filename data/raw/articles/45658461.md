...
*   [Archives](/post/)
*   [Subscribe](/subscribe)
*   [Author](https://nelhage.com/)

# Solving Regex Crosswords with Z3

Oct 21, 2025

For a while now, I’ve been fascinated by [Z3](https://microsoft.github.io/z3guide/) and by [SMT solving](https://en.wikipedia.org/wiki/Satisfiability_modulo_theories) more broadly. While on pat leave recently, I was reminded of the existence of [regular-expression crossword puzzles](https://regexle.com/), and allowed myself to get nerdsniped by writing a Z3-backed solver.
...
## Regular expression crosswords  [_🔗︎_](#regular-expression-crosswords)

To briefly recap: A regular-expression crossword consists of a grid of (unknown) characters, which are to-be-determined. These characters are constrained by a set of given [regular expressions](https://en.wikipedia.org/wiki/Regular_expression), which must match a given row or column, once filled in. I chose to solve the [regexle](https://regexle.com/) variant, specifically, which uses a hexagonal grid, but most of my code would readily adapt to other variants.

## Regexes as DFAs  [_🔗︎_](#regexes-as-dfas)
...
My immediate first thought was to rely on the fact that a regular expression describes a [regular language](https://en.wikipedia.org/wiki/Regular_language)[1](#fn:1), and thus can be recognized using a deterministic finite automaton. Thus, I can encode a regular expression into a DFA, and then express regex matching in Z3 in terms of its transition function[2](#fn:2).
...
## Turning a regular expression into a DFA  [_🔗︎_](#turning-a-regular-expression-into-a-dfa)

With a bit of searching, I discovered [qntm](https://qntm.org/)’s [greenery library](https://github.com/qntm/greenery), which was pleasingly exactly what I was looking for – a relatively straightforward Python library for manipulating regular expressions, including transforming back and forth between regex syntax and DFAs (which it calls [FSMs](https://github.com/qntm/greenery?tab=readme-ov-file#fsm)). It will build an FSM for us, and in general it is pleasingly straightforward, optimized for simplicity, ease-of-use, and experimentation instead of raw performance, which is just fine for us – we’re going to make Z3 do all the heavy lifting.
...
## Mapping into Z3  [_🔗︎_](#mapping-into-z3)
...
Z3 supports [function types](https://microsoft.github.io/z3guide/docs/logic/Uninterpreted-functions-and-constants), which can be declared and manipulated in expressions and assertions. Thus, I declared a function of the appropriate type, and informed Z3 of the transition table using an assertion for each entry in the transition table:
...
## Putting it together  [_🔗︎_](#putting-it-together)

With those primitives, we now just need to define a grid of characters, and add appropriate assertions. I was still using [regexle](https://regexle.com/) as a source of test cases; it operates on a hexagonal grid, and mapping coordinates between the hex grid and the three different clue axes was honestly one of the hardest parts of this exercise! Glossing over that bit, though, the code is quite simple:
...
# Solver performance  [_🔗︎_](#solver-performance)
...
## Pruning states  [_🔗︎_](#pruning-states)
...
## Defining the transition function explicitly  [_🔗︎_](#defining-the-transition-function-explicitly)
...
### Sharing the expression  [_🔗︎_](#sharing-the-expression)
...
#### Using a Z3 lambda expression  [_🔗︎_](#using-a-z3-lambda-expression)
...
#### Using a Z3 function and `macro-finder`  [_🔗︎_](#using-a-z3-function-and-macro-finder)

While exploring other options, however, I was reading [the SMT-LIB specification](https://smt-lib.org/papers/smt-lib-reference-v2.7-r2025-07-07.pdf), and noticed that SMT-LIB allows defining functions with explicit bodies:
...
![Screenshot of the SMT-LIB specification, defining &ldquo;define-fun&rdquo; as equivalent to declaring the named function using &ldquo;declare-fun&rdquo;, and asserting its behavior using a &ldquo;forall&rdquo; ranging over the input variables.](/post/regex-crosswords-z3/smt-define-fun.png "SMT-LIB specification v2.7, p66, defining the semantics of `define-fun`")

SMT-LIB specification v2.7, p66, defining the semantics of `define-fun`
...
## False starts  [_🔗︎_](#false-starts)
...
### Z3 `EnumSort`  [_🔗︎_](#z3-enumsort)

Z3 knows a **lot** about the integers, and has [many different solvers](https://z3prover.github.io/papers/programmingz3.html#sec-solving-arithmetical-fragments) for different classes of integer arithmetic. I wondered: Can we speed up the solver by using some other data type, and just avoid invoking any of that integer logic, at all?

Thus, I tried replacing my states and characters with [SMT-LIB enumeration types](https://microsoft.github.io/z3guide/docs/theories/Datatypes#scalars-enumeration-types) (exposed to Python as [z3.EnumSort](https://z3prover.github.io/api/html/namespacez3py.html#a9cf7ec94a9dd102286790774b5359556)).
...
#### `EnumSort` performance  [_🔗︎_](#enumsort-performance)

When I first switched to `EnumSort`, I saw a **massive** speedup!

![Z3 solve time, integer encoding vs EnumSort. EnumSort solves every side-3 puzzles in under a tenth of second, while the integer encoding takes at least 0.2s and as much as 3s](/post/regex-crosswords-z3/int-v-enum.png "An early violin plot I made of Z3 solve times for side-3 puzzles, using two different representations. These runs use the pruning described above, but still the original pointwise function definition.")

An early violin plot I made of Z3 solve times for side-3 puzzles, using two different representations. These runs use the pruning described above, but still the original pointwise function definition.
...
![The same plot, with two new panels for &ldquo;pointwise (new)&rdquo; and &ldquo;z3.Lambda&rdquo;.](/post/regex-crosswords-z3/int-v-enum-new.png "The same plot, with two new panels for different representations of the transition function.")

The same plot, with two new panels for different representations of the transition function.
...
#### A bizarre performance instability  [_🔗︎_](#a-bizarre-performance-instability)
...
## Using Z3 regular expressions  [_🔗︎_](#using-z3-regular-expressions)
...
Z3, it turns out, also has a [theory of regular expressions](https://microsoft.github.io/z3guide/docs/theories/Regular%20Expressions)! We can bypass our state-machine machinery entirely, and directly encode the regex clues as Z3 regular expressions.

Z3 does not come with a regular-expression parser, but instead has combinators to build up patterns, such as `z3.Re(char)`, `z3.Range(start, end)`, `z3.Star(re)`, and so on. Thus, I [wrote a simple translation layer](https://github.com/nelhage/nelhage-sandbox/blob/main/regexle/src/regexle/z3_re.py) which recursively walks the `greenery` `Pattern` AST, and translates it into a Z3 regex. From there, we can just declare each unknown character as a Z3 string object, and assert each clue like so:
...
![A line plot, with &ldquo;puzzle side length&rdquo; on the X axis, and &ldquo;solve time in seconds&rdquo; on the Y. There are three lines. My current-fastest solver goes from about 10ms at size=3 to about 30ms at size=8. Using Z3 regular expressions goes from about 150ms to about 11s. My original, slow, code has a lot of variance and takes over 10s by size=5, which is the last point shown.](/post/regex-crosswords-z3/perf-overview.png "Comparing performance, as a function of puzzle size, for three variants. \"Current best\" is my best-performing approach, using IntSort, pruning, and z3.Lambda. \"Pointwise, no pruning\" is similar to my very first implementation.")

Comparing performance, as a function of puzzle size, for three variants. "Current best" is my best-performing approach, using IntSort, pruning, and z3.Lambda. "Pointwise, no pruning" is similar to my very first implementation.
...
#### Z3 supports many more features than I’d realized.  [_🔗︎_](#z3-supports-many-more-features-than-id-realized)
...
I also had never encountered the Z3 [tactics](https://microsoft.github.io/z3guide/docs/strategies/tactics) system, which can be used both to help solve problems which are beyond the reach of the base SMT solver, and also to implement custom rewrite/simplification strategies in order to optimize for a certain class of problems. I learned that [Alive2](https://github.com/AliveToolkit/alive2), my personal favorite Z3-backed tool, makes [heavy use of Z3 tactics](https://z3prover.github.io/papers/z3internals.html#sec-a-use-case-of-tactics-from-alive2) to optimize for the specific kinds of expressions that Alive2 generates.

#### Z3 performance is, indeed, quite unstable and unpredictable, at least at times.  [_🔗︎_](#z3-performance-is-indeed-quite-unstable-and-unpredictable-at-least-at-times)
...
One caveat, though, re: usability: new sorts are global per-z3-[Context](https://z3prover.github.io/api/html/classz3py_1_1_context.html), so if you (e.g.) declare an `EnumSort` named “State,” you can’t re-declare a different one with the same name and a different set of values, without restarting your program or creating a whole new context object.

#### Z3 documentation is mixed, but does exist  [_🔗︎_](#z3-documentation-is-mixed-but-does-exist)

I found it pretty hard, at first, to find good documentation that answered deep questions about Z3 internals or strategies or information beyond surface usage, but eventually I collected quite a list. I think that “ask an expert” remains far and away the most effective way to use Z3 (thanks to [Hillel Wayne](https://www.hillelwayne.com/) for answering some questions as I worked on this project!), but here are the some other excellent links I found:
...
1.  I include here the obligatory disclaimer that many modern “regular expression” libraries support non-regular features like backreferences. My strategy won’t work for regex crosswords that use those, but I choose to be okay with that. [↩︎](#fnref:1)
    
2.  Some readers may be thinking “But wait! Doesn’t Z3 natively support regular expressions??” When I started this project I wasn’t aware of that feature, but [I tried them out, eventually, too](#using-z3-regular-expressions)! [↩︎](#fnref:2)
    

Subscribe to my newsletter (blog updates and occasional other content):

  

[Powered by Buttondown.](https://buttondown.email)

[« The ITTAGE indirect branch predictor](/post/ittage-branch-predictor/ "Previous Post: The ITTAGE indirect branch predictor")

* * *

[![Creative Commons License](https://i.creativecommons.org/l/by/4.0/88x31.png)](http://creativecommons.org/licenses/by/4.0/)  
Made of Bugs by [Nelson Elhage](https://nelhage.com) is licensed under a [Creative Commons Attribution 4.0 International License](http://creativecommons.org/licenses/by/4.0/).

![](https://sa.nelhage.com/noscript.gif)