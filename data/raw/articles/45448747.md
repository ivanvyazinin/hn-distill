Welcome — Magic-Wormhole 0.20.0+24.g0a212b2 documentation         
...
## Get Things[¶](#get-things "Link to this heading")

[Traditionally](https://xkcd.com/949/) transferring files is hard, even in 2025. The first and most popular “Thing” is a file or directory. For more on this, see the [File-Transfer Protocol](file-transfer-protocol.html)

Magic Wormhole can send any sort of message, though, including messages making up streams – for example, [Fowl](https://github.com/meejah/fowl) does exactly this. `fowl` is built on a feature called “Dilation”; see [The Dilation Protocol](dilation-protocol.html) for more detail.

## From One Computer to Another[¶](#from-one-computer-to-another "Link to this heading")
...
Initial and relatively small messages are sent via the “Mailbox” server. This lets the two peers contact a well-known resource, and send the initial PAKE messages. After establishing this shared key, the peers bootstrap via further messages over the Mailbox. See [Client-to-Client Protocol](client-protocol.html) for more details.
...
## Safely[¶](#safely "Link to this heading")

Magic Wormhole exists (at least partly) so more software can use a cryptographic construct called “Password Authenticated Key Exchange” (PAKE), currently using the [SPAKE2](https://datatracker.ietf.org/doc/rfc9383/) variant.
...
Early in the protocol a shared secret is established, after which all peer-to-peer traffic is encrypted with keys derived directly from the shared PAKE secret. The section [Known Vulnerabilities](attacks.html) has more details about failure modes.

# Motivational Use-Cases[¶](#motivational-use-cases "Link to this heading")
...
*   [Warp](https://gitlab.gnome.org/World/warp) is a Gnome GUI application to transfer files and directories (works with the CLI);
    
*   [Wormhole](https://gitlab.com/lukas-heiligenbrunner/wormhole): phone application for Android;
    
*   [git withme](https://git.sr.ht/~meejah/git-withme) allows two peers to directly use Git together (without any hosting service like GitLab or [gitolite](https://gitolite.com/gitolite/));
    
*   [Pear On](https://git.sr.ht/~meejah/pear-on) allows two peers to use [tty-share;](https://github.com/elisescu/tty-share) directly, without running or using a centralized server;
    
*   See [The Magic-Wormhole Ecosystem](ecosystem.html) for more implementations and applications
    

# Detailed Overview of Magic Wormhole[¶](#detailed-overview-of-magic-wormhole "Link to this heading")
...
As of now (2023) the magic-wormhole protocol has several client implementations; see [The Magic-Wormhole Ecosystem](ecosystem.html)

Code: [github.com/magic-wormhole/magic-wormhole](https://github.com/magic-wormhole/magic-wormhole) Documentation: [magic-wormhole.readthedocs.io](https://magic-wormhole.readthedocs.io/en/latest/)
...
### MacOS / OS-X[¶](#macos-os-x "Link to this heading")

[Install Homebrew](https://brew.sh/), then run `brew install magic-wormhole`.

### Linux (Debian/Ubuntu)[¶](#linux-debian-ubuntu "Link to this heading")

Magic-wormhole is available with `apt` in Debian 9 “stretch”, Ubuntu 17.04 “zesty”, and later versions:

$ sudo apt install magic-wormhole

### Linux (Fedora)[¶](#linux-fedora "Link to this heading")

Note: magic-wormhole [was removed from Fedora](https://bugzilla.redhat.com/show_bug.cgi?id=2073777) starting in Fedora 37. So this command will only work on Fedora 36 and earlier.

$ sudo dnf install magic-wormhole

### Linux (openSUSE)[¶](#linux-opensuse "Link to this heading")

$ sudo zypper install python-magic-wormhole

### Linux (Snap package)[¶](#linux-snap-package "Link to this heading")

Many linux distributions (including Ubuntu) can install [“Snap” packages](https://snapcraft.io/). Magic-wormhole is available through a third-party package (published by the “snapcrafters” group):

$ sudo snap install wormhole

### Windows[¶](#windows "Link to this heading")

#### Chocolatey[¶](#chocolatey "Link to this heading")

$ choco install magic-wormhole

The binaries for Windows are provided from this project: [https://github.com/aquacash5/magic-wormhole-exe](https://github.com/aquacash5/magic-wormhole-exe)
...
You probably _don’t_ want to use `sudo` when you run `pip`. This tends to create [conflicts](https://github.com/magic-wormhole/magic-wormhole/issues/336) with the system python libraries.
...
The `wormhole` tool uses PAKE “Password-Authenticated Key Exchange”, a family of cryptographic algorithms that uses a short low-entropy password to establish a strong high-entropy shared key. This key can then be used to encrypt data. `wormhole` uses the SPAKE2 algorithm, due to Abdalla and Pointcheval[1](https://www.di.ens.fr/~pointche/Documents/Papers/2005_rsa.pdf).
...
## Timing[¶](#timing "Link to this heading")
...
## CLI tool[¶](#cli-tool "Link to this heading")
...
If you desire shell tab-completion on sub-commands, we include generated files [from Click](https://click.palletsprojects.com/en/8.1.x/shell-completion/) for Bash, Zsh and Fish shells in [wormhole\_completion.bash](https://github.com/magic-wormhole/magic-wormhole/blob/master/wormhole_complete.bash) (or `.zsh`, `.fish`). Put this file in your favourite location and add a line like `source ~/wormhole_completion.bash` to `~/.bashrc` (or similar for `zsh` and `fish` shells).

## Library[¶](#library "Link to this heading")

The `wormhole` module makes it possible for other applications to use these code-protected channels. This includes Twisted support, and (in the future) will include blocking/synchronous support too. See [the API docs](api.html) for details.
...
*   Bugs and patches at the [GitHub project page](https://github.com/magic-wormhole/magic-wormhole).
...
To set up Magic Wormhole for development, you will first need to install [virtualenv](https://docs.python.org/3/tutorial/venv.html).
...
trial wormhole.test.test\_cli.PregeneratedCode.test\_file\_tor
...
pkg\_resources.DistributionNotFound: The 'magic-wormhole==0.9.1-268.g66e0d86.dirty' distribution was not found and is required by the application
...
## License, Compatibility[¶](#license-compatibility "Link to this heading")

This library is released under the MIT license, see LICENSE for details.

This library is compatible with Python 3.10, 3.11, 3.12, 3.13.

[![Logo](_static/wormhole.png)](index.html)

# [Magic-Wormhole](index.html)
...
*   [Documentation overview](index.html)
    *   Previous: [Magic-Wormhole: Get Things From One Computer To Another, Safely](index.html "previous chapter")
    *   Next: [The Magic-Wormhole Ecosystem](ecosystem.html "next chapter")
...
*   [Detailed Overview of Magic Wormhole](#detailed-overview-of-magic-wormhole)
    *   [Example](#example)
...
*   [The Magic-Wormhole Ecosystem](ecosystem.html)
*   [Tor Support in Magic-Wormhole](tor.html)
*   [Protocol/API/Library Introduction](introduction.html)
*   [The Magic-Wormhole API](api.html)
*   [Transit Protocol](transit.html)
*   [Mailbox Server Protocol](server-protocol.html)
*   [Client-to-Client Protocol](client-protocol.html)
*   [File-Transfer Protocol](file-transfer-protocol.html)
*   [Known Vulnerabilities](attacks.html)
*   [Journaled Mode](journal.html)
*   [The Dilation Protocol](dilation-protocol.html)
*   [Contributing to Magic Wormhole](CONTRIBUTING.html)

©2017, Brian Warner. | Powered by [Sphinx 8.2.3](https://www.sphinx-doc.org/) & [Alabaster 1.0.0](https://alabaster.readthedocs.io) | [Page source](_sources/welcome.rst.txt)