...
[hits](https://tinylytics.app/public/thT44b6cyEs19PvjwWsT)

## Table of Contents

Minimize Table of Contents Maximize Table of Contents

1.  [Why implement grid-awareness?](#why-implement-grid-awareness)
2.  [The breakdown](#the-breakdown)
    *   [Step 1: Determine what grid-awareness looks like for you](#step-1-determine-what-grid-awareness-looks-like-for-you)
    *   [Step 2: Determine how you’re going to make it happen](#step-2-determine-how-youre-going-to-make-it-happen)
        *   [11ty](#11ty)
        *   [Astro](#astro)
    *   [Step 3: Finetune and deploy](#step-3-finetune-and-deploy)
3.  [Is this another thing I have to care about?](#is-this-another-thing-i-have-to-care-about)
...
Over the past year, I’ve had the incredible privilege of getting to participate in the Grid-Aware Websites (affectionately abbreviated to GAW) [advisory group](https://www.thegreenwebfoundation.org/news/introducing-our-grid-aware-websites-project/#technical-advisory-group). The Green Web Foundation team have thoroughly explained what a grid-aware website is in this detailed case study of the [Branch Magazine redesign](https://branch.climateaction.tech/issues/issue-9/designing-a-grid-aware-branch/), but if I had to put it in my own words, a grid-aware website responds to the cleanliness or dirtiness of a user’s electricity grid. In simpler terms, whether it is currently using more renewable energy sources or fossil fuels.

To do so in the most performant way possible, this currently involves getting a user’s rough location and using the [Electricity Maps API](https://portal.electricitymaps.com/developer-hub/api/getting-started#geolocation) to check the current status of the grid, and then returning that information to a serverless function on the edge so that the website can be modified, all before it reaches the user’s browser.
...
Following the tutorial provided by the Green Web Foundation, I deployed the site to Cloudflare, then installed the [specific grid-aware worker](https://github.com/thegreenwebfoundation/gaw-plugin-cloudflare-workers) they made (there’s also plans to build out a [Netlify](https://github.com/thegreenwebfoundation/gaw-plugin-netlify-edge) equivalent). Out of the box, the plugin provides a drop-in connection to the Electricity Maps API once you plug in your key, which is very helpful. This worker will get the request, get the user’s rough location, and then determine the current state of the grid.

![The Green Web Foundation's grid-aware statusbar component features a location marker with the current Location (Hong Kong), text that says 'Your local grid: Around average emissions', and a switch toggle to enable the grid-aware auto mode.](https://res.cloudinary.com/nicchan/image/upload/w_1524,h_115,c_lfill,f_auto/v1755821377/toolbar-cropped)

It can also optionally inject the [grid-aware status bar](https://www.thegreenwebfoundation.org/news/a-web-component-for-grid-aware-websites/) web component, which will show some data about the user’s current grid and allow the user to manually toggle between the modes. The status bar is optional, you can also build your own custom solution to give the user control over their grid-aware experience.
...
Below is a comparison of what the site looks like when the grid is ‘dirty’ vs ‘clean’

![The demo product page when the grid is using average/high levels of fossil fuels. The marquee is now a static text section, the product slider has a horizontal scrollbar, and the product tabs are headings and paragraphs.](https://res.cloudinary.com/nicchan/image/upload/w_1560,h_1872,c_lfill,f_auto/v1755822151/grid-aware-before)

'Dirty' grid

![The demo product page when the grid is using low of fossil fuels, and all functionality is enabled. There is a scrolling marquee, the images are in a slider, and the product information is in tabs.](https://res.cloudinary.com/nicchan/image/upload/w_1560,h_1872,c_lfill,f_auto/v1755822151/grid-aware-after)
...
Scroll windows list backwards Scroll windows list forwards
