Your MPSC/SPMC/MPMC queue is not a queue | Alex Saveau

[![Profile Picture](/assets/resized/me2-240.jpg)](/)
...
[@SUPERCILEX](//github.com/SUPERCILEX)

[saveau.alexandre@gmail.com](mailto:saveau.alexandre@gmail.com)

[Stack Overflow](//stackoverflow.com/u/4548500)

[Blog](/blog)

# Your MPSC/SPMC/MPMC queue is not a queue

## Rethinking our approach to lockless channels

Published Aug 16, 2025 • Last updated Sep 25, 2025 • 10 min read

Lockless queues let multiple cores communicate with each other without mutexes, typically to move work around for parallel processing. They come in four variants: `{single,multi}`\-producer `{single,multi}`\-consumer. A producer gives data to a consumer, each of which can be limited to a single thread (i.e. a single-`{producer,consumer}`) or shared across **multi**ple threads. But only the single-producer single-consumer (SPSC) queue is actually a queue!

> This article is part of [a series](/blog/tags/lockness) building [Lockness](//github.com/SUPERCILEX/lockness/blob/master/README.md), a high-performance blocking task executor.

## SPMC/MPMC queues are broken [#](#spmcmpmc-queues-are-broken)
...
## MPSC queues are special [#](#mpsc-queues-are-special)
...
> Note: I’ve left SPSC queues out of this discussion because they are real queues with a generally agreed upon optimal implementation: power-of-2 queue capacity backed by duplicated mmaps with cached head/tail pointers expressed in terms of elements written/read, and optional [get\_robust\_list](//man7.org/linux/man-pages/man2/get_robust_list.2.html) support to handle multiprocess shared memory dead counterparty notification.

## Lockless queues are slow [#](#lockless-queues-are-slow)
...
[![Diagram of MPMC channel contention.](/assets/resized/lockness/mpmc-blocking-min.svg)](/assets/resized/lockness/mpmc-blocking-min.svg)
...
## Lockless algorithm fundamentals [#](#lockless-algorithm-fundamentals)
...
2.  The thread uses the memory (non-atomically). This stage should be as fast as possible and is typically just a `memcpy`.
...
## Lockless bags as a new approach [#](#lockless-bags-as-a-new-approach)
...
Instead of a queue, let’s use a bag! What’s a bag? Well, uhhhh… It’s a bag. You can put stuff in, rummage around, and take stuff out. Notice that I said nothing about _what_ you get out—if it’s in the bag, it’s a valid item to be taken out at any time (i.e. in an unspecified order).
...
[![Diagram of a bag-based MPMC channel.](/assets/resized/lockness/mpmc-bag-min.svg)](/assets/resized/lockness/mpmc-bag-min.svg)
...
## You don’t need unbounded channels [#](#you-dont-need-unbounded-channels)
...
### But here’s an idea to support unbounded lockless bags [#](#but-heres-an-idea-to-support-unbounded-lockless-bags)
...
## Introducing Lockness Bags [#](#introducing-lockness-bags)

[Lock**n**ess Bags](//github.com/SUPERCILEX/lockness/blob/master/bags/README.md) implement the ideas described above and might be used to build the [Lockness Executor](//github.com/SUPERCILEX/lockness/blob/master/executor/README.md) should benchmarking show an advantage.

### Are Lockness bags fast? [#](#are-lockness-bags-fast)

No :(

Queue-based channel implementations win over bags on current hardware by disjointing their producer and consumer memory writes. Remember the `consumed` and `committed` pointers from [the diagram](#mpmc-diagram)? In practice, these are implemented in a distributed fashion: each slot has a flag that marks it as readable or writable. To start, all slots are writable and transition to readable and back as you write and read the slots. The head pointer can only advance if its current slot is readable and conversely for the tail pointer. Crucially, this means consuming a slot almost never touches a cache line producers are actively working with.

On the other hand, lockless bags are implemented with two atomics each representing a bitvector. To produce and consume a value, you must always update both bitvectors. This means producers _and_ consumers are all contending over the same two cache lines. On the other hand, lockless queues limit contention for producers to the tail cache line and similarly consumers only contend on the head cache line.

## The future: hardware accelerating lockless bags [#](#the-future-hardware-accelerating-lockless-bags)

The [next article](atomic-bit-fill) in the series explores the idea of novel instructions that would hardware accelerate lockless bags to significantly outperform all possible software channel implementations.

## Appendix: alternative approaches and their pitfalls [#](#appendix-alternative-approaches-and-their-pitfalls)
...
### Why doesn’t a stack work for MPSC channels? [#](#why-doesnt-a-stack-work-for-mpsc-channels)
...
### Why not use many SPSC queues to make a MPSC channel? [#](#why-not-use-many-spsc-queues-to-make-a-mpsc-channel)
...
### Why are tunnel channels bad? [#](#why-are-tunnel-channels-bad)
...
### Why not store machine-word sized elements in the channel? [#](#why-not-store-machine-word-sized-elements-in-the-channel)
...
Copyright © 2018-2025 Alex Saveau [Source](//github.com/SUPERCILEX/personal-website) | [Twitter](//twitter.com/SUPERCILEX) | [Back to top](#top)