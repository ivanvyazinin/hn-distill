Machine Scheduler in LLVM - Part II | Min Hsu's Homepage 

[Home](/) [Publications](/publications/) [Blog](/blog/) [GitHub](https://github.com/mshockwave) [LinkedIn](https://www.linkedin.com/in/bekketmcclane/) [Mastodon](https://mastodon.social/@mshockwave)
...
![](/images/llvm-misched2-reg-pressure.light.svg)
...
![](/images/llvm-misched2-reg-pressure2.light.svg)
...
The quantity of (potential) register pressure change on a specific pressure set

`PressureDiff`

An array of `PressureChange` , indexed by _pressure set ID_, made by a single instruction

`PressureDiffs`

A data structure that maps from an _instruction_ to its `PressureDiff`

`RegPressureDelta`

This specialized data structure only carries the three metrics Machine Scheduler cares about. Namely, _excess pressure_, _critical max pressure_, and _current max pressure_

`RegisterPressure`

Despite its name, this data structure only carries the maximal register pressure of each pressure set[1](#fn:1) in the current region
...
![](/images/llvm-misched2-resource-pressure1.light.svg)

And prefer more uniform _distribution_ among all the available pipes:

![](/images/llvm-misched2-resource-pressure2.light.svg)
...
```c++
1def PipeA : ProcResource<1>;
2def PipeB : ProcResource<4>;
3def PipeC : ProcResource<2>;
```
...
```c++
1def PipeB : ProcResource<4>;
```

can be thought as having multiple identical execution pipes:

```c++
1def PipeB_0 : ProcResource<1>;
2def PipeB_1 : ProcResource<1>;
3def PipeB_2 : ProcResource<1>;
4def PipeB_3 : ProcResource<1>;
```
...
```c++
1def FooSchedMachineModel : SchedMachineModel {
2    let IssueWidth = 3;
3}
4
5let SchedModel = FooSchedMachineModel in {
6    def PipeA : ProcResource<1>;
7    def PipeB : ProcResource<4>;
8    def PipeC : ProcResource<2>;
9}
```
...
The critical path in this case would be domindated by the load instruction, `ld a0, 0(s0)`, therefore, the **nominal** critical path length is equal to 3 cycles[2](#fn:2). But remember, we have to scale this number with the latency factor, so the **normalized** critical path length is `12 * 3 = 36` cycles.
...
![](/images/llvm-misched2-resource-pressure-timeline1.light.svg)
...
![](/images/llvm-misched2-resource-pressure-timeline2.light.svg)
...
Feel free to leave comments at [https://github.com/mshockwave/portfolio/discussions/14](https://github.com/mshockwave/portfolio/discussions/14)

* * *

1.  As well as the live-in and live-out registers in this region. [↩︎](#fnref:1)
    
2.  In a top-down schedule, the critical path length is effectively defined as the number of cycles up to the point where the last instruction was _issued_, so the latency of `add a1, a0, a0` is not accounted for. [↩︎](#fnref:2)
    

[#llvm](/blog/llvm/)   [#compiler-instruction-scheduling](/blog/compiler-instruction-scheduling/)  

© 2024 Min-Yih Hsu | Made with [Hugo ʕ•ᴥ•ʔ Simple](https://github.com/maolonglong/hugo-simple/) | [RSS](/index.xml)