...
![Cute Kiwi doing cute things](/assets/kiwis/book.svg)[
...
[](https://www.youtube.com/@FrostKiwi/)[](https://github.com/FrostKiwi)[](/feed.xml)
...
> 
> *   [Video Game Blurs (and how the best one works)](#)
>     *   [Setup - No blur yet](#setup---no-blur-yet)
>         *   [Technical breakdown](#technical-breakdown)
>     *   [Box Blur](#box-blur)
>     *   [What even is a kernel?](#what-even-is-a-kernel%3F)
>     *   [Gaussian Blur](#gaussian-blur)
>         *   [What is Gaussian-like?](#what-is-gaussian-like%3F)
>     *   [Performance](#performance)
>         *   [What are we optimizing for?](#what-are-we-optimizing-for%3F)
>     *   [Separable Gaussian Blur](#separable-gaussian-blur)
>     *   [The magic of frequency space](#the-magic-of-frequency-space)
>         *   [What is a Low-Pass filter?](#what-is-a-low-pass-filter%3F)
>     *   [Bilinear Interpolation](#bilinear-interpolation)
>     *   [Downsampling](#downsampling)
>         *   [Smooth Blur Animation](#smooth-blur-animation)
>     *   [Kawase Blur](#kawase-blur)
>     *   [Dual Kawase Blur](#dual-kawase-blur)
>     *   [What are the big boys doing?](#what-are-the-big-boys-doing%3F)
> *   [Comments](#comments)
...
![Texture coordinates, also called UV Coordinates or UVs for short](img/intro.png)

Effect of "[Bloom](https://en.wikipedia.org/wiki/Bloom_\(shader_effect\))", one of many use-cases for blur algorithms

Conceptually, _“Make thing go blurry”_ is easy, boiling down to some form of _“average colors in radius”_. Doing so in [realtime](https://en.wikipedia.org/wiki/Real-time_computing) however, took many a graphics programmer through decades upon decades of research and experimentation, across computer science and maths. In this article, we’ll follow their footsteps.

> A graphics programming time travel, if you will.
> 
> ![](/assets/kiwis/cyber.svg)
...
> This is my submission to this year's [Summer of Math Exposition](https://some.3b1b.co/)
> 
> ![](img/SOMELogo.svg)
...
In the context of video game post-processing, a 3D scene is drawn, also called [rendering](https://en.wikipedia.org/wiki/Rendering_\(computer_graphics\)), and saved to an intermediary image - a [framebuffer](https://learnopengl.com/Advanced-OpenGL/Framebuffers). In turn, this framebuffer is processed to achieve [various effects](https://en.wikipedia.org/wiki/Video_post-processing#Uses_in_3D_rendering). Since this _processing_ happens _after_ a 3D scene is rendered, it’s called _post-processing_. All that, _many_ times a second.

> **Depending on technique**, framebuffers [can hold non-image data](https://learnopengl.com/Advanced-Lighting/Deferred-Shading) and post-processing effects like [Color-correction](https://en.wikipedia.org/wiki/Color_correction) or [Tone-mapping](https://en.wikipedia.org/wiki/Tone_mapping) don't even require intermediate framebuffers: There's [more](https://takahirox.github.io/three.js/examples/webgl_tonemapping.html) than [one way (@35:20)](https://gdcvault.com/play/1020631/The-Revolution-in-Mobile-Game)
> 
> ![](/assets/kiwis/detective.svg)
...
> No coding or graphics programming knowledge required to follow along. [But also no curtains!](https://github.com/FrostKiwi/treasurechest/tree/main/posts/dual-kawase) You can always see [how we talk](https://www.youtube.com/watch?v=ONH-pxBMJu4) with your GPU. Terms and meanings will be explained, once it's relevant.
> 
> ![](/assets/kiwis/speak.svg)
...
> Blur Fragment Shader [noBlurYet.fs](shader/noBlurYet.fs)
> 
> ```glsl
...
> ```
> 
> WebGL Javascript [simple.js](js/blur/simple.js)
> 
> ```javascript
...
> Adding the blurred emission [pass](https://chrismillervfx.wordpress.com/2013/04/15/understanding-render-passes/) as we do in this article, or [thresholding](https://en.wikipedia.org/wiki/Thresholding_\(image_processing\)) the scene and blurring that, is **not** actually how modern video games do bloom. We'll get into that a bit later.
> 
> ![](/assets/kiwis/detective.svg)

Finally, you see [Resolution](https://en.wikipedia.org/wiki/Image_resolution) of the canvas and [Frames per Second / time taken per frame, aka “frametime”](https://en.wikipedia.org/wiki/Frame_rate). A very important piece of the puzzle is _**performance**_, which will become more and more important as the article continues and the [mother of invention](https://en.wikipedia.org/wiki/Necessity_is_the_mother_of_invention) behind our story.

> Frame-rate will be capped at your screen's [refresh rate](https://www.intel.com/content/www/us/en/gaming/resources/highest-refresh-rate-gaming.html), most likely 60 fps / 16.6 ms. We'll get into proper [benchmarking](https://en.wikipedia.org/wiki/Benchmark_\(computing\)) as our hero descents this article into blurry madness
> 
> ![](/assets/kiwis/book.svg)

### Technical breakdown [#](#technical-breakdown)

> Understanding the GPU code is not necessary to follow this article, but if you do choose to [peek behind the curtain](https://github.com/FrostKiwi/treasurechest/tree/main/posts/dual-kawase), here is what you need to know
> 
> ![](/assets/kiwis/teach.svg)
...
> Technically, fragment shaders run per [fragment](https://www.khronos.org/opengl/wiki/Fragment), which aren't necessarily pixel sized and there are [other ways](https://registry.khronos.org/OpenGL/extensions/EXT/EXT_shader_framebuffer_fetch.txt) to read framebuffers, but none of that matters in the context of this article.
> 
> ![](/assets/kiwis/detective.svg)

![Texture coordinates, also called UV Coordinates or UVs for short](img/UV.svg)
...
> Although [there are ways to find out](https://michaldrobot.com/2014/04/01/gcn-execution-patterns-in-full-screen-passes/), we don't know which order output-pixels are processed in, and although the [graphics pipeline can tell us](https://docs.gl/sl4/gl_FragCoord), the shader doesn't even know which output-pixel it currently processes
> 
> ![](/assets/kiwis/book.svg)
...
> ・**GPU info**:
> 
> Blur Fragment Shader [boxBlur.fs](shader/boxBlur.fs)
> 
> ```glsl
...
> ```
> 
> WebGL Javascript [boxBlur.js](js/blur/boxBlur.js)
> 
> ```javascript
> import * as util from '../utility.js'
> 
...
> 
> ![](/assets/kiwis/facepalm.svg)

Then, there’s this `samplePosMultiplier`. It seems to _also_ seemingly increase blur strength, _without_ increasing textureTaps or lowering performance (or lowering performance just a little on certain devices). But if we crank _that_ too much, we get artifacts in the form of repeating patterns. Let’s play with a schematic example:
...
*   the black dots are our _actual_ texture reads _**per-output-pixel**_, our “sample” positions
...
On can say, that an image is a “continous 2D signal”. When we texture tap at a specific coordinate, we are _sampling the “image signal” at that coordinate_. As previously mentioned, we use UV coordinates and are not bound by concepts like “pixels position”. _**Where**_ we place our samples is completely up to us.
...
Doing it too much, brings ugly repeating patterns. This of course leaves some fundamental questions, like where these artifacts come from and what it even means to read between two pixels. _**And**_ on top of that we have to address performance and the boxyness of our blur! But first…

## What even _is_ a kernel? [#](#what-even-is-a-kernel%3F)
...
![Texture Wrapping Modes and results on blurring](img/WrappingModes.png)
...
Convolution as a mathematical concept is surprisingly deep and [3blue1brown](https://www.youtube.com/@3blue1brown) has an excellent video on it, that even covers the image processing topic. Theoretically, we won’t depart from convolutions. We _**can**_ dissect our code and express it as weights and kernels. With the for-loop box blur, that _**was**_ quite easy!

But what is a convolution?  
[YouTube Video](https://www.youtube.com/watch?v=KuXjwB4LzSA) by [3Blue1Brown](https://www.youtube.com/@3blue1brown)
...
Gaussian blur weights formula for point `(x,y)` [(Source)](https://en.wikipedia.org/wiki/Gaussian_blur#Mathematics)

To calculate the weights for point `(x,y)`, the [above formula is used](https://en.wikipedia.org/wiki/Gaussian_blur#Mathematics). The gaussian formula has a weighting multiplier `1/√(2πσ²)`. In the code, there is no such thing though. The formula expresses the gaussian curve as a _continuous_ function going to _infinity_. But our code and its for-loop are different - _**discrete**_ and _**finite**_.
...
> ・**GPU info**:
> 
> Blur Fragment Shader [gaussianBlur.fs](shader/gaussianBlur.fs)
> 
> ```glsl
> /* Float precision to highp, if supported. Large Kernel Sizes result many color
>    contributions and thus require the highest precision to avoid clipping.
...
> ```
> 
> WebGL Javascript [gaussianBlur.js](js/blur/gaussianBlur.js)
> 
> ```javascript
> import * as util from '../utility.js'
> 
...
Binomial Kernels are also Gaussian-like in their frequency response. We won’t expand on these further, just know that we _**can**_ choose kernels by different mathematical criteria, chasing different signal response characteristics. But speaking of which, what even _is_ Gaussian Like? Why do we care?
...
[YouTube Video](https://www.youtube.com/watch?v=vNG3ZAd8wCc) by [Computerphile](https://www.youtube.com/@Computerphile)
...
> Across all platforms a stall is guaranteed to occur on command [`gl.readPixels()`](https://developer.mozilla.org/en-US/docs/Web/API/WebGLRenderingContext/readPixels). Interestingly, the standards conform command for this: [`gl.finish()`](https://developer.mozilla.org/en-US/docs/Web/API/WebGLRenderingContext/finish) is simply ignored by mobile apple devices.
> 
> ![](/assets/kiwis/book.svg)

Below is a button, that unlocks this benchmarking feature, unhiding a benchmark button and `Detailed Benchmark Results` section under each blur. This allows you to start a benchmark with a preset workload, on a separate [Browser Worker](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers). There is only one issue: Browsers get _**very**_ angry if you full-load the GPU this way.
...
These are key numbers graphics programmers chase when writing fragment shaders: **Texture Taps** and **Cache Utilization**. There is another one, we will get into in a moment. Clearly, our Blurs are _**slow**_. Time for a speed up!

## Separable Gaussian Blur [#](#separable-gaussian-blur)

We have not yet left the _classics_ of blur algorithms. One fundamental concept left on the table is “convolution separability”. Certain Convolutions like our [Box Blur](#box-blur), [our Gaussian Blur](#gaussian-blur) and the [Binominal filtering](https://bartwronski.com/2021/10/31/practical-gaussian-filter-binomial-filter-and-small-sigma-gaussians/) mentioned in passing previously can all be performed in two separate passes, by two _**separate**_ 1D Kernels.
...
[YouTube Video](https://www.youtube.com/watch?v=SiJpkucGa1o) by [Computerphile](https://www.youtube.com/@Computerphile)
...
> ・**GPU info**:
> 
> Blur Fragment Shader [gaussianBlurSeparable.fs](shader/gaussianBlurSeparable.fs)
> 
> ```glsl
> /* Float precision to highp, if supported. Large Kernel Sizes result many color
>    contributions and thus require the highest precision to avoid clipping.
...
> ```
> 
> WebGL Javascript [gaussianSeparableBlur.js](js/blur/gaussianSeparableBlur.js)
> 
> ```javascript
> import * as util from '../utility.js'
> 
...
If you benchmark the performance, you will see a massive performance uplift, as compared to our Gaussian Blur! But there _is_ a trade-off made, that’s not quite obvious. In order to have two passes, we are writing out a new framebuffer. Remember the “modern chips are fast but memory access in relation is not” thing?
...
…how about blurs that happen so fast, that they are considered free! We are doing a bit of a detour into _**Frequency Space**_ image manipulation.
...
![FFT Viz Input image](img/256ScreenOverlay.png)
...
For now, play around with removing energy. You can paint on the magnitude image with your fingers or with the mouse. The output image will be reconstructed accordingly. Also, play around with the circular mask and the feathering sliders. Try to build intuition for what’s happening.
 ...
 RemoveFrequency Energy  AddFrequency Energy
...
Repeating patterns in the image lighten up as bright-points in the magnitude representation. Or rather, their frequencies have high energy: E.g. the green grid I added. Removing it in photoshop wouldn’t be easy! But in frequency space it _**is**_ easy! Just paint over the blueish 3 diagonal streaks.

> Removing repeating features by finger-painting black over frequencies still blows me away.
> 
> ![](/assets/kiwis/surprised.svg)
...
[YouTube Video](https://www.youtube.com/watch?v=spUNpyF58BY) by [3Blue1Brown](https://www.youtube.com/@3blue1brown)
...
Then there are the artifacts I have glossed over. The FFT transformation considers the image as an infinite 2D signal. By blurring, we are bleeding through color from the neighbor copies. And that’s not to mention various ringing artifacts that happen. _**None**_ of this is unsolvable! But there a more underlying issue…
...
Try the FFT Example again and decrease the `frequencyCutRadius` to blur. At some point the green lines disappear, right? It _**is**_ a low pass filter, one where high frequencies are literally annihilated. Small bright lights in the distance? Also annihilated…
...
[YouTube Video](https://www.youtube.com/watch?v=Q2aEzeMDHMA) by [Computerphile](https://www.youtube.com/@Computerphile)
...
Bilinear interpolation allows us to resize an image by reading from it at lower resolution. In a way, it’s a free bilinear resize built into every graphics chip, zero performance impact. But there _is_ a limit - the bilinear interpolation is limited to a 2 × 2 sample square. Try to resize the kiwi below in different modes.

> To make this more obvious, the following canvas renders at 25% of [native resolution](https://en.wikipedia.org/wiki/1:1_pixel_mapping).
> 
> ![](/assets/kiwis/teach.svg)
...
> WebGL Vertex Shader [circleAnimationSize.vs](shader/circleAnimationSize.vs)
> 
> ```glsl
...
> ```
> 
> WebGL Fragment Shader [simpleTexture.fs](shader/simpleTexture.fs)
> 
> ```glsl
> precision highp float;
> varying vec2 uv;
...
> ```
> 
> WebGL Javascript [bilinear.js](js/bilinear.js)
> 
> ```javascript
> import * as util from './utility.js'
> 
...
![Downsample and Blur Framebuffers](img/framebuffer.svg)
...
❌ The browser killed this WebGL Context, please reload the page. If this happened as the result of a long benchmark, decrease the iteration count. On some platforms (iOS / iPad) you may have to restart the browser App completely, as the browser will temporarily refuse to allow this site to run WebGL again.
 ...
 Normal  Skip Down-steps  Skip Up-steps
...
> ・**GPU info**:
> 
> Blur Fragment Shader [gaussianSeparableBlur.fs](shader/gaussianSeparableBlur.fs)
> 
> ```glsl
> /* Float precision to highp, if supported. Large Kernel Sizes result many color
>    contributions and thus require the highest precision to avoid clipping.
...
> ```
> 
> WebGL Javascript [downsample.js](js/blur/downsample.js)
> 
> ```javascript
> import * as util from '../utility.js'
> 
...
With smaller blurs you will get “shimmering”, as aliasing artifacts begin, even with our bilinear filtering in place. Small blurs and lower resolution don’t mix. This is _**especially**_ painful in bloom mode with strong `lightBrightness`, as lights will start to “turn on and off” as they are not resolved correctly at lower resolutions.
...
![Page 159 from presentation Next Generation Post Processing in Call of Duty: Advanced Warfare by Jorge Jimenez](img/jimenez2014.png)

Page 159 from presentation [Next Generation Post Processing in Call of Duty: Advanced Warfare](https://www.iryoku.com/next-generation-post-processing-in-call-of-duty-advanced-warfare/) by [Jorge Jimenez](https://www.iryoku.com/)
...
![Nearest Neightbor Interpolation](img/Interpolation-nearest.svg) ![Bilinear Neightbor Interpolation](img/Interpolation-bilinear.svg)

Visualization of the bilinear interpolation ([Source](https://en.wikipedia.org/wiki/Bicubic_interpolation))

How to keeps things smooth when upsampling is the field of “Reconstruction filters”. By skipping intermediate upsampling steps we are performing a 2 × 2 sample bilinear reconstruction of very small pixels. As a result we get the bilinear filtering characteristic pyramid shaped hot spots. _**How**_ we upscale matters.
...
![](img/blurSliderDemo1.png) ![](img/blurSliderDemo2.png)
...
![kawase blur filter pattern](img/kawase1.png) ![kawase blur filter pattern](img/kawase2.png)

Sample placement in what later become known as the "Kawase Blur"  
Excerpt from GDC presentation [Frame Buffer Postprocessing Effects in DOUBLE-S.T.E.A.L](https://web.archive.org/web/20060909063116/http://www.daionet.gr.jp/~masa/archives/GDC2003_DSTEAL.ppt) (2003)
...
> ・**GPU info**:
> 
> WebGL Fragment Shader [kawase.fs](shader/kawase.fs)
> 
> ```glsl
...
> ```
> 
> WebGL Javascript [kawase.js](js/blur/kawase.js)
> 
> ```javascript
> import * as util from '../utility.js'
> 
...
![Dual Kawase sampling patterns](img/dualKawasePattern.jpg)

Dual Kawase sampling patterns  
Excerpt from [Bandwidth-Efficient Rendering](https://www.iryoku.com/next-generation-post-processing-in-call-of-duty-advanced-warfare/), talk by [Marius Bjørge](https://theorg.com/org/arm/org-chart/marius-bjorge)
...
> ・**GPU info**:
> 
> WebGL Fragment Shader [dual-kawase-down.fs](shader/dual-kawase-down.fs)
> 
> ```glsl
> /* Float precision to highp, if supported. Large iterations result many color
>    contributions and thus require the highest precision to avoid clipping.
...
> ```
> 
> WebGL Fragment Shader [dual-kawase-up.fs](shader/dual-kawase-up.fs)
> 
> ```glsl
> /* Float precision to highp, if supported. Large iterations result many color
>    contributions and thus require the highest precision to avoid clipping.
...
> ```
> 
> WebGL Javascript [dual-kawase.js](js/blur/dual-kawase.js)
> 
> ```javascript
> import * as util from '../utility.js'
> 
...
![KDE Plasma's Blur with noise at max strength](img/kdeBlur.png)

KDE Plasma's Blur with noise at max strength ([Source](https://web.archive.org/web/20220427124712/https://phabricator.kde.org/D9848))
...
[Next Generation Post Processing in Call of Duty: Advanced Warfare](https://www.iryoku.com/next-generation-post-processing-in-call-of-duty-advanced-warfare/) by [Jorge Jimenez](https://www.iryoku.com/)
...
Excerpt from talk [Low Complexity, High Fidelity - INSIDE Rendering](https://gdcvault.com/play/1023304/Low-Complexity-High-Fidelity-INSIDE) by Mikkel Gjoel & Mikkel Svendsen
...
![Bloom in Video game ](img/bloom-oblivion.jpg)

Bloom in Video game [The Elder Scrolls IV: Oblivion](https://en.wikipedia.org/wiki/The_Elder_Scrolls_IV:_Oblivion), from article by [Bloom Disasters](https://gangles.ca/2008/07/18/bloom-disasters/)
...
![Physically Based Blur](img/bloom.png)

Multiple blurs stacked to create a natural light fall-off  
Page 144 in [Next Generation Post Processing in Call of Duty: Advanced Warfare](https://www.iryoku.com/next-generation-post-processing-in-call-of-duty-advanced-warfare/) by [Jorge Jimenez](https://www.iryoku.com/)
...
[YouTube Video](https://www.youtube.com/watch?v=rJPKTCdk-WI) by [FrostKiwi](https://www.youtube.com/@FrostKiwi)

[More Articles](/) [Top](#)

* * *

## Comment via GitHub [#](#comments)
