...
You might have heard that Zig 0.15 introduces a new IO interface, with the focus for this release being the new std.Io.Reader and std.Io.Writer types. The old "interfaces" had problems. Like [this performance issue](https://github.com/ziglang/zig/issues/17985) that I opened. And it relied on a [mix of types](https://www.openmymind.net/In-Zig-Whats-a-Writer/), which always confused me, and a lot of `anytype` - which is generally great, but a poor foundation to build an interface on.
...
When I looked through Zig's source code, there's [only one place](https://github.com/ziglang/zig/blob/306176046e6ae5e30bc58e5f3bcf786159e367f2/lib/std/http/Client.zig#L329) using `tls.Client`. It helped to get me where where I am. I couldn't find any tests.
