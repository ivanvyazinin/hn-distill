...
[encapsulation](/tags/encapsulation.html)

[web development](/tags/web%20development.html)
...
1: There's an argument here that the name “page object” is misleading because it makes you think you should have just one page object per page. Something like “panel object” would be better - but the term “page object” is what's become accepted. Another illustration of why naming is one of the [TwoHardThings](/bliki/TwoHardThings.html).
...
There are differences of opinion on whether page objects should include assertions themselves, or just provide data for test scripts to do the assertions. Advocates of including assertions in page objects say that this helps avoid duplication of assertions in test scripts, makes it easier to provide better error messages, and supports a more [TellDontAsk](/bliki/TellDontAsk.html) style API. Advocates of assertion-free page objects say that including assertions mixes the responsibilities of providing access to page data with assertion logic, and leads to a bloated page object.
...
It's common to write tests using some form of [DomainSpecificLanguage](/bliki/DomainSpecificLanguage.html), such as Cucumber or an internal DSL. If you do this it's best to layer the testing DSL over the page objects so that you have a parser that translates DSL statements into calls on the page object.

If you have WebDriver APIs in your test methods, You're Doing It Wrong. -- [Simon Stewart](http://blog.rocketpoweredjetpants.com).

Patterns that aim to move logic out of UI elements (such as [Presentation Model](/eaaDev/PresentationModel.html), [Supervising Controller](/eaaDev/SupervisingPresenter.html), and [Passive View](/eaaDev/PassiveScreen.html)) make it less useful to test through the UI and thus reduce the need for page objects.
...
I first described this pattern under the name [Window Driver](/eaaDev/WindowDriver.html). However since then the term “page object” was popularized by the Selenium web testing framework and that's become the generally used name.

[Selenium's wiki](https://github.com/SeleniumHQ/selenium/wiki/PageObjects) strongly encourages using page objects and provides advice on how they should be used. It also favors assertion-free page objects.

A team measured the times to update two versions of a suite of selenium tests after a software upgrade. They found the version with page object took a bit longer for the first test case, but much faster for the rest. For more details see [Leotta et al, “Improving test suites maintainability with the page object pattern”, ICSTW 2013](http://scholar.google.com/citations?view_op=view_citation&hl=en&user=n010PrAAAAAJ&citation_for_view=n010PrAAAAAJ:roLk4NBRz8UC)

## Acknowledgements

Perryn Fowler, Pete Hodgson, and Simon Stewart gave particularly useful comments on drafts of this post - although as usual I owe much to various denizens of Thoughtworks's internal software development list for their suggestions and corrections.
