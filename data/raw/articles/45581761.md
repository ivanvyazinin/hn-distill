...
## The expected migration work [🔗︎](#the-expected-migration-work)
...
2.  The `litestream.yml` configuration file format has changed slightly. There used to be an array field called `replicas`, but 0.5.0 changes this to a dictionary called `replica` (singular).
...
## My test migration to Litestream 0.5.0 [🔗︎](#my-test-migration-to-litestream-050)
...
2.  The server kept failing due to [a bug in Litestream 0.3.13](https://github.com/benbjohnson/litestream/issues/688) that was fixed in 0.5.0.

### Uploading to Backblaze backends no longer works [🔗︎](#uploading-to-backblaze-backends-no-longer-works)
...
### `-if-replica-exists` disappeared [🔗︎](#-if-replica-exists-disappeared)
...
### Restore fails with `transaction not available` [🔗︎](#restore-fails-with-transaction-not-available)
...
### Litestream no longer creates directories [🔗︎](#litestream-no-longer-creates-directories)
...
[Twitter](https://twitter.com/intent/tweet?via=deliberatecoder&text=Hold%20Off%20on%20Litestream%200.5.0%20https%3a%2f%2fmtlynch.io%2fnotes%2fhold-off-on-litestream-0.5.0%2f "Share on Twitter") [Facebook](https://www.facebook.com/sharer/sharer.php?u=https%3a%2f%2fmtlynch.io%2fnotes%2fhold-off-on-litestream-0.5.0%2f "Share on Facebook") [LinkedIn](https://www.linkedin.com/shareArticle?mini=true&url=https%3a%2f%2fmtlynch.io%2fnotes%2fhold-off-on-litestream-0.5.0%2f "Share on LinkedIn")
