...
[![](/img/icon/linkedin-in.svg)](https://linkedin.com/in/nikpapas) [![](/img/icon/github.svg)](https://github.com/4rknova) [![](/img/icon/twitter.svg)](https://twitter.com/4rknova) [![](/img/icon/shadertoy.svg)](https://shadertoy.com/user/4rknova/sort=love) [![](/img/icon/artstation.svg)](https://www.artstation.com/arknova) [![](/img/icon/gscholar.svg)](https://scholar.google.com/citations?user=8loH5X0AAAAJ) [![](/img/icon/mobygames.png)](https://www.mobygames.com/person/849617/nikolaos-papadopoulos/) [![](/img/icon/rss.svg)](https://4rknova.com/feed.xml)
...
![Foil sticker animation](/img/post/20250830/anim01.gif)
...
Overall peel strength (0 = flat, 1 = fully peeled).

`uPeelAngle`

float

Peel direction in degrees.
...
Distance moved by vertex, used to darken lifted areas.

The shader goes through the following simple steps:

1.  Compute vector from hinge to current vertex.
2.  Calculate the peel factor and angle.
3.  Define the rotation axis and apply Rodrigues’ rotation formula to rotate the vertex around that axis.
4.  Apply the same rotation to the normal.
5.  Calculate a fake ambient occlusion term.

![Information label](/img/icon/info.svg)

The Rodrigues rotation formula is a compact way to rotate a 3D vector around an arbitrary axis. It takes a vector \\(\\mathbf{v}\\), a unit axis of rotation \\(\\mathbf{k}\\), and an angle \\(\\theta\\) and returns the rotated vector \\(\\mathbf{v}\_{rot}\\)
...
Discard pixels below this alpha.
...
Randomness threshold for flakes.

`uFlakeThreshold`

float

Brightness threshold to show flakes.

`uFlakeBrightness`

float

Base brightness of flakes.

`uMetalness`

float

PBR-like metal reflectivity control.
...
Mask controlling metallic regions.

`uIridescence`

float

Strength of angle-dependent rainbow effect.

`uIriMin`, `uIriRange`

float

Range for simulated film thickness.

`uPeelAmount`, `uPeelAngle`

float

Peel geometry info for shading.

This is how this works:

1.  Alpha cutoff to discard transparent pixels early.
2.  Back-face shading to render the rear surface as plain white or darkened, depending on peel.
3.  Foil flakes are computed using procedural noise. Normals are perturbed slightly to create sparkle variation. The environment map is sampled to get an iridescent tint.
4.  Iridescence (thin-film approximation) is calculated using sine-based waves to shift hue by view angle.
5.  Environment reflections are modulated by Fresnel.
6.  Final shading combines diffuse base, reflections, iridescence, and flakes.
