...
[![Bitrig](/assets/logos/logo_bug.png)](/)

[Home](/)[About](/about)

[![Download on the App Store](/assets/app-store/Download_on_the_App_Store_Badge_US-UK_RGB_wht_092917.svg)](https://apps.apple.com/app/apple-store/id6747835910?pt=127942101&ct=Top%20Navigation&mt=8)

[About](/about)[![Download on the App Store](/assets/app-store/Download_on_the_App_Store_Badge_US-UK_RGB_wht_092917.svg)](https://apps.apple.com/app/apple-store/id6747835910?pt=127942101&ct=Top%20Navigation&mt=8)

# How we built an interpreter for Swift (a compiled language)

September 4, 2025by Jacob Xiao
...
`func evaluateInitializer(type: String, arguments: [Argument]) -> Any? {   if type == "Text" {     return Text(arguments.first?.value.stringValue ?? "")   } else if type == "Image" {     return Image(arguments.first?.value.stringValue ?? "")   ... }`
...
`struct ShapeStub: Shape {     var interpreter: Interpreter     var instance: CustomInstance     var layoutDirectionBehavior: LayoutDirectionBehavior {         instance.instanceMemberProperty("layoutDirectionBehavior", interpreter: interpreter).layoutDirectionBehaviorValue     }     func path(in rect: CGRect) -> Path {         let arguments = [Argument(label: "in", value: rect)]         return instance.instanceFunction("path", arguments: arguments, interpreter: interpreter).pathValue     } }`
...
`struct ViewStub: View {     var interpreter: Interpreter     var instance: CustomInstance     var body: AnyView {         instance.instanceMemberProperty("body", interpreter: interpreter).viewValue             .map { AnyView($0) }   } }`
...
© 2025 Bitrig. All rights reserved.
