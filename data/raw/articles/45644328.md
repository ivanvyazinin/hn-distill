...
*   [All Posts](/posts)
*   [Research Log](/posts/research-log)
*   **Popular Posts**

*   [BERT is just a Single Text Diffusion Step](/posts/roberta-diffusion/)
*   [Running GPT-2 in WebGL: Rediscovering Classic GPGPU Programming](/posts/gpu-shader-programming/)
*   [How to Fix Hugo's iOS Code-Block Text-Size Rendering Issue](/posts/fixing-ios-codeblocks/)
...
*   [Research Log](/posts/research-log/)
*   [Running GPT-2 in WebGL: Rediscovering Classic GPGPU Programming](/posts/gpu-shader-programming/)
*   [Mathematical Statistics](/posts/statistics/)
*   [Common Probability Distributions](/posts/probability-distributions/)

*   [𝕏](https://x.com/nathanbarrydev)
*   [Github](https://github.com/nathan-barry)
*   [LinkedIn](https://www.linkedin.com/in/nathanbarrydev/)

 ![Menu](/icons/menu.svg)![Table of Contents](/icons/toc.svg)

*   *   [A Short History of Transformers](#a-short-history-of-transformers)
    *   [Discrete Language Diffusion Models](#discrete-language-diffusion-models)
    *   [RoBERTa Diffusion](#roberta-diffusion)
    *   [Conclusion](#conclusion)

[![A doorknob](/doorknob.svg)](/)

# BERT is just a Single Text Diffusion Step

![Calendar](/icons/calendar.svg) October 20, 2025

[Machine Learning](/tags/Machine-Learning/)

![Text Diffusion](/images/roberta-diffusion.gif)
...
I read the paper [Large Language Diffusion Models](https://arxiv.org/abs/2502.09992) and was surprised to find that discrete language diffusion is just a generalization of masked language modeling (MLM), something we’ve been doing since [2018](https://arxiv.org/abs/1810.04805). The first thought I had was, “can we finetune a BERT-like model to do text generation?” I decided to try a quick proof of concept out of curiosity.
...
Here we use the HuggingFace `transformers` and `dataset` libraries to pull in the original RoBERTa weights, tokenizer, and the Trainer class to easily finetune the model on the WikiText dataset. The main code ([full code here](https://github.com/nathan-barry/RoBERTaDiffusion)) looks like this below:
...
Currently we have 10 diffusion steps, so we randomly sample a percentage $p$ out of `mask_probs` (1.0, 0.9, 0.9, …, 0.1) and mask that percent of the tokens each batch. The custom `diffusion_collator` function ([see code here](https://github.com/nathan-barry/RoBERTaDiffusion/blob/main/finetune.py#L77)) samples one mask-probability `p` from `mask_probs` per batch and sets each token to `<MASK>` with `p` probability.
...
![RoBERTa Diffusion vs GPT](/images/roberta-diffusion-gpt.gif)
...
Even without architectural changes, a fine-tuned RoBERTa can generate coherent looking text after slightly modifying the training objective, validating the idea that BERT-style models are essentially just text diffusion models trained on one masking rate.
 ...
 [![Calendar](/icons/calendar.svg) Last modified: October 20, 2025](https://github.com/nathan-barry/nathan.rs/commit/098faab7972fac3ab806ed680b1934583aae462e "Last modified by Nathan Barry | October 20, 2025")

[Research Log ![Forward](/icons/forward.svg)](/posts/research-log/) 

nathan.rs - © 2025 Nathan Barry
