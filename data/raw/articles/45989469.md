...
[Blog](/) [Newsletter](/mail/archive) [Feed](/rss) [Books](/books) [Tempest](https://tempestphp.com)

# What's new in PHP 8.5

Written on 2025-11-20

PHP 8.5 was released on [November 20, 2025](https://wiki.php.net/todo/php85). It includes the pipe operator, clone with, a new URI parser, and more.
...
$output = $input
    |> trim(...)
    |> (fn (string $string) => str\_replace(' ', '-', $string))
    |> (fn (string $string) => str\_replace(\['.', '/', '…'\], '', $string))
    |> strtolower(...);
...
I think this is a great feature. The only thing I find unfortunate is that it doesn't work when cloning readonly properties from the outside (which I think is a common use case). To do so, you have to specifically reset the propery's write access to `public(set)`. [I explained the problem here](https://www.youtube.com/watch?v=hkuy11kLlmM).
...
## [Added array\_first() and array\_last()](https://wiki.php.net/rfc/array_first_last)
...
$first = $array\[array\_key\_first($array)\] ?? null;

You can now write this:

$first = array\_first($array);
...
*   [There's a new `FILTER_THROW_ON_FAILURE` flag when calling `filter_var()`](https://wiki.php.net/rfc/filter_throw_on_failure)

* * *

## Deprecations and breaking changes

*   [Non-standard cast names like `(boolean)` and `(integer)` are deprecated](https://wiki.php.net/rfc/deprecations_php_8_5#deprecate_non-standard_cast_names)
*   [Backticks as an alias for `shell_exec()` are deprecated](https://wiki.php.net/rfc/deprecations_php_8_5#deprecate_backticks_as_an_alias_for_shell_exec)
...
[Back](/) [Up next: Game changing editions](https://stitcher.io/blog/game-changing-editions)

## Things I wish I knew when I started programming

[![Things I wish I knew when I started programming cover image](/img/static/books/things-cover.png)](https://things-i-wish-i-knew.com/)
...
[Read more](https://things-i-wish-i-knew.com/)
...
Noticed a tpyo? You can [submit a PR](https://github.com/brendt/stitcher.io) to fix it.

[Home](/)[RSS](/rss)[Newsletter](/mail)[Discord](https://tempestphp.com/discord)© 2025 stitcher.io