...
#   [![](https://hacks.mozilla.org/wp-content/themes/Hax/img/mdn-logo-mono.svg) ![Mozilla](https://hacks.mozilla.org/wp-content/themes/Hax/img/wordmark.svg) Hacks](https://hacks.mozilla.org)

[Hacks on YouTube](http://www.youtube.com/user/mozhacks "YouTube") [@mozhacks on Twitter](https://twitter.com/mozhacks "Twitter") [Hacks RSS Feed](https://hacks.mozilla.org/feed/ "RSS Feed") [Download Firefox](https://www.mozilla.org/firefox/download/thanks/?utm_source=hacks.mozilla.org&utm_medium=referral&utm_campaign=header-download-button&utm_content=header-download-button)

# CRLite: Fast, private, and comprehensive certificate revocation checking in Firefox

### ![](https://secure.gravatar.com/avatar/b66d9c3a8a5dfc50d372f2fee6947ffdeb533a92b4433e2f6003b39916da0edb?s=64&d=mm&r=g) By [John Schanck](https://hacks.mozilla.org/author/jschanck/)

Posted on August 19, 2025 in [Feature](https://hacks.mozilla.org/category/feature/ "View all posts in Feature"), [Firefox](https://hacks.mozilla.org/category/firefox/ "View all posts in Firefox"), [Privacy](https://hacks.mozilla.org/category/privacy/ "View all posts in Privacy"), and [Security](https://hacks.mozilla.org/category/security/ "View all posts in Security")
...
[![A graph showing "Median TLS Handshake Time (ms)" and "Revocation mechanism usage" over time. As the percentage of revocation checks performed with CRLite increases from 0% to 80%, the median TLS handshake time decreases from 56.4 ms to 39.9 ms.](https://hacks.mozilla.org/wp-content/uploads/2025/07/chart.svg)](https://hacks.mozilla.org/wp-content/uploads/2025/07/chart.svg)
...
Including all revocations is essential for security as there is no reliable way today to distinguish security-critical revocations from administrative revocations. Roughly half of all revocations are made without a specified reason code, and some of these revocations are likely due to security concerns that the certificate’s owner did not wish to highlight. When reason codes _are_ used, they are often used in an ambiguous way that does not clearly map to security risk. In this environment, the only secure approach is to check all revocations, which is now possible with CRLite.
...
[More articles by John Schanck…](https://hacks.mozilla.org/author/jschanck/)
