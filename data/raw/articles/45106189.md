...
*   [](https://www.youtube.com/c/AnthonyShaw)
*   [](/rss.xml)
*   [](# "Toggle dark mode")
...
by Anthony Shaw, September 2, 2025
...
2.  [PEP 734: Multiple interpreters in the stdlib](https://docs.python.org/3.14/whatsnew/3.14.html#pep-734-multiple-interpreters-in-the-stdlib)
...
## Problem 1: What is an asynchronous-shaped problem?[¶](#problem-1-what-is-an-asynchronous-shaped-problem "Permanent link")
...
2.  The local end (asyncio HTTP library) can yield control while waiting for the response
3.  Operating-Systems have stacks and APIs for managing sockets and network
...
### Side-Quest: Why isn’t file IO async?[¶](#side-quest-why-isnt-file-io-async "Permanent link")
...
There were io\_uring APIs for other platforms, Rust has implementations with [tokio](https://github.com/tokio-rs/tokio-uring), C++ has [Asio](https://think-async.com/Asio/asio-1.24.0/doc/asio/history.html#asio.history.asio_1_21_0) and Node.JS has [libuv](https://www.phoronix.com/news/libuv-io-uring).

So, the asyncio Wiki is a little out of date, but

1.  **Most** production Python applications run on Linux, where the implementation is `io_uring`
2.  `io_uring` has been plagued by security issues so bad that RedHat, Google and others have restricted or removed its availability. After paying out $1 million in bug bounties related to `io_uring`, [Google disabled it on some products](https://security.googleblog.com/2023/06/learnings-from-kctf-vrps-42-linux.html). The issue was severe; many of the related bug‑bounty reports involved io\_uring exploits.
...
### Recap: What are the async operations in asyncio?[¶](#recap-what-are-the-async-operations-in-asyncio "Permanent link")
...
[`asyncio.open_connection()`](https://docs.python.org/3/library/asyncio-stream.html#asyncio-streams)
...
[`asyncio.subprocess`](https://docs.python.org/3/library/asyncio-subprocess.html#asyncio-subprocess)

Asynchronously run subprocesses.

Queues

[`asyncio.Queue`](https://docs.python.org/3/library/asyncio-queue.html#asyncio-queues)

Asynchronous queue implementation.

## Problem 2: No matter how fast you run, you can’t escape the GIL[¶](#problem-2-no-matter-how-fast-you-run-you-cant-escape-the-gil "Permanent link")
...
### Does the free-threaded mode make asyncio more useful or redundant?[¶](#does-the-free-threaded-mode-make-asyncio-more-useful-or-redundant "Permanent link")

Python 3.13 introduced a very-unstable “free-threaded” build of Python where the GIL is removed and replaced with smaller, more granular locks. See my [PyCon US 2024 Talk](https://www.youtube.com/watch?v=Mp5wKOL4L2Q) for a summary of parallelism. The 3.13 build wasn’t stable enough for any production use. 3.14 is looking far improved and I think we can start to introduce free-threading in 2026 in some narrow, well-tested scenarios.
...
## Problem 3: Maintaining two APIs is hard[¶](#problem-3-maintaining-two-apis-is-hard "Permanent link")
...
### Async properties[¶](#async-properties "Permanent link")
...
### Duplicated implementations[¶](#duplicated-implementations "Permanent link")
...
### Backend fragmentation[¶](#backend-fragmentation "Permanent link")
...
### Testing overhead[¶](#testing-overhead "Permanent link")
...
![Why is Python so slow?](/img/posts/snail.jpg)

Why is Python so slow?

A look into what makes Python slower than other languages

_Published July 16, 2018_



](/posts/why-is-python-so-slow.html)

[

![async test patterns for Pytest](/img/home-bg.jpg)

async test patterns for Pytest

Some examples and patterns for testing async code from Pytest

_Published August 5, 2021_



](/posts/async-test-patterns-for-pytest-and-unittest.html)

[

![Fine Tuning Python WSGI and ASGI applications for Flask, Django, and FastAPI](/img/posts/little-python-lemonade-stand.jpeg)
...
_Published December 20, 2023_



](/posts/fine-tuning-wsgi-and-asgi-applications.html)

*   [](https://twitter.com/anthonypjshaw)
*   [](https://youtube.com/c/AnthonyShaw)
*   [](https://github.com/tonybaloney)
