...
Since then, OpenAI has released GPT-5-codex, and supposedly uses it (plus Codex, their CLI coding tool) to automate a lot of their product development and AI research. I wanted to try the same thing. Codex-plus-me did a much better job than me alone[1](#fn-1). Here’s an example of the best output I got from the model I trained with Codex:
...
Codex trained 19 different n-gram models, of which the above example (a 4-gram model) was the best[3](#fn-3). In my view, this is one of the strengths of LLM-based AI research: **it is trivial to tell the model “go and sweep a bunch of different values for the hyperparameters”**. Of course, you can do this yourself. But it’s a lot easier to just tell the model to do it.
...
This was an interesting approach for the challenge: going for pure volume and hoping for a lucky training run. You can’t do this with a larger model, since it takes so long to train[4](#fn-4), but the five-minute limit makes it possible.
...
I really like this approach. It’s exactly what I was looking for from the start: a cool architectural trick that genuinely helps, but only really makes sense for this weird challenge[5](#fn-5).
...
If you liked this post, consider [subscribing](https://buttondown.com/seangoedecke) to email updates about my new posts, or [sharing it on Hacker News](https://news.ycombinator.com/submitlink?u=https://www.seangoedecke.com/ai-research-with-codex/&t=GPT-5-Codex is a better AI researcher than me).

October 7, 2025 │ Tags: [ai](/tags/ai/), [projects](/tags/projects/)
...
*   [← How I influence tech company politics as a staff software engineer](/how-to-influence-politics/)
