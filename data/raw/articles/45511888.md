...
![](/media/images/blog-arc-prize-team.jpg)

By [ARC Prize Team](https://x.com/arcprize)
...
On June 8, 2025, the [Hierarchical Reasoning Model](https://arxiv.org/abs/2506.21734) (HRM) paper was published by [Guan Wang](https://www.linkedin.com/in/guan-wang-447402338) et al. The release went viral within the AI community. X/Twitter discussions hit over 4 million views and tens of thousands of likes \[[1](https://x.com/makingAGI/status/1947286324735856747), [2](https://x.com/deedydas/status/1951677875004100814), [3](https://x.com/omarsar0/status/1951751651729060081), [4](https://x.com/heyshrutimishra/status/1951231278700965977)\] and YouTube videos dissecting the work surpassed 475K views \[[1](https://www.youtube.com/watch?v=0OnyVmj6yxY), [2](https://youtu.be/xI-7KO1UmgE?t=2026), [3](https://www.youtube.com/watch?v=RK7lysjz_G0), [4](https://www.youtube.com/watch?v=TUsbk8vPDoM)\].

The headline [claim](https://x.com/makingAGI/status/1947286324735856747) of the paper: the brain-inspired architecture of HRM scored 41% on ARC-AGI-1 with only 1,000 training tasks and a 27M (relatively small) parameter model.
...
First of all: we were able to approximately reproduce the claimed numbers. [HRM](https://github.com/sapientinc/HRM) shows impressive performance for its size on the ARC-AGI Semi-Private sets:
...
Findings 2 & 3 suggest that the paper's approach is fundamentally similar to [Liao and Gu's "ARC-AGI without pretraining"](https://iliao2345.github.io/blog_posts/arc_agi_without_pretraining/arc_agi_without_pretraining.html).

Want to skip to the technical findings? See [Analyzing HRM's Contribution to ARC Scores](#analyzing-hrms-contribution-to-arc-scores).

* * *

### Hierarchical Reasoning Model

![HRM Overview](/media/images/hrm-overview.jpg)

Figure 1: The HRM approach draws inspiration from signal processing in the brain at different frequencies.

The [Hierarchical Reasoning Model](https://github.com/sapientinc/HRM), published by [Sapient](https://sapient.inc/models), a Singapore-based AI research lab, is said to be inspired by the hierarchical and multi-time scale processing of the human brain.

HRM is a 27M parameter model that uses **iterative refinement** over a few short "thinking" bursts.

Each burst produces:

1.  A predicted output grid - This is the model's work-in-progress prediction to an ARC-AGI task.
2.  A "halt or continue" score - This score determines whether or not to continue refining the prediction or submit it as final.
...
Within the outer loop, HRM runs two coupled, recurrent modules **"H"** (slow planner) and "**L"** (fast worker). These two modules work together to update a _shared_ hidden state rather separate outputs. The net effect is that the model alternates between plan (H) and details (L) until the internal state "agrees with itself" enough to produce an answer.
...
*   [ARC-AGI-1](https://arcprize.org/arc-agi/1/) - 2019, designed to challenge deep learning systems
*   [ARC-AGI-2](https://arcprize.org/arc-agi/2/) - 2025, designed to challenge reasoning systems

We test selected bespoke solutions, like HRM, in accordance with our [testing policy](https://arcprize.org/policy). To be considered for verification, solutions must be open source, cost less than $10K to run, and complete in under 12 hours.

* * *

### Official Verified HRM ARC-AGI Scores

![ARC-AGI-1 Leaderboard with HRM](/media/images/leaderboard_HRM.png)

Figure 2: ARC-AGI-1 [Leaderboard](/leaderboard) with HRM performance over cost per task.

#### ARC-AGI-1 (100 Tasks)

_Score: 32%, Runtime: 9h 16m, Total Cost: $148.50 ($1.48/task)_

32% on ARC-AGI-1 is an impressive score with such a small model. A small drop from HRM's claimed Public Evaluation score (41%) to Semi-Private is expected. ARC-AGI-1's Public and Semi-Private sets have not been difficulty calibrated. The observed drop (-9pp) is on the high side of normal variation. If the model had been _overfit_ to the Public set, Semi-Private performance could have collapsed (e.g., ~10% or less). This was not observed.

This result shows that, indeed, something interesting is going on with HRM.

Note: The relatively high cost of running HRM is due to training and inference coupled in a single run. The paper's authors have mentioned they are working to _decouple_ this process in order to submit their solution to the [ARC Prize 2025](https://arcprize.org/competitions/2025/) Kaggle competition.

#### ARC-AGI-2 (120 Tasks)

_Score: 2%, Runtime: 12h 35m, Total Cost: $201 ($1.68/task)_

ARC-AGI-2 is significantly harder than ARC-AGI-1, so a large performance drop is expected. Unlike ARC-AGI-1, the ARC-AGI-2 Public and Semi-Private sets _are_ difficulty-calibrated. Scores on each should, in principle, be similar. Though >0% shows model strength, we do not consider 2% as meaningful progress on ARC-AGI-2.
...
We looked closer at the 4 main components of the HRM paper: the HRM model architecture, the H-L hierarchical computation, the outer refinement loop, and the use of data augmentation. [Ndea](https://ndea.com/) researcher [Konstantin Schürholt](https://kschuerholt.github.io/) drove this analysis.
...
2.  Replace the HRM model with a similar sized transformer.
...
![HRM vs Transformer Performance](/media/images/hrm_transformer_performance.png)
...
In addition to the hierarchical architecture, the HRM paper proposes to use an **outer loop** ("recurrent connectivity") around the model. This feeds the model output _back_ into itself, allowing the model to iteratively refine its predictions.
...
This part of the HRM method is similar to the [Universal Transformer](https://arxiv.org/abs/1807.03819), which features both a recurrent refinement loop around a transformer model, as well as a version of ACT.

In our second series of experiments, we wanted to understand what impact the outer refinement loop, as well as ACT, have on the overall performance. We varied the number of maximum outer loops during _training_ while using maximum loops during inference (following the HRM implementation).

![ACT chart by number of loops](/media/images/hrm_act_loops.png)
...
![ACT chart by number of loops](/media/images/hrm_perf_by_act_type.png)
...
In the original paper, HRM is trained on augmented versions of the _demonstration pairs_ of 960 tasks:

*   The 400 tasks from the ARC-AGI-1 training set.
*   160 tasks from the closely-related [ConceptARC](https://arxiv.org/abs/2305.07141) benchmark.
*   The 400 tasks from the ARC-AGI-1 evaluation set, which are the tasks being targeted during evaluation.

To note, this does not imply any data leakage, since the model never sees at training time the _test pairs_ of the evaluation tasks – which is what the model gets evaluated on.
...
With this setup, the model reached 31% pass@2 accuracy, a relatively small drop from the original 41%. This suggests that the large majority of the performance is driven by training on the tasks seen at evaluation time. This means that what HRM does is fundamentally a _zero-pretraining test-time training_ approach, similar to [Liao and Gu's "ARC-AGI without pretraining"](https://iliao2345.github.io/blog_posts/arc_agi_without_pretraining/). This amounts to using the model as a kind of program synthesis substrate – using gradient descent on the demonstration pairs of a task to encode in the weights of the model a program that performs the task.
...
![HRM Task Augmentations](/media/images/hrm_task_augmentations.png)
...
1.  The number of maximum augmentations when compiling the dataset
2.  The number of maximum predictions used for the majority vote
...
![HRM Task Augmentations](/media/images/hrm_augmentations.png)
...
The results in Figure 7 show 2 trends. First, using data augmentations _does_ improve performance significantly. However, only using 300 augmentations, instead of the 1,000 as the paper used, achieves near max performance. Using only 30 augmentations (3% of total used in paper) comes within 4% of max performance.
...
At training and inference time, the model _only_ receives the input and the `puzzle_id` - there is no few-shot context with other input-output examples of the task. The HRM model has to learn to relate a `puzzle_id` to a specific transformation, so that it can predict the output from the input.

To that end, it feeds the `puzzle_id` into a large embedding layer. This embedding layer is key - without it, the model would not know what to do with an input. This presents a major limitation: the model can only be applied on puzzles with `puzzle_id`s it has seen at training time.
...
For the same reason, in this version of HRM, the inference data _has_ to be part of the training dataset. Varying, for example, the number of augmentations separately is not straight forward.
...
To reproduce our experiments and continue the research, we've [published our code](https://github.com/arcprize/hierarchical-reasoning-model-analysis). It builds on the official HRM code and adds small modifications. The repository contains instructions of how to set up the environment, change parameters, and run the experiments.

Thanks to the paper authors for sharing their results and for their responsiveness in discussing their work. Thanks to researcher [Konstantin Schürholt](https://kschuerholt.github.io/) of [Ndea](https://ndea.com/) for leading this analysis.
...
This post is also available via [slide deck](https://docs.google.com/presentation/d/12IAuVKZXvbW6uCwzDhzN1PBh4fdjypjqyucYzoKJKMg/edit?usp=sharing).

[![](/media/images/arc-prize-logo-secret.svg)](/)

*    [![](/media/images/icon-email.svg) Newsletter](https://arcprize.kit.com/bc80575d89)
*    [![](/media/images/icon-discord.svg) Discord](https://discord.gg/9b77dPAmcA)
*    [![](/media/images/icon-x.svg) Twitter](https://twitter.com/arcprize)
*    [![](/media/images/icon-youtube.svg) YouTube](https://www.youtube.com/channel/UC_rdrp-QkrZn-ce9uCE-0EA)
*   [![](/media/images/icon-github.svg)GitHub](https://github.com/arcprize/ARC-AGI-2)

© 2025 ARC Prize, Inc. [Privacy](/privacy) [Terms](/terms) [Donate](/donate) [Testing Policy](/policy)
...
Competition is now open on Kaggle! Mar 26 - Nov 3. Sign up below for details.
