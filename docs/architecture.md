...
### Backfill извлечения (одноразовая миграция)

Кэшированный `data/raw/articles/{id}.md` — это уже пост-turndown полной страницы, поэтому Readability помогает только при повторном скачивании. `scripts/backfill-extraction.mts` инвалидирует кэш для набора историй (`--ids` или все из индекса), чтобы следующий `data:summarize` пере-извлёк статью и прогнал детектор. Пересборка сама по себе включается через `EXTRACT_POLICY_VERSION` в `inputHash`. Для worker/D1: `listPendingStoryIds` исключает `post_status='ok'`, поэтому нужно ещё сбросить статус:
