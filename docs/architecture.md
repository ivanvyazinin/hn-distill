...
Worker не использует `UPDATE processing_state`: обычная выборка ограничена
`stories.updated_at = current fetchedISO`, поэтому такой reset не охватывает историю.
Вместо этого есть отдельный opt-in drain. Он выбирает `article_extracts.source_kind IS NULL`
по всей D1 независимо от текущего `TOP_N`, но обрабатывает не более
`WORKER_SUMMARIZE_MAX_PER_CRON` записей за cron. Для запуска:

1. Проверить размер очереди без изменений данных:

```bash
bunx wrangler d1 execute hn_distill --remote \
  --command "SELECT COUNT(*) AS remaining FROM article_extracts a JOIN stories s ON s.id=a.story_id WHERE a.source_kind IS NULL AND s.url IS NOT NULL AND TRIM(s.url) != ''"
```

2. Выставить `WORKER_EXTRACTION_BACKFILL_ENABLE = "true"` в `[vars]` файла
   `wrangler.toml`, убедиться, что `POST_SUMMARY_ONLY_IF_MISSING=false`, и задеплоить
   Worker. Каждый cron напрямую ставит найденные ID в очередь (или обрабатывает
   inline), поэтому старые истории не обязаны входить в текущий fetch.
3. Повторять read-only запрос выше. Когда `remaining = 0`, вернуть flag в `"false"`
   и задеплоить ещё раз. Если fetch временно не удался, строка остаётся legacy и
   будет повторена после `WORKER_RETRY_COOLDOWN_SECONDS`.
