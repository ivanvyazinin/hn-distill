...
## Диагностика отклонённого сжатия комментариев

В `hourly-build` и `daily-catchup` каждая семантически отклонённая попытка сжатия
сохраняется отдельным JSON в Actions artifact
`comments-compress-rejections-<run_id>-<run_attempt>` (хранение 7 дней).
Загрузка выполняется и после ошибки пайплайна; отсутствие отказов не создаёт
артефакт. Ошибка записи или загрузки диагностики не блокирует публикацию.

Скачать артефакт конкретного запуска:

```sh
gh run download <run_id> --repo ivanvyazinin/hn-distill \
  --name comments-compress-rejections-<run_id>-<run_attempt> \
  --dir /tmp/comments-compress-rejections-<run_id>-<run_attempt>
```

JSON содержит `schemaVersion: 1`, `createdISO`, `storyId`, `model`, `hop`
(начиная с 1), `reason`, `triggers`, `text`, `sourceHash` и `sourceChars`.
`text` — полный отклонённый текст после очистки, именно тот, который проверял
валидатор. Для `latin_prose` поле `triggers[].detail` содержит до пяти совпавших
слов/фрагментов существующей эвристики. Сопоставьте их с `text`, прежде чем
считать отказ дефектом модели или ложным срабатыванием фильтра.

Первая отклонённая попытка сохраняется, даже если следующая модель успешно
выполнила сжатие. Два JSON с одним `storyId`, но разными `hop` — две попытки,
не две проблемные карточки. Успешные ответы и транспортные ошибки сюда не пишутся.
Ранние отказы по размеру или доле кириллицы имеют `reason`, но пустой `triggers`.

Локально запись включается через `COMMENTS_COMPRESS_DIAGNOSTICS_DIR=/tmp/comments-compress-rejections`;
пустое или отсутствующее значение отключает файловую диагностику.
В Actions каталог находится в `runner.temp`, вне `data/`, VPS-бэкапа и Pages.
Промпт, полный исходный пересказ и настройки доступа в JSON не сохраняются.
Диагностика не меняет фильтр, выбор следующей модели или состояние `rejected`
и не восстанавливает ответы прошлых запусков.
